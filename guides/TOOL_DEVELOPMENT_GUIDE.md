...
export const TOOL_GROUPS: Record<string, ToolGroupConfig> = {
    // ... existing groups
    external: {
        tools: ["your_tool_name", "web", "fetch"],
    }
}
```
...
Following this pattern guarantees that both `web` and `fetch` (and any new tool) instantly display their results in the VS Code sidebar once the user approves the call.
