...
export function convertModelNameForVertex(modelId: string): string {
	return modelId.replace(/-(\d{8})$/, "@$1")
}

export function getClaudeCodeModelId(modelId: ClaudeCodeModelId | string, useVertex: boolean = false): string {
	if (useVertex) {
		return convertModelNameForVertex(modelId)
	}
	return modelId
}
