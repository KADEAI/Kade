...
export interface ExtensionStateContextType extends ExtensionState {
	...
	setInfinitySavedPrompts: (value: Array<{ id: string; label: string; prompt: string }>) => void
...
export const mergeExtensionState = (prevState: ExtensionState, newState: ExtensionState) => {
	...
	if (prevState.clineMessages && newClineMessages) {
		...
		if (prevState.clineMessages.length === newClineMessages.length) {
			...
			for (let i = 0; i < newClineMessages.length; i++) {
				...
				if (prev.ts !== next.ts || prev.text !== next.text || prev.partial !== next.partial) {
					allSame = false
					break
				}
			}
	...
	return { ...rest, apiConfiguration, customModePrompts, customSupportPrompts, experiments, clineMessages }
}
...
export const ExtensionStateContextProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	...
	const pendingMessageUpdatesRef = React.useRef<Map<number, ExtensionMessage>>(new Map())
	...
	const flushMessageUpdates = useCallback(() => {
		...
		setState((prevState) => {
			...
			for (const message of updates.values()) {
				...
				if (payloadTaskId && next.currentTaskItem?.id && payloadTaskId !== next.currentTaskItem.id) continue
				const lastIndex = findLastIndex(next.clineMessages, (msg) => msg.ts === clineMessage.ts)
				if (lastIndex === -1) continue
				const existingMsg = next.clineMessages[lastIndex]
				if (existingMsg.text === clineMessage.text &&
					existingMsg.partial === clineMessage.partial &&
					existingMsg.type === clineMessage.type &&
					existingMsg.ask === clineMessage.ask &&
					existingMsg.say === clineMessage.say) continue
				const newClineMessages = next === prevState ? [...next.clineMessages] : next.clineMessages
		...
		(event: MessageEvent) => {
			...
			switch (message.type) {
				...
				case "messageUpdated": {
					...
					if (ts !== undefined) {
						...
						if (!messageUpdateRafRef.current) {
							messageUpdateRafRef.current = requestAnimationFrame(flushMessageUpdates)
						}
					}
					break
				}
				...
				case "resourceMonitorData": {
					setState((prevState) => ({ ...prevState, resourceMonitorData: message.payload }))
					break
				}
				case "dismissedUpsells": {
					if (message.list) {
						setState((prevState) => ({ ...prevState, dismissedUpsells: message.list }))
					}
					break
				}
			}
		},
		[setListApiConfigMeta, flushMessageUpdates],
	)
	...
	const contextValue: ExtensionStateContextType = {
		...
		setSubAgentToolEnabled: (value) => setState((prevState) => ({ ...prevState, subAgentToolEnabled: value })),
		setShowSubAgentBanner: (value) => setState((prevState) => ({ ...prevState, showSubAgentBanner: value })),
		setApiConfiguration,
		...
		setCollapseCodeToolsByDefault: (value) =>
			setState((prevState) => ({ ...prevState, collapseCodeToolsByDefault: value })),
		...
		setInfinityEnabled: (value) => setState((prevState) => ({ ...prevState, infinityEnabled: value })),
		setInfinityPrompt: (value) => setState((prevState) => ({ ...prevState, infinityPrompt: value })),
		setInfinityIntervalMinutes: (value) => setState((prevState) => ({ ...prevState, infinityIntervalMinutes: value })),
		setInfinityIsRunning: (value) => setState((prevState) => ({ ...prevState, infinityIsRunning: value })),
		setInfinityScheduleType: (value) => setState((prevState) => ({ ...prevState, infinityScheduleType: value })),
		setInfinityScheduleHour: (value) => setState((prevState) => ({ ...prevState, infinityScheduleHour: value })),
		setInfinityScheduleMinute: (value) => setState((prevState) => ({ ...prevState, infinityScheduleMinute: value })),
		...
		setSlidingWindowSize: (value) => setState((prevState) => ({ ...prevState, slidingWindowSize: value })),
	}
