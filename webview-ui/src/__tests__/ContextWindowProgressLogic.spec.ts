...
describe("ContextWindowProgress Logic", () => {
  ...
  test("calculates correct token distribution with default 8192 reservation", () => {
    ...
    expect(
      result.currentPercent + result.reservedPercent + result.availablePercent,
    ).toBeCloseTo(100);
  });
  ...
  test("uses provided maxTokens when available instead of default calculation", () => {
    ...
    const defaultResult = calculateTokenDistribution(
      contextWindow,
      contextTokens,
    );
    ...
    const customResult = calculateTokenDistribution(
      contextWindow,
      contextTokens,
      customMaxTokens,
    );
    ...
    expect(
      customResult.currentPercent +
        customResult.reservedPercent +
        customResult.availablePercent,
    ).toBeCloseTo(100);
  });
