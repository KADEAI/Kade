...
vi.mock("@vscode/webview-ui-toolkit/react", () => ({
  VSCodeBadge: ({ children }: { children: React.ReactNode }) => (
    <div data-testid="vscode-badge">{children}</div>
  ),
}));
...
vi.mock("@src/components/chat/TaskHeader", async () => {
  const originalModule = await vi.importActual(
    "@src/components/chat/TaskHeader",
  );
...
describe("ContextWindowProgress", () => {
  ...
  it("renders correctly with valid inputs", () => {
    ...
    expect(screen.getByTestId("context-tokens-count")).toHaveTextContent(
      "1000",
    ); // contextTokens
    ...
    expect(screen.getByTestId("context-window-size")).toHaveTextContent(
      /(4000|128000)/,
    ); // contextWindow
  });
  ...
  it("calculates percentages correctly", () => {
    ...
    expect(
      progressBarContainer?.querySelector(".flex-1.relative"),
    ).toBeInTheDocument();
  });
});
