...
describe("Command Autocomplete", () => {
  ...
  describe("slash command command suggestions", () => {
    it('should return all commands when query is just "/"', () => {
      ...
      const commandOptions = options.filter(
        (option) => option.type === ContextMenuOptionType.Command,
      );
    ...
    it("should handle commands with special characters in names", () => {
      ...
      const setupDevOption = options.find(
        (option) => option.value === "setup-dev",
      );
  ...
  describe("command integration with modes", () => {
    ...
    it("should return both modes and commands for slash commands", () => {
      ...
      const modeOptions = options.filter(
        (option) => option.type === ContextMenuOptionType.Mode,
      );
      const commandOptions = options.filter(
        (option) => option.type === ContextMenuOptionType.Command,
      );
  ...
  describe("argument hint functionality", () => {
    it("should include argumentHint in command options when present", () => {
      ...
      const releaseOption = options.find(
    ...
    it("should handle commands with empty argumentHint", () => {
      const commandsWithEmptyHint: Command[] = [
        { name: "test-command", source: "project", argumentHint: "" },
      ];
      ...
      const options = getContextMenuOptions(
        ...
        [],
        [],
        commandsWithEmptyHint,
      );

      const testOption = options.find(
        (option) => option.value === "test-command",
      );
  ...
  describe("edge cases", () => {
    ...
    it("should handle very long command names", () => {
      ...
      expect(commandOptions[0].value).toBe(
        "very-long-command-name-that-exceeds-normal-length",
      );
    });
