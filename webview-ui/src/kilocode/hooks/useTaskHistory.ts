...
function fetchTaskHistory(
  payload: TaskHistoryRequestPayload,
): Promise<TaskHistoryResponsePayload> {
  return new Promise((resolve, reject) => {
    const cleanup = () => {
      window.removeEventListener("message", handle);
    };
...
export function useTaskHistory(
  ...
  return useQuery({
    queryKey: [
      "taskHistory",
      String(taskHistoryVersion),
      JSON.stringify(payload),
    ],
    queryFn: () =>
      fetchTaskHistory({ ...payload, requestId: crypto.randomUUID() }),
