...
export function useAgentManagerMessages() {
	...
	const setManualFolders = useSetAtom(manualFoldersAtom)
	const setSessionToFolder = useSetAtom(sessionToFolderAtom)
	const setCollapsedFolders = useSetAtom(collapsedFoldersAtom)
	const setWorkspaceConfigLoaded = useSetAtom(workspaceConfigLoadedAtom)
			...
			switch (message.type) {
				...
				case "agentManager.state": {
					...
					for (const session of state.sessions) {
						...
						// ALWAYS sync state machine if backend says session is finished
						// This fixes the "stuck spinner" issue on old chats
						if (session.status === "done") {
							sendSessionEvent({
								sessionId: session.sessionId,
								event: { type: "ask_completion_result" },
							})
							...
							sendSessionEvent({
								sessionId: session.sessionId,
								event: { type: "ask_api_req_failed" }, // Transitions to error state
							})
						}
					}
				...
				case "agentManager.workspaceConfig": {
					const { config } = message as any
					if (config) {
						if (config.manualFolders) setManualFolders(config.manualFolders)
						if (config.sessionToFolder) setSessionToFolder(config.sessionToFolder)
						if (config.collapsedFolders) setCollapsedFolders(new Set(config.collapsedFolders))
