...
import { selectedFolderIdAtom, manualFoldersAtom } from "../state/atoms/workspaces"
...
function NewAgentForm() {
	...
	// Added for Claudix port parity
	const mergedSessions = useAtomValue(mergedSessionsAtom)
	const totalMessages = mergedSessions.length
	...
	const manualFolders = useAtomValue(manualFoldersAtom)

	const selectedFolder = useMemo(() => {
		if (!selectedFolderId) return null
		return manualFolders.find((f) => f.id === selectedFolderId)
	}, [selectedFolderId, manualFolders])

	const cwdName = selectedFolder?.name || "Workspace"
	...
	const handleStart = () => {
		...
		vscode.postMessage({
			...
			labels,
			existingBranch: effectiveRunMode === "worktree" ? selectedBranch || undefined : undefined,
		})
	}
	...
	/* Form render */
	return (
		<div className="am-center-form am-empty-manager">
			<div className="am-manager-header-row">
				<h2 className="am-start-title">
					Start new conversation in
					<span className="am-workspace-selector">
						<ChevronDown size={14} />
						{cwdName}
					</span>
				</h2>

				<div className="am-view-inbox" onClick={showInbox}>
					<Inbox size={14} />
					Inbox <span className="am-inbox-count">({totalMessages})</span>
				</div>
			</div>
			...
			<div style={{ width: "100%", maxWidth: "100%", minWidth: 0 }}>
				<div
					...
					<DynamicTextArea
						...
						disabled={isStarting}
						placeholder={t("sessionDetail.placeholderTask")} // "Ask anything, @ for context"
						minRows={5}
						maxRows={12}
						style={{
							paddingBottom: "40px",
							/* Top/side padding handled by CSS class now */
						}}
						className={cn(
							"am-manager-input",
							"w-full",
					...
					<div
						className="absolute bottom-[1px] left-2 right-2 h-10 pointer-events-none z-[2]"
						aria-hidden="true"
					/>
				...
				<div className="am-quick-actions">
					<div className="am-action-item" onClick={openEditor}>
