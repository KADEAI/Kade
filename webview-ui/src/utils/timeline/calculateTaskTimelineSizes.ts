 ...
 */
export function calculateTaskTimelineSizes(
  ...
  const rawData = messages.map((message, index) => {
    ...
    const timingMs = calculateMessageTiming(
      message,
  ...
  return rawData.map(({ contentLength, timingMs }, index) => {
    ...
    const contentRatio = Math.min(
      1,
      contentLength / Math.max(1, maxContentLength),
    );
    ...
    const effectiveTiming =
      index === messages.length - 1
        ? MIN_WIDTH_PX
        : (timingMs ?? AVERAGE_REQUEST_TIME_MS);
    ...
    const height =
      MIN_HEIGHT_PX +
      contentRatio * (MAX_HEIGHT_PX - MIN_HEIGHT_PX - TOP_PADDING_PX);
...
function calculateMessageContentLength(
  message: ClineMessage | ClineMessage[],
): number {
  if (Array.isArray(message)) {
    return message.reduce(
      (total, msg) => total + calculateMessageContentLength(msg),
      0,
    );
  }
...
function calculateMessageTiming(
  ...
  const currentTs = Array.isArray(currentMessage)
    ? currentMessage[0]?.ts
    : currentMessage.ts;
  const nextTs = Array.isArray(nextMessage)
    ? nextMessage[0]?.ts
    : nextMessage.ts;
...
function findMinMaxBoundingValues(
  ...
  const validTimings = rawData
    .map((d) => d.timingMs)
    .filter((t): t is number => t !== null);
  const maxTiming =
    validTimings.length > 0
      ? Math.max(...validTimings)
