  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data.type === "profileDataResponse") {
        ...
        if (!success) {
          console.error(
            "KILOTEL: Failed to identify Kilo user, message doesn't indicate success:",
            payload,
          );
        } else if (tokenFromMessage !== kilocodeToken) {
          console.error(
            "KILOTEL: Failed to identify Kilo user, token mismatch:",
            payload,
          );
        } else if (!email) {
          console.error(
            "KILOTEL: Failed to identify Kilo user, email missing:",
            payload,
          );
        } else {
