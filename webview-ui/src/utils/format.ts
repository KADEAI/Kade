...
const getNumberSuffix = (
  key: "thousand_suffix" | "million_suffix" | "billion_suffix",
  fallback: string,
) => {
  const translated = i18next.t(`common:number_format.${key}`, {
...
export function formatLargeNumber(num: number): string {
  if (num >= 1e9) {
    return (num / 1e9).toFixed(1) + getNumberSuffix("billion_suffix", "b");
  }
  if (num >= 1e6) {
    return (num / 1e6).toFixed(1) + getNumberSuffix("million_suffix", "m");
  }
  if (num >= 1e3) {
    return (num / 1e3).toFixed(1) + getNumberSuffix("thousand_suffix", "k");
  }
  return num.toString();
}
