...
import { reportWebviewDebugEvent } from "./webviewDebug";
...
export function initializeSourceMaps(): void {
  ...
  console.debug(
    "Initializing CSP-compatible source map support for production build",
  );
  ...
  window.addEventListener("error", async (event) => {
    if (event.error && event.error instanceof Error) {
      try {
        ...
        reportWebviewDebugEvent({
          source: "sourceMapInitializer",
          event: "window_error_enhancement_failed",
          level: "error",
          data: {
            message: event.message,
            filename: event.filename,
  ...
  window.addEventListener("unhandledrejection", async (event) => {
    if (event.reason && event.reason instanceof Error) {
      try {
        ...
        reportWebviewDebugEvent({
          source: "sourceMapInitializer",
          event: "unhandled_rejection_enhancement_failed",
          level: "error",
          data: {
            reason: event.reason,
            error: e,
    ...
    for (let i = 0; i < scripts.length; i++) {
      ...
      if (script.src) {
        ...
        fetch(script.src)
          ...
          .then((content) => {
            ...
            if (sourceMappingURLMatch && sourceMappingURLMatch[1]) {
              ...
              if (!sourceMappingURL.startsWith("data:")) {
                ...
                const baseUrl = scriptUrlObj.href.substring(
                  0,
                  scriptUrlObj.href.lastIndexOf("/") + 1,
                );
          ...
          .catch((e) =>
            console.debug("Error checking for inline sourceMappingURL:", e),
          );
      }
    }
  } catch (e) {
        ...
        if (e instanceof Error) {
          ...
          (window as any).__applySourceMaps(e).then((enhanced: Error) => {
            ...
            if ("sourceMappedComponentStack" in enhanced) {
              console.log(
                "Source mapped component stack:",
                enhanced.sourceMappedComponentStack,
              );
            }
          });
        }
        ...
        if (response.ok) {
          ...
          const originalFileName =
            sourceMap.sources && sourceMap.sources.length > 0
              ...
              : "unknown";
          console.log(
            `Source map found for ${scriptUrl}. Original file: ${originalFileName}`,
          );
          return true;
        } else {
