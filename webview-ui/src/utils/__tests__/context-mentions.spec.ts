...
describe("getContextMenuOptions", () => {
  ...
  it("should return all option types for empty query", () => {
    ...
    expect(result).toHaveLength(7); // kade_change: added image option
    expect(result.map((item) => item.type)).toEqual([
      ...
      ContextMenuOptionType.Image, // kade_change
  ...
  it("should filter by selected type when query is empty", () => {
    ...
    expect(result.map((item) => item.type)).toContain(
      ContextMenuOptionType.File,
    );
    expect(result.map((item) => item.type)).toContain(
  ...
  it("should include dynamic search results along with other matches", () => {
    ...
    expect(
      result.some((item) => item.type === ContextMenuOptionType.OpenedFile),
    ).toBe(true);
    expect(result.some((item) => item.value === "/search/result1.ts")).toBe(
      true,
    );
  });
  ...
  it("should maintain correct result ordering according to implementation", () => {
    ...
    const gitResults = result.filter(
      (item) => item.type === ContextMenuOptionType.Git,
    );

    // Find the indexes of the first item of each type in the results array
    const firstFileIndex = result.findIndex((item) =>
      fileResults.some((f) => f === item),
    );

    const firstSearchResultIndex = result.findIndex((item) =>
      searchResults.some((s) => s === item),
    );

    const firstGitResultIndex = result.findIndex((item) =>
      gitResults.some((g) => g === item),
    );
  ...
  it("should include git results when dynamic search results exist", () => {
    ...
    expect(result.some((item) => item.type === ContextMenuOptionType.Git)).toBe(
      true,
    );
    // Verify dynamic search results are also present
    expect(result.some((item) => item.value === "/search/result1.ts")).toBe(
  ...
  it("should deduplicate items correctly when combining different result types", () => {
    ...
    expect(
      result.some(
        (item) =>
          item.value === "/unique/path.ts" || item.value === "unique/path.ts",
      ),
    ).toBe(true);
  });
  ...
  it("should place opened files first in result order", () => {
    ...
    const firstOpenedFileIndex = result.findIndex(
      (item) => item.type === ContextMenuOptionType.OpenedFile,
    );
    ...
    const firstGitResultIndex = result.findIndex(
      (item) => item.type === ContextMenuOptionType.Git,
    );
  ...
  it("should return search results with correct labels/descriptions (no escaping needed here)", () => {
    ...
    const fileResult = options.find(
      (o) => o.label === "search result spaces.ts",
    );
    expect(fileResult).toBeDefined();
    // Value should be the normalized path, description might be the same or label
    expect(fileResult?.value).toBe(
      "/Users/test/project/src/search result spaces.ts",
    );
    expect(fileResult?.description).toBe(
      "/Users/test/project/src/search result spaces.ts",
  ...
  it("should handle formatting of search results without escaping spaces in display", () => {
    ...
    expect(
      formattedItems[0].type !== ContextMenuOptionType.NoResults,
    ).toBeTruthy();
...
describe("shouldShowContextMenu", () => {
  ...
  it("should return true when typing path with escaped spaces", () => {
    ...
    expect(shouldShowContextMenu("@/path/to/file\\ with\\ spaces", 28)).toBe(
      true,
    ); // Cursor within path after escaped spaces
  });
  ...
  describe("slash command + @ mention interaction", () => {
    ...
    it("should return true for @ mentions in middle of slash command message", () => {
      expect(
        shouldShowContextMenu("/code fix the @component.tsx issue", 20),
      ).toBe(true);
    });

    it("should return false when @ is followed by space in slash command", () => {
      expect(shouldShowContextMenu("/newtask implement @ file", 21)).toBe(
        false,
      );
    });
    ...
    it("should return true for multiple @ mentions after slash command", () => {
      expect(
        shouldShowContextMenu("/debug check @file1.ts and @file2.ts", 30),
      ).toBe(true);
    });
  });
});
