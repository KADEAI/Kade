...
import { WAITING_FOR_USER_INPUT_TEXT } from "@roo/ExtensionMessage";
...
export const taskTimelineColorPalette = {
  USER_INTERACTION:
    "bg-[var(--vscode-charts-purple)]", // Purple for user messages
  ...
  ASSISTANT_QUESTION:
    "bg-[color-mix(in_srgb,var(--vscode-editor-findMatchBackground)_60%,var(--vscode-foreground))]", // Yellowish/tan
 ...
 */
export const TASK_TIMELINE_MESSAGE_TYPES: Record<
  string,
  TaskTimelineMessageTypeConfig
> = {
  ...
  "ask:completion_result": {
    ...
    translationKey:
      "kilocode:taskTimeline.tooltip.messageTypes.completion_result",
  },
  "ask:browser_action_launch": {
    color: taskTimelineColorPalette.SYSTEM_GENERAL_TOOL,
    translationKey:
      "kilocode:taskTimeline.tooltip.messageTypes.browser_action_launch",
  },
  "ask:use_mcp_server": {
    color: taskTimelineColorPalette.SYSTEM_GENERAL_TOOL,
  ...
  "say:mcp_server_response": {
    color: taskTimelineColorPalette.SYSTEM_GENERAL_TOOL,
    translationKey:
      "kilocode:taskTimeline.tooltip.messageTypes.mcp_server_response",
  },
  "say:command_output": {
    color: taskTimelineColorPalette.SYSTEM_GENERAL_TOOL,
  ...
  "say:checkpoint_saved": {
    color: taskTimelineColorPalette.SUCCESS,
    translationKey:
      "kilocode:taskTimeline.tooltip.messageTypes.checkpoint_saved",
  },
  "say:condense_context": {
    color: taskTimelineColorPalette.ASSISTANT_MUTTERING,
    translationKey:
      "kilocode:taskTimeline.tooltip.messageTypes.condense_context",
  },
  "say:user_feedback": {
 ...
 */
export function getMessageTypeDescription(
  ...
  const translationKey = config
    ? config.translationKey
    : "kilocode:taskTimeline.tooltip.messageTypes.unknown";
 ...
 */
export function isFileOperation(message: ClineMessage): {
  isFile: boolean;
  isWrite: boolean;
} {
  if (message.ask === "tool" && message.text) {
    try {
      ...
      console.error(
        "Failed to parse message text JSON - cannot determine if message contained toolData",
        message,
      );
    }
  }
 ...
 */
export function getTaskTimelineMessageColor(
  message: ClineMessage | ClineMessage[],
): string {
  ...
  if (singleMessage.type === "ask") {
    ...
    if (fileOp.isFile) {
      return fileOp.isWrite
        ? taskTimelineColorPalette.SYSTEM_WRITE
        : taskTimelineColorPalette.SYSTEM_READ;
    }
  }
...
export function shouldShowInTimeline(message: ClineMessage): boolean {
  if (
    ...
    message.text === WAITING_FOR_USER_INPUT_TEXT
  ) {
    return false;
  }
