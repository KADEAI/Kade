...
export const slashCommandRegexGlobal = new RegExp(
  slashCommandRegex.source,
  "g",
);
...
function enabledWorkflowToggles(
...
export function insertSlashCommand(
  ...
  const newValue =
    text.substring(0, slashIndex + 1) +
    commandName +
    (commandEndIndex > -1 ? text.substring(commandEndIndex) : " "); // add extra space at the end if only slash command
