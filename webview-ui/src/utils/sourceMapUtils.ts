 ...
 */
export async function applySourceMapsToComponentStack(
  componentStack: string,
): Promise<string> {
  if (!componentStack) {
    console.debug(
      "applySourceMapsToComponentStack: Empty component stack provided",
    );
  ...
  try {
      ...
      lines.map(async (line) => {
        ...
        try {
          ...
          if (stackFrames.length > 0) {
            ...
            const mappedLineNumber =
              frame.lineNumber || parseInt(lineNumber, 10);
            const mappedColumnNumber =
          ...
          console.debug(
            `Error processing component stack line with StackTrace.js:`,
            e,
          );
        }
    ...
    console.error(
      "Error applying source maps to component stack with StackTrace.js:",
      error,
    );
    return componentStack;
  }
 ...
 */
export function enhanceErrorWithSourceMaps(
  error: Error,
  componentStack?: string,
  ...
  return new Promise<EnhancedError>((resolve) => {
    ...
    Promise.all([stackPromise, componentStackPromise])
      .then(([sourceMappedStack, sourceMappedComponentStack]) => {
        console.debug(
          "Source mapped stacks applied successfully with StackTrace.js",
        );
      ...
      .catch((mapError) => {
        console.error(
          "Error applying source maps with StackTrace.js:",
          mapError,
        );
