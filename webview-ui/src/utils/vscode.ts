...
import { MaybeTypedWebviewMessage as WebviewMessage } from "@roo/WebviewMessage"; // kade_change - using MaybeTypedWebviewMessage
...
class VSCodeAPIWrapper {
  ...
  public postMessage(message: WebviewMessage) {
    if (typeof window !== "undefined" && message?.type === "showTaskWithId") {
      window.dispatchEvent(
        new CustomEvent("chat-task-switch-start", {
          detail: { taskId: (message as any).text },
        }),
      );
    }
    if (this.vsCodeApi) {
