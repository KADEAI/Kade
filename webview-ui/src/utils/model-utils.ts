...
export const calculateTokenDistribution = (
  ...
  const availableSize = Math.max(
    0,
    safeContextWindow - safeContextTokens - reservedForOutput,
  );
...
/**
 * Interface representing basic model information for aliasing
 */
...
/**
 * Cleans up model display names by removing "preview" and other annoying suffixes.
 */
export const getModelAlias = (
  modelId: string,
  modelInfo?: ModelAliasInfo,
): string => {
  const name = modelInfo?.displayName || modelId;
  // Handle -preview, -preview-MM-DD, -preview-YYYY-MM-DD, etc.
  return name.replace(/-preview(-\d+)?(-\d+)?/g, "").replace(/-preview/g, "");
};
