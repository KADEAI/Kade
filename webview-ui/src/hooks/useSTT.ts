...
import { LocalWhisperClient, LocalWhisperEvent } from "../services/stt/LocalWhisperClient"
...
export function useSTT(options: UseSTTOptions = {}): UseSTTReturn {
	...
	const handleLocalStatusChange = useCallback((event: LocalWhisperEvent) => {
		switch (event.status) {
			case "loading":
				setIsModelLoading(true)
				setModelLoadingProgress(typeof event.data === 'string' ? event.data : "Loading model...")
				break
			case "ready":
			...
			case "complete":
				if (event.output) {
					...
					if (finalText) {
						...
						setSegments(prev => [...prev, { text: finalText, isPreview: false }])
					}
	...
	useEffect(() => {
		...
		const handler = (event: MessageEvent) => {
			...
			if (msg.type?.startsWith("localWhisper:")) {
				console.log("🎙️ [useSTT] Received localWhisper message:", msg.type, msg)
				switch (msg.type) {
					case "localWhisper:init":
						...
						console.log("🎙️ [useSTT] Starting local whisper initialization, sessionId:", msg.sessionId)
						
						if (!localClientRef.current) {
							localClientRef.current = new LocalWhisperClient(handleLocalStatusChange)
						}
					...
					case "localWhisper:transcribe":
						...
						console.log("🎙️ [useSTT] Received audio for transcription, samples:", msg.audio?.length)
						if (msg.sessionId !== sessionIdRef.current) return
						
						if (localClientRef.current) {
							// Convert array back to Float32Array and transcribe
							const audio = new Float32Array(msg.audio)
							console.log("🎙️ [useSTT] Transcribing audio with", audio.length, "samples")
