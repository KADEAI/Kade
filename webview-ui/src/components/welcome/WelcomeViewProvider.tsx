...
const WelcomeViewProvider = () => {
  ...
  useEffect(() => {
    if (cloudIsAuthenticated && authInProgress && selectedProvider === "roo") {
  ...
  const handleGetStarted = useCallback(() => {
    if (selectedProvider === "roo") {
      ...
      // Use custom provider - no validation, just save and continue
      ...
      // Dismiss the welcome screen — same as KiloCodeAuth's "Continue" button
      const currentState = (vscode.getState() as Record<string, unknown>) || {};
      vscode.setState({ ...currentState, welcomeDismissed: true });
      window.dispatchEvent(new CustomEvent("welcomeDismissed"));
    }
  ...
  const handleManualUrlChange = (e: any) => {
    ...
    setTimeout(() => {
      if (
        url.trim() &&
        url.includes("://") &&
        url.includes("/auth/clerk/callback")
      ) {
        setManualErrorMessage(false);
      ...
      <Tab>
        <TabContent className="flex flex-col gap-4 p-6">
          <div className="flex flex-col items-start gap-4 pt-8">
            ...
            <h2 className="mt-0 mb-0 text-lg font-semibold">
