...
describe("AutoApproveToggle", () => {
  ...
  const initialProps = {
    ...
    alwaysAllowDelete: false, // kade_change
  ...
  test("renders all toggle buttons with correct initial ARIA attributes", () => {
    ...
    Object.values(autoApproveSettingsConfig).forEach((config) => {
      ...
      expect(button).toHaveAttribute(
        "aria-pressed",
        String(initialProps[config.key]),
      );
    });
  });
  ...
  test("calls onToggle with the correct key and value when a button is clicked", () => {
    ...
    const writeToggleButton = screen.getByTestId(
      autoApproveSettingsConfig.alwaysAllowWrite.testId,
    );
    ...
    const readOnlyButton = screen.getByTestId(
      autoApproveSettingsConfig.alwaysAllowReadOnly.testId,
    );
  ...
  test("updates aria-pressed attribute after toggle", () => {
    ...
    expect(
      screen.getByTestId(autoApproveSettingsConfig.alwaysAllowWrite.testId),
