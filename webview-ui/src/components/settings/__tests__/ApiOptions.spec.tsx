...
vi.mock("vscrui", () => ({
  Checkbox: ({ children, checked, onChange }: any) => (
    <label
      data-testid={`checkbox-${children?.toString().replace(/\s+/g, "-").toLowerCase()}`}
    >
      <input
        type="checkbox"
        checked={checked}
...
vi.mock("@/components/ui", () => ({
  ...
  SelectTrigger: ({ children }: any) => (
    <div className="select-trigger-mock">{children}</div>
  ),
  SelectValue: ({ children }: any) => (
    <div className="select-value-mock">{children}</div>
  ),
  SelectContent: ({ children }: any) => (
    <div className="select-content-mock">{children}</div>
  ),
  ...
  Command: ({ children }: any) => (
    <div className="command-mock">{children}</div>
  ),
  CommandEmpty: ({ children }: any) => (
    <div className="command-empty-mock">{children}</div>
  ),
  CommandGroup: ({ children }: any) => (
    <div className="command-group-mock">{children}</div>
  ),
  CommandInput: ({
    value,
    onValueChange,
  ...
  CommandList: ({ children }: any) => (
    <div className="command-list-mock">{children}</div>
  ),
  Popover: ({ children, _open, _onOpenChange }: any) => (
    <div className="popover-mock">{children}</div>
  ),
  PopoverContent: ({ children, _className }: any) => (
    <div className="popover-content-mock">{children}</div>
  ),
  PopoverTrigger: ({ children, _asChild }: any) => (
    <div className="popover-trigger-mock">{children}</div>
  ),
  ...
  SearchableSelect: ({
    ...
    <div
      className="searchable-select-mock"
      data-testid={dataTestId || "provider-popover-trigger"}
    >
      <select
        value={value}
        onChange={(e) => onValueChange && onValueChange(e.target.value)}
  ...
  CollapsibleTrigger: ({ children, className, onClick }: any) => (
    <div
      className={`collapsible-trigger-mock ${className || ""}`}
      onClick={onClick}
    >
      {children}
    </div>
  ...
  CollapsibleContent: ({ children, className }: any) => (
    <div className={`collapsible-content-mock ${className || ""}`}>
      {children}
    </div>
  ),
}));
    ...
    <div data-testid="diff-settings-control">
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) =>
            onChange("fuzzyMatchThreshold", parseFloat(e.target.value))
          }
          min={0.8}
          max={1}
          step={0.005}
  ...
  LiteLLM: ({ apiConfiguration, setApiConfigurationField }: any) => (
    <div data-testid="litellm-provider">
      <input
        ...
        onChange={(e) =>
          setApiConfigurationField("litellmBaseUrl", e.target.value)
        }
      ...
      <input
        ...
        onChange={(e) =>
          setApiConfigurationField("litellmApiKey", e.target.value)
        }
        placeholder="API Key"
      />
...
vi.mock("../providers/RooBalanceDisplay", () => ({
  RooBalanceDisplay: () => (
    <div data-testid="roo-balance-display">Balance: $10.00</div>
  ),
}));
...
describe("ApiOptions", () => {
  it("shows diff settings, temperature and rate limit controls by default", () => {
    ...
    expect(
      screen.getByText(/enable editing through diffs/i),
    ).toBeInTheDocument();
    expect(screen.getByTestId("temperature-control")).toBeInTheDocument();
    expect(
      screen.getByTestId("rate-limit-seconds-control"),
    ).toBeInTheDocument();
  });
  ...
  it("hides all controls when fromWelcomeView is true", () => {
    ...
    expect(
      screen.queryByText(/enable editing through diffs/i),
    ).not.toBeInTheDocument();
    expect(screen.queryByTestId("temperature-control")).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("rate-limit-seconds-control"),
    ).not.toBeInTheDocument();
  });
  ...
  it.skip("filters providers by search input and shows no match message when appropriate", () => {
    ...
    const providerSelect = providerSelectContainer.querySelector(
      "select",
  ...
  describe("OpenAI provider tests", () => {
    it("removes reasoningEffort from openAiCustomModelInfo when unchecked", () => {
      ...
      const checkbox =
        screen.getByTestId(
          "checkbox-input-settings:providers.setreasoninglevel",
      ...
      expect(updatedInfo).toHaveProperty(
        "contextWindow",
  ...
  describe("Anthropic provider deployment name settings", () => {
    it("shows deployment name checkbox and input when deployment name is set", () => {
      ...
      const checkbox = screen.getByTestId(
        "checkbox-input-settings:providers.anthropicuseazuredeployment",
      );
  ...
  describe("LiteLLM provider tests", () => {
    it("renders LiteLLM component when provider is selected", () => {
      ...
      expect(screen.getByTestId("litellm-base-url")).toHaveValue(
        "http://localhost:4000",
      );
  ...
  describe("Roo provider tests", () => {
    it("shows balance display when authenticated", () => {
      ...
      const useExtensionStateMock = vi.spyOn(
    ...
    it("does not show balance display when not authenticated", () => {
      ...
      expect(
        screen.queryByTestId("roo-balance-display"),
      ).not.toBeInTheDocument();
    });
  ...
  describe("OpenCode provider tests", () => {
    it("shows OpenCode in the provider selector", () => {
      renderApiOptions({
        apiConfiguration: {},
      });
    ...
    it("renders OpenCode settings when selected", () => {
      renderApiOptions({
        apiConfiguration: {
      ...
      expect(
        screen.getByRole("button", { name: "Get OpenCode API Key" }),
      ).toBeInTheDocument();
    });
  });

  describe("AIHubMix provider tests", () => {
    it("shows AIHubMix in the provider selector", () => {
      renderApiOptions({
        apiConfiguration: {},
      });
      ...
      expect(providerOptions).toContain("aihubmix");
      expect(providerOptions).toContain("bluesminds");
    });

    it("renders AIHubMix settings when selected", () => {
      renderApiOptions({
        apiConfiguration: {
      ...
      expect(
        screen.getByRole("button", { name: "Get AIHubMix API Key" }),
      ).toBeInTheDocument();
    });

    it("renders Bluesminds settings when selected", () => {
      renderApiOptions({
        apiConfiguration: {
          apiProvider: "bluesminds",
        },
      });

      expect(
        screen.getByRole("button", { name: "Open Bluesminds" }),
      ).toBeInTheDocument();
    });
  });

  describe("Zed provider tests", () => {
    it("shows Zed in the provider selector", () => {
      renderApiOptions({
        apiConfiguration: {},
      });
      ...
      expect(providerOptions).toContain("zed");
    });

    it("renders Zed settings when selected", () => {
      renderApiOptions({
        apiConfiguration: {
          apiProvider: "zed",
        },
      ...
      expect(
        screen.getByRole("button", {
          name: /sign in with zed|settings:providers\.zed\.signInButton/i,
        }),
      ).toBeInTheDocument();
    });
  });
