...
vitest.mock("@/components/ui", () => ({
  ...
  CommandList: ({ children }: any) => (
    <div className="command-list">{children}</div>
  ),
  CommandEmpty: ({ children }: any) =>
    children ? <div className="command-empty">{children}</div> : null,
  CommandGroup: ({ children }: any) => (
    <div className="command-group">{children}</div>
  ),
...
describe("ApiConfigManager", () => {
  ...
  it("creates new profile with entered name", () => {
    ...
    expect(mockOnUpsertConfig).toHaveBeenCalledWith("New Profile", "chat"); // kade_change - autocomplete profile type system
  });
  ...
  it("allows renaming the current config", () => {
    ...
    // kade_change start - use rename form for scoped queries
  ...
  it("disables delete button when only one config exists", () => {
    render(
      <ApiConfigManager
        {...defaultProps}
        listApiConfigMeta={[{ id: "default", name: "Default Config" }]}
      />,
    );
