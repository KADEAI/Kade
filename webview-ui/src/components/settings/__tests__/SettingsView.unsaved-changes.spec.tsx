...
vi.mock("../common/Tab", () => ({
  ...
  TabList: ({ children }: any) => (
    <div data-testid="settings-tab-list">{children}</div>
  ),
  TabTrigger: React.forwardRef<HTMLButtonElement, any>(
...
vi.mock("../ApiConfigManager", () => ({
  default: vi.fn(() => (
    <div data-testid="api-config-manager">ApiConfigManager</div>
  )),
}));
...
describe("SettingsView - Unsaved Changes Detection", () => {
  ...
  it.skip("should not show unsaved changes when settings are automatically initialized", async () => {
    ...
    expect(
      screen.queryByText("settings:unsavedChangesDialog.title"),
