...
vi.mock("@/utils/docLinks", () => ({
  buildDocLink: (path: string, anchor?: string) =>
    `https://docs.example.com/${path}${anchor ? `#${anchor}` : ""}`,
}));
    ...
    <div data-testid={`command-item-${command.name}`}>
      ...
      {command.source !== "built-in" && (
        <button
          ...
          data-testid={`delete-${command.name}`}
        >
          Delete
        </button>
      )}
      <button
        ...
        data-testid={`click-${command.name}`}
      >
        Click
      </button>
    </div>
...
const renderSlashCommandsSettings = (
  commands: Command[] = mockCommands,
  cwd?: string,
) => {
...
describe("SlashCommandsSettings", () => {
  ...
  it("renders section header with icon and title", () => {
    ...
    expect(
      screen.getByText("settings:sections.slashCommands"),
    ).toBeInTheDocument();
  });
  ...
  it("displays built-in commands in their own section", () => {
    ...
    expect(
      screen.getByText("chat:slashCommands.builtInCommands"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("command-item-built-in-command"),
    ).toBeInTheDocument();
  });
  ...
  it("displays global commands in their own section", () => {
    ...
    expect(
      screen.getByText("chat:slashCommands.globalCommands"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("command-item-global-command"),
    ).toBeInTheDocument();
  });
  ...
  it("displays project commands when in a workspace", () => {
    ...
    expect(
      screen.getByText("chat:slashCommands.workspaceCommands"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("command-item-project-command"),
    ).toBeInTheDocument();
  });
  ...
  it("does not display project commands when not in a workspace", () => {
    ...
    const commandsWithoutProject = mockCommands.filter(
      (cmd) => cmd.source !== "project",
    );
    renderSlashCommandsSettings(commandsWithoutProject, "");

    // Project commands should not be shown
    expect(
      screen.queryByTestId("command-item-project-command"),
  ...
  it("opens delete confirmation dialog when delete button is clicked", () => {
    ...
    expect(
      screen.getByText("chat:slashCommands.deleteDialog.title"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        "chat:slashCommands.deleteDialog.description global-command",
      ),
    ).toBeInTheDocument();
  });
