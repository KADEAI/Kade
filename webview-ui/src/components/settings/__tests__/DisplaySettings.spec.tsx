  ...
  useAppTranslation: () => ({
    t: (key: string) => {
      ...
      const translations: { [key: string]: string } = {
        ...
        "settings:display.taskTimeline.description":
          "Display task messages as a color-coded visual timeline",
        "settings:display.showTimestamps.label": "Show event timestamps",
        "settings:display.showTimestamps.description":
          "Display timestamps for chat messages and events",
      };
...
vi.mock("../../chat/TaskTimeline", () => ({
  TaskTimeline: ({ groupedMessages, isTaskActive }: any) => (
    <div data-testid="task-timeline-preview">
      TaskTimeline Preview ({groupedMessages?.length || 0} messages, active:{" "}
      {isTaskActive ? "yes" : "no"})
    </div>
  ),
}));
...
describe("DisplaySettings", () => {
  ...
  it("renders all display settings components", () => {
    ...
    expect(
      screen.getByText("Collapse edit/write/bash tools by default"),
    ).toBeInTheDocument();
  ...
  it("renders task timeline checkbox with correct initial state", () => {
    ...
    const taskTimelineCheckbox = screen
      .getByText("Show task timeline")
      .closest("label");
    const checkbox = taskTimelineCheckbox?.querySelector(
      'input[type="checkbox"]',
    ) as HTMLInputElement;
  ...
  it("renders showTimestamps checkbox with correct initial state", () => {
    ...
    const showTimestampsCheckbox = screen
      .getByText("Show event timestamps")
      .closest("label");
    const checkbox = showTimestampsCheckbox?.querySelector(
      'input[type="checkbox"]',
    ) as HTMLInputElement;
  ...
  it("calls setCachedStateField when collapse code tools checkbox is toggled", () => {
    render(
      <DisplaySettings
        showTaskTimeline={true}
        showTimestamps={false}
    ...
    const codeToolsCheckbox = screen
      .getByText("Collapse edit/write/bash tools by default")
      .closest("label");
    const checkbox = codeToolsCheckbox?.querySelector(
      'input[type="checkbox"]',
    ) as HTMLInputElement;
  ...
  it("calls setCachedStateField when vibe styling checkbox is toggled", () => {
    render(
      <DisplaySettings
        showTaskTimeline={true}
        showTimestamps={false}
    ...
    const vibeCheckbox = screen
      .getByText("Enable inline vibe styling")
      .closest("label");
    const checkbox = vibeCheckbox?.querySelector(
      'input[type="checkbox"]',
    ) as HTMLInputElement;
  ...
  it("calls setCachedStateField when prompt suggestions checkbox is toggled", () => {
    render(
      <DisplaySettings
        showTaskTimeline={true}
        showTimestamps={false}
    ...
    const promptSuggestionsCheckbox = screen
      .getByText("Show Prompt Suggestions")
      .closest("label");
    const checkbox = promptSuggestionsCheckbox?.querySelector(
      'input[type="checkbox"]',
    ) as HTMLInputElement;
  ...
  it("renders vibe styling checkbox unchecked when disabled", () => {
    render(
      <DisplaySettings
        showTaskTimeline={true}
        showTimestamps={false}
  ...
  it("renders task timeline preview with sample data", () => {
    ...
    expect(preview).toHaveTextContent(
      "TaskTimeline Preview (3 messages, active: no)",
    );
  });

  it("renders the timeline preview section", () => {
    render(
      <DisplaySettings
        showTaskTimeline={true}
        showTimestamps={false}
  ...
  it("handles both checkboxes being checked", () => {
    ...
    const taskTimelineInput = taskTimelineCheckbox?.querySelector(
      'input[type="checkbox"]',
    ) as HTMLInputElement;
    const showTimestampsInput = showTimestampsCheckbox?.querySelector(
      'input[type="checkbox"]',
    ) as HTMLInputElement;
