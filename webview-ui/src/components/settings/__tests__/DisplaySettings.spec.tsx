...
describe("DisplaySettings", () => {
	...
	it("renders all display settings components", () => {
		...
		expect(screen.getByText("Collapse edit/write/bash tools by default")).toBeInTheDocument()
	...
	it("calls setCachedStateField when collapse code tools checkbox is toggled", () => {
		...
		const codeToolsCheckbox = screen.getByText("Collapse edit/write/bash tools by default").closest("label")
		const checkbox = codeToolsCheckbox?.querySelector('input[type="checkbox"]') as HTMLInputElement

		fireEvent.click(checkbox)

		expect(mockSetCachedStateField).toHaveBeenCalledWith("collapseCodeToolsByDefault", true)
	})
