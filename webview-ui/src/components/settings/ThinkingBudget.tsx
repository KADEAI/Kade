...
export const ThinkingBudget = ({
  ...
  const isGemini25Pro =
    selectedModelId && selectedModelId.includes("gemini-2.5-pro");
  ...
  const isReasoningBudgetSupported =
    !!modelInfo && modelInfo.supportsReasoningBudget;
  const isReasoningBudgetRequired =
    !!modelInfo && modelInfo.requiredReasoningBudget;
  const isReasoningEffortSupported =
    !!modelInfo && modelInfo.supportsReasoningEffort;
  ...
  const availableOptions: ReadonlyArray<ReasoningEffortOption> =
    shouldAutoAddDisable
  ...
  const modelDefaultReasoningEffort = modelInfo?.reasoningEffort as
  ...
  const storedReasoningEffort = apiConfiguration.reasoningEffort as
    | ReasoningEffortOption
    | undefined;
  const currentReasoningEffort: ReasoningEffortOption =
    storedReasoningEffort || defaultReasoningEffort;
  ...
  useEffect(() => {
    if (isReasoningEffortSupported && !apiConfiguration.reasoningEffort) {
      ...
      if (
        ...
        setApiConfigurationField(
          "reasoningEffort",
          defaultReasoningEffort as ReasoningEffortWithMinimal,
          false,
        );
      }
    }
  ...
  useEffect(() => {
    if (
      isReasoningBudgetSupported &&
      modelInfo?.maxTokens &&
      customMaxOutputTokens > modelInfo.maxTokens
    ) {
      setApiConfigurationField(
        "modelMaxTokens",
    ...
    <>
      {!isReasoningBudgetRequired &&
        apiConfiguration.apiProvider !==
          "virtual-quota-fallback" /*kade_change*/ && (
          <div className="flex flex-col gap-1">
            <Checkbox
              checked={enableReasoningEffort}
              onChange={(checked: boolean) =>
      ...
      <Select
        ...
        onValueChange={(value: ReasoningEffortOption) => {
          ...
          if (value === "disable") {
            ...
            setApiConfigurationField(
              "reasoningEffort",
              value as ReasoningEffortWithMinimal,
            );
          }
        }}
      >
            ...
            placeholder={
              currentReasoningEffort
                ? currentReasoningEffort === "none" ||
                  ...
                  : t(
                      `settings:providers.reasoningEffort.${currentReasoningEffort}`,
                    )
