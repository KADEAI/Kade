...
const FREE_PROVIDER_GUIDE = [
	{
		...
		details: ["Free trial for new users", "Free trial available for pro account"],
	},
	{
		label: "Baseten",
		provider: "baseten",
	...
	},
	{
		label: "Cerebras",
		provider: "cerebras",
		details: ["1 million tokens per day free tier", "Access to all Cerebras models"],
	},
	{
		label: "CLI OAuth (Google Gemini)",
		provider: undefined,
		details: ["Free trial through OAuth", "Free trial available for pro account"],
	},
	{
	...
	},
	{
		...
		provider: "gemini",
		details: ["Free tier for several models with rate limits", "$300 free credits for new customers"],
	},
	{
	...
	},
	{
		label: "Kilo Gateway",
		provider: "kilocode",
		details: ["Free models with unlimited usage", "GLM 5, Kimi K2.5, MiniMax 2.5, and others"],
	},
	{
		label: "LiteLLM",
		provider: "litellm",
	...
	},
	{
		label: "Qwen Code",
		provider: "qwen-code",
		details: ["2,000 requests per day via OAuth", "60 requests per minute rate limit"],
	},
	{
		label: "SambaNova",
		provider: "sambanova",
...
const ApiOptions = ({
	...
	useRegisterSetting({ settingId: "provider-select", section: "providers", label: t("settings:providers.apiProvider") })
	useRegisterSetting({ settingId: "provider-model", section: "providers", label: t("settings:providers.model") })
	useRegisterSetting({ settingId: "advanced-todo-list", section: "providers", label: t("settings:advanced.todoList.label") })
	useRegisterSetting({ settingId: "advanced-diff", section: "providers", label: t("settings:advanced.diff.label") })
	useRegisterSetting({ settingId: "advanced-temperature", section: "providers", label: t("settings:temperature.useCustom") })
	useRegisterSetting({ settingId: "advanced-ratelimit-after", section: "providers", label: t("settings:providers.rateLimitAfter.label") })
	useRegisterSetting({ settingId: "advanced-ratelimit-seconds", section: "providers", label: t("settings:providers.rateLimitSeconds.label") })
	useRegisterSetting({ settingId: "advanced-mistake-limit", section: "providers", label: t("settings:providers.consecutiveMistakeLimit.label") })
	...
	// 3. XML fallback
	const defaultProtocol = selectedModelInfo?.defaultToolProtocol || TOOL_PROTOCOL.MARKDOWN
	...
	return (
		<div className="flex flex-col gap-6">
			...
			<div className="rounded-[24px] border border-vscode-input-border/30 bg-vscode-editor-background/40 p-5 flex flex-col gap-5 shadow-xl overflow-hidden" data-setting-id="provider-select">
						...
						return (
							provider?.website && (
								<VSCodeLink
									...
									target="_blank"
									title={t("settings:providers.visitWebsite", { name: provider.label })}
				...
				{!hideRecommendation && (
				<div className="rounded-2xl border border-vscode-input-border/30 bg-vscode-editor-background/50 overflow-hidden">
					<button
						type="button"
						onClick={() => setIsFreeGuideOpen((open) => !open)}
						className="w-full flex items-center justify-between gap-3 px-4 py-3 text-left hover:bg-vscode-toolbar-hoverBackground/40 transition-colors"
						data-testid="free-provider-guide-toggle">
						<div className="min-w-0">
							<div className="text-[12px] font-semibold text-vscode-foreground">Free Provider Guide</div>
							<div className="text-[11px] text-vscode-descriptionForeground">
								See which providers offer free tiers, credits, trials, or OAuth access.
							</div>
						</div>
						<div className="flex items-center gap-2 shrink-0">
							<span className="text-[10px] px-2 py-0.5 rounded-full border border-vscode-input-border/40 text-vscode-descriptionForeground">
								{FREE_PROVIDER_GUIDE.length} listed
							</span>
							<ChevronDown
								className={cn("size-4 text-vscode-descriptionForeground transition-transform", isFreeGuideOpen && "rotate-180")}
							/>
						</div>
					</button>
					...
					{selectedFreeProviderGuide && (
						<div className="px-4 pb-3">
							<div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-[11px] text-vscode-foreground">
								<span className="font-semibold">Selected provider has free access options:</span>{" "}
					...
					{isFreeGuideOpen && (
						<div className="px-4 pb-4 flex flex-col gap-2" data-testid="free-provider-guide-content">
							<div className="text-[11px] text-vscode-descriptionForeground">
								Free tiers can change over time and may include rate limits or trial restrictions.
							</div>
										...
										<div
											...
											className={cn(
												...
												isSelected
													...
													: "border-vscode-input-border/30 bg-vscode-editor-background/30",
											)}>
											<div className="flex items-center justify-between gap-2">
												<span className="text-[12px] font-medium text-vscode-foreground">{entry.label}</span>
												{isSelected ? (
													<span className="text-[10px] px-2 py-0.5 rounded-full border border-emerald-500/30 text-emerald-300">
														Selected
													</span>
												) : null}
											</div>
											<ul className="text-[11px] text-vscode-descriptionForeground space-y-1 pl-4 list-disc">
				...
				<div className="flex flex-col gap-4">
					...
					{selectedProviderModels.length > 0 && !["openai-codex", "antigravity", "gemini-cli"].includes(selectedProvider) && (
						<>
							...
							{!selectedModelInfo?.deprecated &&
								!["kilocode", "openrouter", "glama", "unbound", "requesty", "litellm", "deepinfra", "nano-gpt", "ovhcloud", "inception", "gemini", "openai-codex", "antigravity", "gemini-cli"].includes(
									selectedProvider,
								) && (
			...
			{!fromWelcomeView && selectedProvider !== ("virtual-quota-fallback" as any) && !hideProtocolAndAdvanced && (
				<div className="flex flex-col gap-4">
					<div className="rounded-[24px] border border-vscode-input-border/30 bg-vscode-editor-background/40 p-5 flex flex-col gap-5 shadow-sm">
						...
						<ToolProtocolSelector
							value={((apiConfiguration.toolProtocol as any) === "xml" || (apiConfiguration.toolProtocol as any) === "native" ? TOOL_PROTOCOL.MARKDOWN : (apiConfiguration.toolProtocol || defaultProtocol)) as ToolProtocol}
							onChange={(value) => setApiConfigurationField("toolProtocol", value)}
							unifiedFormatVariant={apiConfiguration.unifiedFormatVariant || "structured"}
							onUnifiedFormatVariantChange={(value) => setApiConfigurationField("unifiedFormatVariant", value)}
							disableBatchToolUse={apiConfiguration.disableBatchToolUse}
							onDisableBatchToolUseChange={(value) => setApiConfigurationField("disableBatchToolUse", value)}
							maxToolCalls={apiConfiguration.maxToolCalls}
							onMaxToolCallsChange={(value) => setApiConfigurationField("maxToolCalls", value)}
							minimalSystemPrompt={apiConfiguration.minimalSystemPrompt}
							onMinimalSystemPromptChange={(value) => setApiConfigurationField("minimalSystemPrompt", value)}
						/>
					</div>
							...
							<CollapsibleContent className="px-5 pb-5 flex flex-col gap-6">
								<div className="border-t border-vscode-input-border/50 pt-5 flex flex-col gap-6">
									<TodoListSettingsControl
										...
										onChange={(field: any, value: any) => setApiConfigurationField(field, value)}
									/>
									{/* <DiffSettingsControl
