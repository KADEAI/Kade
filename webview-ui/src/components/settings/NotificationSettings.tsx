...
export const NotificationSettings = ({
	...
	useRegisterSetting({ settingId: "notifications-desktop", section: "notifications", label: t("kilocode:settings.systemNotifications.label") })
	useRegisterSetting({ settingId: "notifications-sound", section: "notifications", label: t("settings:notifications.sound.label") })
	useRegisterSetting({ settingId: "notifications-tts", section: "notifications", label: t("settings:notifications.tts.label") })
			...
			<Section className="flex flex-col gap-6">
				...
				<div className="rounded-[24px] border border-vscode-input-border/30 bg-vscode-editor-background/40 p-5 flex flex-col gap-5 shadow-xl">
					...
					<div className="flex flex-col gap-3">
						<div className="flex items-center justify-between" data-setting-id="notifications-desktop">
							<VSCodeCheckbox
							...
							<StandardTooltip content={t("kilocode:settings.systemNotifications.description")}>
						...
						{systemNotificationsEnabled && (
							<div className="flex items-center justify-between ml-6 bg-vscode-textBlockQuote-background/20 px-3 py-1.5 rounded border border-vscode-textBlockQuote-border/10">
								...
								<VSCodeButton appearance="secondary" onClick={onTestNotificationClick} className="scale-90 origin-right">
				...
				<div className="rounded-[24px] border border-vscode-input-border/30 bg-vscode-editor-background/40 p-5 flex flex-col gap-5 shadow-xl">
					...
					<div className="flex flex-col gap-4">
						...
						<div className="flex flex-col gap-2">
							<div className="flex items-center justify-between" data-setting-id="notifications-sound">
								<VSCodeCheckbox
								...
								<StandardTooltip content={t("settings:notifications.sound.description")}>
									<Info className="size-3.5 text-vscode-descriptionForeground cursor-help" />
								</StandardTooltip>
							</div>
						...
						<div className="flex flex-col gap-2 border-t border-vscode-input-border/10 pt-3">
							<div className="flex items-center justify-between" data-setting-id="notifications-tts">
								<VSCodeCheckbox
									checked={ttsEnabled}
									onChange={(e: any) => setCachedStateField("ttsEnabled", e.target.checked)}
									data-testid="tts-enabled-checkbox">
									<span className="font-medium text-sm">{t("settings:notifications.tts.label")}</span>
								</VSCodeCheckbox>
								<StandardTooltip content={t("settings:notifications.tts.description")}>
									<Info className="size-3.5 text-vscode-descriptionForeground cursor-help" />
								</StandardTooltip>
							</div>
