...
import { Zap, Images, Brain, FileCode2 } from "lucide-react";
...
export const ExperimentalSettings = ({
  ...
  useRegisterSetting({
    settingId: "exp-image-gen",
    section: "experimental",
    label: "Enable AI image generation",
  });
  useRegisterSetting({
    settingId: "exp-multi-file",
    section: "experimental",
    label: "Enable concurrent file edits",
  });
  useRegisterSetting({
    settingId: "exp-stt",
    section: "experimental",
  ...
  useRegisterSetting({
    settingId: "exp-power-steering",
    section: "experimental",
    label: 'Use experimental "power steering" mode',
  });
    ...
    if (key === "MORPH_FAST_APPLY") {
      return (
        <div
          ...
          data-setting-id="exp-fast-apply"
        >
          <ExperimentalFeature
            experimentKey={key}
            enabled={enabled}
                ...
                if (key === "SPEECH_TO_TEXT") {
                  return (
                    <div
                      key={key}
                      className="flex flex-col gap-2"
                      data-setting-id="exp-stt"
                    >
                      <SpeechToTextSettings
                        enabled={enabled}
                        onChange={(checked: boolean) =>
                          setExperimentEnabled(id, checked)
                        }
                        sttProvider={apiConfiguration?.sttProvider}
                        setSttProvider={(
                          provider: "openai" | "gemini" | "local",
                        ) => setApiConfigurationField("sttProvider", provider)}
                        sttModelId={apiConfiguration?.sttModelId}
                        setSttModelId={(modelId: string) =>
                          setApiConfigurationField("sttModelId", modelId)
                        }
                      />
                    </div>
                  );
