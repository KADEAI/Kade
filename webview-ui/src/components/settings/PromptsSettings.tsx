...
import { VSCodeTextArea, VSCodeCheckbox, VSCodeLink } from "@vscode/webview-ui-toolkit/react"
import { MessageSquare, Sparkles, Settings, Info, RotateCcw, Cloud, Send, Braces } from "lucide-react"
...
const PromptsSettings = ({
	...
	useRegisterSetting({ settingId: "prompts-template-select", section: "prompts", label: "Select Prompt Template" })
	useRegisterSetting({ settingId: "prompts-edit-content", section: "prompts", label: t("prompts:supportPrompts.prompt") })
	useRegisterSetting({ settingId: "prompts-api-config", section: "prompts", label: "API Configuration" })
	useRegisterSetting({ settingId: "prompts-include-history", section: "prompts", label: t("prompts:supportPrompts.enhance.includeTaskHistory") })
	useRegisterSetting({ settingId: "prompts-test-enhancement", section: "prompts", label: t("prompts:supportPrompts.enhance.testEnhancement") })
	...
	return (
		<div className="flex flex-col">
			...
			<Section className="flex flex-col gap-6">
				...
				<div className="rounded-[24px] border border-vscode-input-border/30 bg-vscode-editor-background/40 p-5 flex flex-col gap-5 shadow-xl" data-setting-id="prompts-template-select">
					...
					<div className="flex flex-col gap-3">
						<Select
							...
							<SelectTrigger className="w-full text-[13px]" data-testid="support-prompt-select-trigger">
								<SelectValue placeholder={t("settings:common.select")} />
							</SelectTrigger>
							<SelectContent>
								{Object.keys(supportPrompt.default).map((type) => (
									<SelectItem key={type} value={type} data-testid={`${type}-option`} className="text-[13px]">
				...
				{/* Edit Prompt Card */}
				<div className="rounded-[24px] border border-vscode-input-border/30 bg-vscode-editor-background/40 p-5 flex flex-col gap-5 shadow-xl" data-setting-id="prompts-edit-content">
				...
				{(activeSupportOption === "ENHANCE" || activeSupportOption === "CONDENSE") && (
					<div className="rounded-[24px] border border-vscode-input-border/30 bg-vscode-editor-background/40 p-5 flex flex-col gap-5 shadow-xl" data-setting-id="prompts-api-config">
						<div className="flex items-center gap-2 border-b border-vscode-input-border/50 pb-2">
						...
						<div className="flex flex-col gap-5 pl-3.5 border-l-2 border-vscode-focusBorder/50">
							<div className="flex flex-col gap-2">
								<div className="flex items-center justify-between">
									...
									<StandardTooltip content={activeSupportOption === "ENHANCE"
								...
								<Select
									...
									<SelectTrigger data-testid="api-config-select" className="w-full text-[13px]">
										<SelectValue
											placeholder={
												activeSupportOption === "ENHANCE"
							...
							{activeSupportOption === "ENHANCE" && (
								<>
									...
									<div className="flex flex-col gap-1.5" data-setting-id="prompts-include-history">
										<div className="flex items-center justify-between">
											<VSCodeCheckbox
											...
											<StandardTooltip content={t("prompts:supportPrompts.enhance.includeTaskHistoryDescription")}>
												<Info className="size-3.5 text-vscode-descriptionForeground cursor-help" />
											</StandardTooltip>
										</div>
									</div>
				...
				{activeSupportOption === "ENHANCE" && (
					<div className="rounded-[24px] border border-vscode-input-border/30 bg-vscode-editor-background/40 p-5 flex flex-col gap-5 shadow-xl" data-setting-id="prompts-test-enhancement">
						<div className="flex items-center gap-2 border-b border-vscode-input-border/50 pb-2">
