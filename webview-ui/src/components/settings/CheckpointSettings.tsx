...
import {
  ...
  VSCodeTextField,
  VSCodeButton,
} from "@vscode/webview-ui-toolkit/react"; // kade_change
...
export const CheckpointSettings = ({
  ...
  useRegisterSetting({
    settingId: "checkpoints-autopurge",
    section: "checkpoints",
    label: t("settings:autoPurge.enable.label"),
  });
  useRegisterSetting({
    settingId: "checkpoints-retention-default",
    section: "checkpoints",
    label: t("settings:autoPurge.defaultRetention.label"),
  });
  useRegisterSetting({
    settingId: "checkpoints-purge-favorited",
    section: "checkpoints",
    label: t("settings:autoPurge.neverPurgeFavorited.label"),
  });
  useRegisterSetting({
    settingId: "checkpoints-retention-favorited",
    section: "checkpoints",
    label: t("settings:autoPurge.favoritedRetention.label"),
  });
  useRegisterSetting({
    settingId: "checkpoints-retention-completed",
    section: "checkpoints",
    label: t("settings:autoPurge.completedRetention.label"),
  });
  useRegisterSetting({
    settingId: "checkpoints-retention-incomplete",
    section: "checkpoints",
    label: t("settings:autoPurge.incompleteRetention.label"),
  });
  useRegisterSetting({
    settingId: "checkpoints-manual-purge",
    section: "checkpoints",
    label: t("settings:autoPurge.manualPurge.button"),
  });
  return (
      ...
      <Section className="flex flex-col gap-6 pt-0">
        ...
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-[24px] p-5 flex items-start gap-4 shadow-xl">
          ...
          <div className="flex flex-col gap-1.5 pt-0.5">
            <div className="text-[13px] font-bold text-amber-500 uppercase tracking-widest">
              Legacy Option
            </div>
            <div className="text-[12px] leading-relaxed text-vscode-foreground/90 font-medium">
              This feature is now a legacy option from Cline/RooCode. Kade has
              moved to using native VS Code snapshots and an advanced edit
              history service. This transition saves tremendous amounts of
              storage and eliminates the need for having to backup on each turn;
              Backing up of a file is only made when an edit/write has been
              done. To revert, you have three options: simply press the undo
              button in the edit/write tool block, reject the change in the diff
              bar above the chat input box (you can still undo edits in the tool
              blocks even after approving changes), or undo the message send by
              clicking on your previous messages. Please only use this setting
              if specifically required for legacy workflows.
            </div>
          </div>
        </div>
        ...
        <div className="rounded-[24px] border border-vscode-input-border/30 bg-vscode-editor-background/40 p-5 flex flex-col gap-5 shadow-xl">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div
              ...
              data-setting-id="checkpoints-timeout"
            >
        ...
        <div className="rounded-[24px] border border-vscode-input-border/30 bg-vscode-editor-background/40 p-5 flex flex-col gap-5 shadow-xl">
          ...
          <div className="flex flex-col gap-5">
            <div data-setting-id="checkpoints-autopurge">
              ...
              >
                <span className="font-medium text-[13px]">
                  {t("settings:autoPurge.enable.label")}
                </span>
              </VSCodeCheckbox>
              <div className="text-vscode-descriptionForeground text-[11px] mt-1 pl-7 opacity-80 leading-relaxed">
            ...
            {autoPurgeEnabled && (
              <div className="flex flex-col gap-5 pl-4 border-l-2 border-vscode-focusBorder/50">
                ...
                {autoPurgeFavoritedTaskRetentionDays !== null && (
                  <div
                    className="pl-7"
                    data-setting-id="checkpoints-retention-favorited"
                  >
                    <label className="block text-[13px] font-medium mb-2">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-vscode-input-border/10">
                  ...
                  <VSCodeButton
                    ...
                    data-setting-id="checkpoints-manual-purge"
                  >
