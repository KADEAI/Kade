...
export const HuggingFace = ({
  ...
  const onMessage = useCallback((event: MessageEvent) => {
    ...
    switch (message.type) {
      case "huggingFaceModels":
        setModels(
          message.huggingFaceModels?.sort((a, b) => a.id.localeCompare(b.id)) ||
            [],
        );
        setLoading(false);
        break;
  ...
  const currentModel = models.find(
    (m) => m.id === apiConfiguration?.huggingFaceModelId,
  );
  ...
  const currentProvider = useMemo(() => {
    if (!currentModel || !selectedProvider || selectedProvider === "auto")
      return null;
  ...
  const modelCapabilities = useMemo(() => {
    ...
    const maxTokens =
      currentProvider?.context_length ||
      currentModel.providers.find((p) => p.context_length)?.context_length;
    const supportsTools =
      currentProvider?.supports_tools ||
      currentModel.providers.some((p) => p.supports_tools);
  ...
  return (
    <>
        ...
        <label className="block font-medium text-sm">
          ...
          {loading && (
            <span className="text-xs text-gray-400 ml-2">
              {t("settings:providers.huggingFaceLoading")}
            </span>
          )}
          {!loading && (
            <span className="text-xs text-gray-400 ml-2">
      ...
      {currentModel && availableProviders.length > 0 && (
        <div className="flex flex-col gap-2">
          <label className="block font-medium text-sm">
            {t("settings:providers.huggingFaceProvider")}
          </label>
          <SearchableSelect
            ...
            searchPlaceholder={t(
              "settings:providers.huggingFaceSearchProviders",
            )}
