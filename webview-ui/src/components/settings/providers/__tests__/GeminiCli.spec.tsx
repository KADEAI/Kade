...
describe("GeminiCli", () => {
  ...
  it("renders all required elements", () => {
    ...
    expect(
      screen.getByText("settings:providers.geminiCli.oauthPath"),
    ).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText("~/.gemini/oauth_creds.json"),
    ).toBeInTheDocument();

    // Check for description text
    expect(
      screen.getByText("settings:providers.geminiCli.description"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("settings:providers.geminiCli.setupLink"),
    ).toBeInTheDocument();

    // Check for requirements
    expect(
      screen.getByText("settings:providers.geminiCli.requirementsTitle"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("settings:providers.geminiCli.requirement1"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("settings:providers.geminiCli.requirement2"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("settings:providers.geminiCli.requirement3"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("settings:providers.geminiCli.requirement4"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("settings:providers.geminiCli.requirement5"),
    ).toBeInTheDocument();

    // Check for free access note
    expect(
      screen.getByText("settings:providers.geminiCli.freeAccess"),
    ).toBeInTheDocument();
  });
  ...
  it("calls setApiConfigurationField when OAuth path is changed", () => {
    ...
    const oauthInput = screen.getByPlaceholderText(
  ...
  it("renders setup link with correct href", () => {
    ...
    const setupLink = screen.getByText(
      "settings:providers.geminiCli.setupLink",
    );
  ...
  it("shows OAuth path description", () => {
    ...
    expect(
      screen.getByText("settings:providers.geminiCli.oauthPathDescription"),
    ).toBeInTheDocument();
  });
  ...
  it("renders all requirements in a list", () => {
    ...
    expect(listItems[0]).toHaveTextContent(
      "settings:providers.geminiCli.requirement1",
    );
    expect(listItems[1]).toHaveTextContent(
      "settings:providers.geminiCli.requirement2",
    );
    expect(listItems[2]).toHaveTextContent(
      "settings:providers.geminiCli.requirement3",
    );
    expect(listItems[3]).toHaveTextContent(
      "settings:providers.geminiCli.requirement4",
    );
    expect(listItems[4]).toHaveTextContent(
      "settings:providers.geminiCli.requirement5",
    );
  });
