...
vi.mock("@vscode/webview-ui-toolkit/react", () => ({
  VSCodeTextField: ({
    ...
    return (
      <div
        data-testid={
          dataTestId ? `${dataTestId}-text-field` : "vscode-text-field"
        }
        className={className}
        style={style}
      >
  ...
  VSCodeButton: ({ children, onClick, appearance, title }: any) => (
    <button
      onClick={onClick}
      title={title}
      data-testid={`vscode-button-${appearance}`}
    >
      {children}
    </button>
  ),
...
vi.mock("../../R1FormatSetting", () => ({
  R1FormatSetting: () => (
    <div data-testid="r1-format-setting">R1 Format Setting</div>
  ),
}));

vi.mock("../../ThinkingBudget", () => ({
  ThinkingBudget: () => (
    <div data-testid="thinking-budget">Thinking Budget</div>
  ),
}));
...
describe("OpenAICompatible Component - includeMaxTokens checkbox", () => {
  ...
  describe("Checkbox Rendering", () => {
    it("should render the includeMaxTokens checkbox", () => {
      ...
      const checkbox = screen.getByTestId(
        "checkbox-settings:includemaxoutputtokens",
      );
      expect(checkbox).toBeInTheDocument();

      // Check that the description text is rendered
      expect(
        screen.getByText("settings:includeMaxOutputTokensDescription"),
      ).toBeInTheDocument();
    });
    ...
    it("should render the checkbox with correct translation keys", () => {
      ...
      expect(
        screen.getByText("settings:includeMaxOutputTokens"),
      ).toBeInTheDocument();
  ...
  describe("Initial State", () => {
    it("should show checkbox as checked when includeMaxTokens is true", () => {
      ...
      const checkboxInput = screen.getByTestId(
        "checkbox-input-settings:includemaxoutputtokens",
      );
  ...
  describe("Component Updates", () => {
    it("should update checkbox state when apiConfiguration changes", () => {
      ...
      let checkboxInput = screen.getByTestId(
        "checkbox-input-settings:includemaxoutputtokens",
      );
      expect(checkboxInput).toBeChecked();
      ...
      checkboxInput = screen.getByTestId(
        "checkbox-input-settings:includemaxoutputtokens",
      );
      expect(checkboxInput).not.toBeChecked();
    });
  ...
  describe("UI Structure", () => {
    it("should render the checkbox with description in correct structure", () => {
      ...
      const description = screen.getByText(
        "settings:includeMaxOutputTokensDescription",
      );
