 ...
 */
const UsageProgressBar: React.FC<{ utilization: number; label: string }> = ({
  utilization,
  label,
}) => {
...
export const ClaudeCodeRateLimitDashboard: React.FC<
  ClaudeCodeRateLimitDashboardProps
> = ({ isAuthenticated }) => {
  const { t } = useAppTranslation();
  const [rateLimits, setRateLimits] = useState<ClaudeCodeRateLimitInfo | null>(
    null,
  );
    ...
    return (
      <div className="bg-vscode-editor-background border border-vscode-panel-border rounded-md p-3">
        <div className="text-sm text-vscode-descriptionForeground">
          {t("settings:providers.claudeCode.rateLimits.loading")}
        </div>
      </div>
    );
  }
      ...
      <div className="bg-vscode-editor-background border border-vscode-panel-border rounded-md p-3">
        <div className="flex items-center justify-between">
          <div className="text-sm text-vscode-errorForeground">
            {t("settings:providers.claudeCode.rateLimits.error")}
          </div>
          <button
          ...
          >
            {t("settings:providers.claudeCode.rateLimits.retry")}
          </button>
        </div>
      </div>
    );
    ...
    <div className="bg-vscode-editor-background border border-vscode-panel-border rounded-md p-3">
      <div className="mb-3">
        <div className="text-sm font-medium text-vscode-foreground">
          {t("settings:providers.claudeCode.rateLimits.title")}
        </div>
      </div>
      ...
      <div className="space-y-3">
          ...
          <div className="flex items-center justify-between text-xs">
            <span className="text-vscode-foreground">
              {rateLimits.representativeClaim ||
                t("settings:providers.claudeCode.rateLimits.fiveHour")}
            </span>
            <span className="text-vscode-descriptionForeground">
              {formatUtilization(rateLimits.fiveHour.utilization)}{" "}
              {t("settings:providers.claudeCode.rateLimits.used")} •{" "}
              {t("settings:providers.claudeCode.rateLimits.resetsIn")}{" "}
              {formatResetTime(rateLimits.fiveHour.resetTime)}
            </span>
          </div>
          <UsageProgressBar
            utilization={rateLimits.fiveHour.utilization}
            label=""
          />
        </div>
        ...
        {rateLimits.weeklyUnified &&
          rateLimits.weeklyUnified.utilization > 0 && (
            <div className="flex flex-col gap-1">
              <div className="flex items-center justify-between text-xs">
                <span className="text-vscode-foreground">
                  {t("settings:providers.claudeCode.rateLimits.weekly")}
                </span>
                <span className="text-vscode-descriptionForeground">
                  {formatUtilization(rateLimits.weeklyUnified.utilization)}{" "}
                  {t("settings:providers.claudeCode.rateLimits.used")} •{" "}
                  {t("settings:providers.claudeCode.rateLimits.resetsIn")}{" "}
              ...
              <UsageProgressBar
                utilization={rateLimits.weeklyUnified.utilization}
                label=""
              />
            </div>
          )}
