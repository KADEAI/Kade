...
export const VirtualQuotaFallbackProvider = ({
  ...
  const [usageData, setUsageData] = useState<
    Record<string, UsageResultByDuration>
  >({});

  // Get current profile ID to exclude from available profiles
  const currentProfile = listApiConfigMeta?.find(
  ...
  const availableProfiles = useMemo(() => {
    return (
      listApiConfigMeta?.filter((profile: ProviderSettingsEntry) => {
        return (
          profile.apiProvider !== "virtual-quota-fallback" &&
  ...
  const profiles = useMemo(() => {
    return apiConfiguration.profiles && apiConfiguration.profiles.length > 0
