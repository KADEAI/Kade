  ...
  return (
    <>
      <h3 className="text-lg font-medium mb-0">
        <Trans i18nKey="kilocode:virtualProvider.title">
      ...
      <div className="text-sm text-vscode-descriptionForeground mb-4">
        <Trans i18nKey="kilocode:virtualProvider.description">
          Configure a list of profiles each with their own limits. When one
          profiles limits are reached, the next profile in the list will be used
          until none remain.
        </Trans>
      </div>
      ...
      <div className="space-y-1">
        {profiles.map((profile, index) => {
          ...
          return (
            <div
              key={index}
              className="border border-vscode-settings-sashBorder rounded-md p-2"
            >
              <div className="flex items-center justify-between mb-3">
                <label className="block font-medium">
                  {index === 0
                    ? t("kilocode:virtualProvider.primaryProfileLabel", {
                        number: index + 1,
                      })
                    : t("kilocode:virtualProvider.profileLabel", {
                        number: index + 1,
                      })}
                </label>
              ...
              <SearchableSelect
                ...
                placeholder={t(
                  "kilocode:virtualProvider.selectProfilePlaceholder",
                )}
        ...
        <div className="flex justify-center p-4">
          ...
          >
            ...
            <Trans i18nKey="kilocode:virtualProvider.addProfile">
              Add Profile
            </Trans>
          </VSCodeButton>
        </div>
        ...
        {availableProfiles.length === 0 ? (
          <div className="text-sm text-vscode-descriptionForeground text-center p-4 border border-vscode-settings-sashBorder rounded-md">
            <Trans i18nKey="kilocode:virtualProvider.noProfilesAvailable">
              No profile profiles available. Please configure at least one
              non-virtual profile profile first.
            </Trans>
          </div>
        ) : null}
      </div>
      ...
      <div className="p-4 border border-vscode-editorWarning-foreground rounded-md">
        <div className="text-md font-semibold text-vscode-editorWarning-foreground">
          <Trans i18nKey="kilocode:virtualProvider.dangerZoneTitle">
            Danger Zone
          </Trans>
        </div>
        ...
        >
          <Trans i18nKey="kilocode:virtualProvider.clearUsageData">
            Clear Usage Data
          </Trans>
        </VSCodeButton>
      </div>
      ...
      <AlertDialog open={isAlertOpen} onOpenChange={onSetIsAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              <Trans i18nKey="kilocode:virtualProvider.confirmClearTitle">
                Are you sure?
              </Trans>
            </AlertDialogTitle>
            <AlertDialogDescription>
              <Trans i18nKey="kilocode:virtualProvider.confirmClearDescription">
                This will permanently delete all stored usage data for virtual
...
export const VirtualLimitInputsPresentation = ({
  profile,
  index,
  onProfileChange,
  usage,
  ...
  const handleLimitChange = useCallback(
    (
      limitKey: keyof NonNullable<
        VirtualQuotaFallbackProviderData["profileLimits"]
      >,
    ) =>
      (event: unknown) => {
  ...
  return (
    <div className="space-y-4 p-2 rounded-md mt-2">
        ...
        <div className="grid grid-cols-3 gap-x-4">
          <div>
            <label className="block text-xs text-vscode-descriptionForeground mb-1">
              <Trans i18nKey="kilocode:virtualProvider.perMinute">
                Per Minute
              </Trans>
            </label>
            <VSCodeTextField
      ...
      <div>
        <label className="block text-sm font-medium mb-2">
          <Trans i18nKey="kilocode:virtualProvider.requestsLabel">
            Requests
          </Trans>
        </label>
        <div className="grid grid-cols-3 gap-x-4">
