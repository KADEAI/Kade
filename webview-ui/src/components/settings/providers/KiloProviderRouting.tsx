...
type ProviderPreference =
  | {
      type: typeof DEFAULT_VALUE_KEY | "price" | "throughput" | "latency";
    }
  | {
...
const getProviderPreference = (
  ...
  apiConfiguration.openRouterSpecificProvider !==
    OPENROUTER_DEFAULT_PROVIDER_NAME
    ? {
        type: SPECIFIC_PROVIDER_KEY,
        provider: apiConfiguration.openRouterSpecificProvider,
      }
...
export const KiloProviderRoutingManagedByOrganization = (props: {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="text-sm text-vscode-descriptionForeground">
        ...
        >
          {t(
            "kilocode:settings.provider.providerRouting.managedByOrganization",
          )}
        </VSCodeButtonLink>
      </div>
    </div>
...
export const KiloProviderRouting = ({
  ...
  const providers = Object.values(
    useModelProviders(kilocodeDefaultModel, apiConfiguration).data ?? {},
  );
  ...
  const onValueChange = (value: string) => {
    ...
    setApiConfigurationField(
      ...
      preference?.type === SPECIFIC_PROVIDER_KEY
        ? preference.provider
        : OPENROUTER_DEFAULT_PROVIDER_NAME,
    );
  };
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <Select
        value={JSON.stringify(getProviderPreference(apiConfiguration))}
        onValueChange={onValueChange}
      >
        <SelectTrigger
          className={cn(
            "w-full",
            specificProviderIsInvalid && "border-destructive border-3",
          )}
        >
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
          ...
          <ProviderSelectItem
            value={{ type: openRouterProviderSortSchema.Values.price }}
          >
            {t("kilocode:settings.provider.providerRouting.sorting.price")}
          </ProviderSelectItem>
          <ProviderSelectItem
            value={{ type: openRouterProviderSortSchema.Values.throughput }}
          >
            {t("kilocode:settings.provider.providerRouting.sorting.throughput")}
          </ProviderSelectItem>
          <ProviderSelectItem
            value={{ type: openRouterProviderSortSchema.Values.latency }}
          >
          ...
          {specificProviderIsInvalid && (
            <ProviderSelectItem
              value={{ type: SPECIFIC_PROVIDER_KEY, provider: specficProvider }}
            >
              {specficProvider}
            </ProviderSelectItem>
          )}
          {providers
            .filter((p) => !p.label.includes("claude-3-5"))
            .map((provider) => (
              <ProviderSelectItem
      ...
      <Select
        ...
        onValueChange={(value) => {
          ...
          setApiConfigurationField(
            "openRouterZdr",
            value === ZERO_DATA_RETENTION_KEY || undefined,
          );
        }}
      >
        ...
        <SelectContent>
          <SelectItem value={DEFAULT_VALUE_KEY}>
            {apiConfiguration.apiProvider === "kilocode"
              ...
              : t(
                  "kilocode:settings.provider.providerRouting.dataCollection.default",
                )}
          </SelectItem>
          <SelectItem
            value={openRouterProviderDataCollectionSchema.Values.allow}
          >
            {t(
              "kilocode:settings.provider.providerRouting.dataCollection.allow",
            )}
          </SelectItem>
          <SelectItem
            value={openRouterProviderDataCollectionSchema.Values.deny}
          >
            {t(
              "kilocode:settings.provider.providerRouting.dataCollection.deny",
            )}
          </SelectItem>
