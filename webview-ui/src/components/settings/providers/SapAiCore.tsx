...
const SapAiCore = ({
  ...
  const sapAiCoreConfiguration = useMemo(
    () => ({
      ...
      sapAiCoreUseOrchestration:
        apiConfiguration.sapAiCoreUseOrchestration || false,
    }),
    [apiConfiguration],
  );
  ...
  const clearModelSelection = useCallback(() => {
    ...
    fieldsToReset.forEach((field) =>
      setApiConfigurationField(field as keyof ProviderSettings, undefined),
    );
  }, [setApiConfigurationField]);
  ...
  const getAvailableDeployments = useCallback(
    (modelId?: string) => {
      return Object.values(deployments).filter(
        (deployment) => deployment.model === modelId,
      );
    },
    [deployments],
  );
      ...
      .map(
        (modelId): SearchableSelectOption => ({
          ...
          disabled:
            ...
            !hasAvailableDeployments(modelId),
        }),
      )
      .sort((a, b) => {
        if (
  ...
  const deploymentOptions = useMemo(() => {
    ...
    return getAvailableDeployments(apiConfiguration.sapAiCoreModelId)
      .sort(
        (a, b) =>
          Number(b.targetStatus === "RUNNING") -
          Number(a.targetStatus === "RUNNING"),
      )
      .map(
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-col gap-2">
        <label
          htmlFor="sap-ai-core-service-key"
      ...
      <div className="flex flex-col gap-2">
        <label
          htmlFor="sap-ai-core-resource-group"
          className="block font-medium mb-1"
        >
      ...
      <div className="flex items-center space-x-2">
        ...
        />
        <label htmlFor="sap-ai-core-orchestration">
          {t("settings:providers.sapAiCore.orchestrationMode")}
        </label>
      </div>
        ...
        apiConfiguration.sapAiCoreModelId && (
          <div className="flex flex-col gap-2">
            ...
            <SearchableSelect
              ...
              searchPlaceholder={t(
                "settings:providers.sapAiCore.searchDeployments",
              )}
      ...
      <div className="mt-2">
        <VSCodeLink
          href="https://help.sap.com/docs/sap-ai-core"
          target="_blank"
        >
