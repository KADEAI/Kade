...
function formatDurationSeconds(totalSeconds: number, t: Translate): string {
  ...
  if (days > 0) {
    return t("settings:providers.openAiCodexRateLimits.duration.daysHours", {
      days,
      hours,
    });
  }
  if (hours > 0) {
    return t("settings:providers.openAiCodexRateLimits.duration.hoursMinutes", {
      hours,
      minutes,
    });
  }
  return t("settings:providers.openAiCodexRateLimits.duration.minutes", {
    minutes,
  });
}
...
function formatResetTimeMs(resetMs: number | undefined, t: Translate): string {
  if (!resetMs)
    return t("settings:providers.openAiCodexRateLimits.time.notAvailable");
  const diffMs = resetMs - Date.now();
  if (diffMs <= 0)
    return t("settings:providers.openAiCodexRateLimits.time.now");
...
function formatWindowLabel(
  ...
  if (windowMinutes === 60)
    return t("settings:providers.openAiCodexRateLimits.window.oneHour");
  if (windowMinutes === 24 * 60)
    return t("settings:providers.openAiCodexRateLimits.window.daily");
  if (windowMinutes === 7 * 24 * 60)
    return t("settings:providers.openAiCodexRateLimits.window.weekly");
  if (windowMinutes === 5 * 60)
    return t("settings:providers.openAiCodexRateLimits.window.fiveHour");
  if (windowMinutes % (24 * 60) === 0) {
    return t("settings:providers.openAiCodexRateLimits.window.days", {
      days: windowMinutes / (24 * 60),
    });
  }
  if (windowMinutes % 60 === 0) {
    return t("settings:providers.openAiCodexRateLimits.window.hours", {
      hours: windowMinutes / 60,
    });
  }
  return t("settings:providers.openAiCodexRateLimits.window.minutes", {
...
function formatPlanLabel(planType: string | undefined, t: Translate): string {
  if (!planType)
    return t("settings:providers.openAiCodexRateLimits.plan.default");
  return t("settings:providers.openAiCodexRateLimits.plan.withType", {
    planType,
  });
}
...
export const OpenAICodexRateLimitDashboard: React.FC<
  OpenAICodexRateLimitDashboardProps
> = ({ isAuthenticated }) => {
  const { t } = useAppTranslation();
  const [rateLimits, setRateLimits] = useState<OpenAiCodexRateLimitInfo | null>(
    null,
  );
  ...
  if (error) {
    return (
      <div className="bg-vscode-editor-background border border-vscode-panel-border rounded-md p-3">
        ...
        <div className="mt-2 text-xs text-vscode-descriptionForeground break-words">
          {error}
        </div>
      </div>
    );
  ...
  const primaryWindowLabel = primary
    ? formatWindowLabel(primary.windowMinutes, t)
    : undefined;
  const primaryTimeRemaining = primary?.resetsAt
    ? formatTimeRemainingMs(primary.resetsAt - Date.now(), t)
    : "";
  const primaryUsed = primary ? Math.round(primary.usedPercent) : undefined;

  const secondaryWindowLabel = secondary
    ? formatWindowLabel(secondary.windowMinutes, t)
    : undefined;
  const secondaryTimeRemaining = secondary?.resetsAt
    ? formatTimeRemainingMs(secondary.resetsAt - Date.now(), t)
    : "";
  const secondaryUsed = secondary
    ? Math.round(secondary.usedPercent)
    : undefined;

  const getUsageStatusLabel = (
    ...
    const usedLabel =
      used !== undefined
        ? t("settings:providers.openAiCodexRateLimits.usedPercent", {
            percent: used,
          })
        : "";
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between text-xs">
              <span className="text-vscode-foreground">
                {primaryWindowLabel ??
                  t("settings:providers.openAiCodexRateLimits.window.usage")}
              </span>
