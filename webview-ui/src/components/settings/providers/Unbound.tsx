...
export const Unbound = ({
  ...
  const requestModels = useCallback(async () => {
    ...
    const updatedModels = queryClient.getQueryData<{ unbound: RouterModels }>([
      "routerModels",
    ])?.unbound;
    if (updatedModels && Object.keys(updatedModels).length > 0) {
      ...
      const modelExists =
        currentModelId &&
        Object.prototype.hasOwnProperty.call(updatedModels, currentModelId);
      ...
      if (!currentModelId || !modelExists) {
        ...
        setApiConfigurationField(
          ...
          false,
        ); // false = automatic model selection
      }
    }
  ...
  const handleRefresh = useCallback(async () => {
    ...
    if (requestModelsResult) {
      ...
      invalidKeyTimerRef.current = setTimeout(
