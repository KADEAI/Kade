...
export const InfinitySettings = ({
  ...
  const [localInterval, setLocalInterval] = useState(
    infinityIntervalMinutes.toString(),
  );
  const [localScheduleHour, setLocalScheduleHour] = useState(
    infinityScheduleHour.toString().padStart(2, "0"),
  );
  const [localScheduleMinute, setLocalScheduleMinute] = useState(
    infinityScheduleMinute.toString().padStart(2, "0"),
  );
  ...
  const handleScheduleHourChange = (value: string) => {
    ...
    const parsed = parseInt(value, 10);
    const nextHour = Number.isNaN(parsed)
      ? 9
  ...
  const handleScheduleMinuteChange = (value: string) => {
    ...
    const parsed = parseInt(value, 10);
    const nextMinute = Number.isNaN(parsed)
      ? 0
      : Math.min(59, Math.max(0, parsed));
  ...
  const handleSavePrompt = () => {
    ...
    setCachedStateField("infinitySavedPrompts", [
      ...infinitySavedPrompts,
      nextPrompt,
    ]);
  ...
  const handleDeleteSavedPrompt = (savedPromptId: string) => {
    const nextSavedPrompts = infinitySavedPrompts.filter(
      (prompt) => prompt.id !== savedPromptId,
    );
  ...
  const activePromptText =
    infinitySavedPrompts.find((prompt) => prompt.id === activeInfinityPromptId)
      ?.prompt ?? localPrompt;
  ...
  const nextRunLabel =
    infinityIsRunning && infinityNextRunAt
        ...
        <div className="flex flex-col gap-5">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-3">
                <div className="text-sm font-medium text-white/90">
                  Recurring workspace automation
                </div>
              ...
              <p className="mt-1 text-sm leading-7 text-vscode-descriptionForeground">
                Set up recurring prompts that automatically execute at specified
                intervals. Perfect for monitoring, periodic checks, or
                continuous analysis.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 rounded-2xl border border-white/[0.05] bg-white/[0.02] px-4 py-3">
            <VSCodeCheckbox
              ...
              onChange={(e: any) =>
                setCachedStateField("infinityEnabled", e.target.checked)
              }
            />
          ...
          <div
            className={`flex flex-col gap-5 ${!infinityEnabled ? "pointer-events-none opacity-55" : ""}`}
          >
            <div className="rounded-[22px] border border-white/[0.05] bg-[#252525] px-5 py-5 shadow-[0_8px_24px_rgba(0,0,0,0.18)]">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-semibold tracking-[0.01em] text-white/95">
                    Prompt Library
                  </div>
                </div>
                ...
                <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
                  <div className="flex flex-col gap-2">
                    ...
                    <VSCodeTextField
                      ...
                      onInput={(e: any) =>
                        setNewSavedPromptLabel(e.target.value)
                      }
                      className="w-full"
                    />
                  </div>
                ...
                {infinitySavedPrompts.length > 0 ? (
                  <div className="mt-1 flex flex-col gap-2.5">
                    {infinitySavedPrompts.map((savedPrompt) => {
                      const isActive =
                        savedPrompt.id === activeInfinityPromptId;
                        ...
                        >
                          <div className="flex items-start gap-3">
                            <button
                              type="button"
                              onClick={() =>
                                handleSelectSavedPrompt(savedPrompt)
                              }
                  ...
                  <div className="mt-1 rounded-2xl border border-dashed border-white/[0.08] bg-black/10 px-4 py-4 text-sm leading-6 text-vscode-descriptionForeground">
                    Save prompts here to build a reusable Infinity library.
                    Check one saved prompt to make it the active automation.
                  </div>
                )}
              </div>
            </div>

            <div className="rounded-[22px] border border-white/[0.05] bg-[#252525] px-5 py-5 shadow-[0_8px_24px_rgba(0,0,0,0.16)]">
              <div className="flex items-start gap-3">
                <div className="mt-0.5 rounded-xl border border-white/[0.05] bg-white/[0.03] p-2.5">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-1 text-xs leading-6 text-vscode-descriptionForeground">
                    Pick a cadence for the active Infinity prompt. Daily and
                    hourly modes stay aligned to specific clock times.
                  </div>
                </div>
              </div>
              ...
              <div className="mt-5 flex flex-col gap-4">
                <div className="flex flex-col gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setCachedStateField("infinityScheduleType", value)
                    }
                  >
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-3">
                      <Input
                        ...
                        placeholder="00"
                        onChange={(e) =>
                          handleScheduleMinuteChange(e.target.value)
                        }
                        className="h-10 w-24"
                      />
                      <span className="text-sm text-vscode-descriptionForeground">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        placeholder="09"
                        onChange={(e) =>
                          handleScheduleHourChange(e.target.value)
                        }
                        className="h-10 w-24"
                      />
                      <span className="text-sm text-vscode-descriptionForeground">
            ...
            <div className="rounded-[22px] border border-white/[0.05] bg-[#252525] px-5 py-5 shadow-[0_8px_24px_rgba(0,0,0,0.14)]">
              <div className="flex items-start gap-3">
                <div className="mt-0.5 rounded-xl border border-white/[0.05] bg-white/[0.03] p-2.5">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-1 text-xs leading-6 text-vscode-descriptionForeground">
                    Keep an eye on the current cadence and the next queued run
              ...
              <div className="mt-5 grid gap-3 md:grid-cols-[auto_1fr] md:items-center">
                ...
                <div className="rounded-2xl border border-white/[0.06] bg-black/15 px-4 py-3">
                  ...
                  <div className="mt-1 text-xs text-vscode-descriptionForeground">
                    {nextRunLabel
                      ? `Next queued run: ${nextRunLabel}`
                      : "Start Infinity to schedule the next run."}
                  </div>
                </div>
              </div>
            </div>
