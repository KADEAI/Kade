...
export function SettingsSearch({
  ...
  const moveHighlight = useCallback(
    (direction: 1 | -1) => {
      ...
      const currentIndex = highlightedResultId
        ? flatIds.indexOf(highlightedResultId)
        : -1;
      const nextIndex =
        (currentIndex + direction + flatIds.length) % flatIds.length;
    ...
    (event: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (event.key === "Enter" && highlightedResultId) {
        ...
        const selected = results.find(
          (r) => r.settingId === highlightedResultId,
        );
        if (selected) {
  ...
  useEffect(() => {
    ...
    setHighlightedResultId((current) =>
      current && results.some((r) => r.settingId === current)
        ? current
        : results[0]?.settingId,
    );
  }, [isOpen, results]);
  ...
  useEffect(() => {
    ...
    const element = document.getElementById(
      `settings-search-result-${highlightedResultId}`,
    );
