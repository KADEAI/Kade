...
export const DisplaySettings = ({
	...
	useRegisterSetting({ settingId: "display-collapse-thinking", section: "display", label: t("settings:ui.collapseThinking.label") })
	useRegisterSetting({ settingId: "display-send-on-enter", section: "display", label: t("settings:display.sendMessageOnEnter.label") })
	useRegisterSetting({ settingId: "display-show-timestamps", section: "display", label: t("settings:display.showTimestamps.label") })
	useRegisterSetting({ settingId: "display-collapse-code-tools", section: "display", label: "Collapse edit/write/bash tools by default" })
		useRegisterSetting({ settingId: "display-history-mode", section: "display", label: "History View Mode" })
	useRegisterSetting({ settingId: "display-task-timeline", section: "display", label: t("settings:display.taskTimeline.label") })
	useRegisterSetting({ settingId: "display-cost-threshold", section: "display", label: t("settings:display.costThreshold.label") })
	useRegisterSetting({ settingId: "display-tool-theme", section: "display", label: "Tool Theming" })
	useRegisterSetting({ settingId: "display-sub-agent-banner", section: "display", label: "Show Welcome Banner" })
			...
			<Section className="flex flex-col gap-6">
				...
				<div className="rounded-[24px] border border-vscode-input-border/30 bg-vscode-editor-background/40 p-5 flex flex-col gap-5 shadow-xl">
					...
					<div className="flex flex-col gap-3">
						<div className="flex items-center justify-between" data-setting-id="display-collapse-thinking">
							<VSCodeCheckbox
								checked={reasoningBlockCollapsed}
								onChange={(e: any) => handleReasoningBlockCollapsedChange(e.target.checked)}
								data-testid="collapse-thinking-checkbox">
								<span className="font-medium text-sm">{t("settings:ui.collapseThinking.label")}</span>
							</VSCodeCheckbox>
							<StandardTooltip content={t("settings:ui.collapseThinking.description")}>
								<Info className="size-3.5 text-vscode-descriptionForeground cursor-help" />
							</StandardTooltip>
						</div>

						<div className="flex items-center justify-between" data-setting-id="display-send-on-enter">
							<VSCodeCheckbox
								checked={sendMessageOnEnter}
								onChange={(e: any) => {
									setCachedStateField("sendMessageOnEnter", e.target?.checked || false)
								}}>
								<span className="font-medium text-sm">{t("settings:display.sendMessageOnEnter.label")}</span>
							</VSCodeCheckbox>
							<StandardTooltip content={t("settings:display.sendMessageOnEnter.description")}>
								<Info className="size-3.5 text-vscode-descriptionForeground cursor-help" />
							</StandardTooltip>
						</div>

						<div className="flex items-center justify-between" data-setting-id="display-show-timestamps">
							<VSCodeCheckbox
								checked={showTimestamps}
								onChange={(e: any) => {
									setCachedStateField("showTimestamps", e.target.checked)
								}}>
								<span className="font-medium text-sm">{t("settings:display.showTimestamps.label")}</span>
							</VSCodeCheckbox>
							<StandardTooltip content={t("settings:display.showTimestamps.description")}>
								<Info className="size-3.5 text-vscode-descriptionForeground cursor-help" />
							</StandardTooltip>
						</div>

						<div className="flex items-center justify-between" data-setting-id="display-collapse-code-tools">
							<VSCodeCheckbox
								checked={collapseCodeToolsByDefault}
								onChange={(e: any) => {
									setCachedStateField("collapseCodeToolsByDefault", e.target.checked)
								}}>
								<span className="font-medium text-sm">Collapse edit/write/bash tools by default</span>
							</VSCodeCheckbox>
						...
						<div className="flex items-center justify-between" data-setting-id="display-sub-agent-banner">
							<VSCodeCheckbox
				...
				<div className="rounded-[24px] border border-vscode-input-border/30 bg-vscode-editor-background/40 p-5 flex flex-col gap-5 shadow-xl">
					...
					<div className="flex flex-col gap-4">
						<div className="flex flex-col gap-2" data-setting-id="display-history-mode">
							...
							<Select
								...
								onValueChange={(value) => setHistoryViewType?.(value as HistoryViewType)}>
								<SelectTrigger className="w-full">
									<SelectValue placeholder="Select history view mode" />
								</SelectTrigger>
								<SelectContent>
									<SelectItem value="dropdown">Dropdown Overlay</SelectItem>
									<SelectItem value="dropdown-top">Dropdown Overlay (Top View)</SelectItem>
						...
						<div className="flex flex-col gap-3 border-t border-vscode-input-border/10 pt-3">
							<div className="flex items-center justify-between" data-setting-id="display-task-timeline">
								<VSCodeCheckbox
									checked={showTaskTimeline}
									onChange={(e: any) => {
										setCachedStateField("showTaskTimeline", e.target?.checked || false)
									}}>
									<span className="font-medium text-sm">{t("settings:display.taskTimeline.label")}</span>
								</VSCodeCheckbox>
								<StandardTooltip content={t("settings:display.taskTimeline.description")}>
									<Info className="size-3.5 text-vscode-descriptionForeground cursor-help" />
								</StandardTooltip>
							</div>
				...
				<div className="rounded-[24px] border border-vscode-input-border/30 bg-vscode-editor-background/40 p-5 flex flex-col gap-5 shadow-xl">
					...
					<div className="flex flex-col gap-3" data-setting-id="display-cost-threshold">
						<div className="flex items-center justify-between">
							<div className="text-sm font-medium">{t("settings:display.costThreshold.label")}</div>
							<StandardTooltip content={t("settings:display.costThreshold.description")}>
								<Info className="size-3.5 text-vscode-descriptionForeground cursor-help" />
							</StandardTooltip>
						</div>
				...
				<div className="rounded-[24px] border border-vscode-input-border/30 bg-vscode-editor-background/40 p-5 flex flex-col gap-5 shadow-xl">
					...
					<div className="flex flex-col gap-4" data-setting-id="display-tool-theme">
