...
/**
 * Hook to register a setting in the search index.
 */
export function useRegisterSetting(
  setting: Omit<SearchableSettingData, "sectionLabel">,
) {
  const context = useSearchIndexContext();
  const registerSetting = context?.registerSetting;
  ...
  useMemo(() => {
    if (registerSetting) {
      registerSetting({ settingId, section, label });
    }
  }, [registerSetting, settingId, section, label]);
}
 ...
 */
export function useSearchIndexRegistry(
  getSectionLabel: (section: SectionName) => string,
) {
  const settingsRef = useRef<
    Map<string, Omit<SearchableSettingData, "sectionLabel">>
  >(new Map());
