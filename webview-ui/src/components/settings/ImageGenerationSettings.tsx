...
export const ImageGenerationSettings = ({
  ...
  useRegisterSetting({
    settingId: "image-gen-provider",
    section: "experimental",
    label: t("settings:experimental.IMAGE_GENERATION.providerLabel"),
  });
  useRegisterSetting({
    settingId: "image-gen-key",
    section: "experimental",
    label: t("settings:experimental.IMAGE_GENERATION.kiloCodeApiKeyLabel"),
  });
  useRegisterSetting({
    settingId: "image-gen-model",
    section: "experimental",
    label: t("settings:experimental.IMAGE_GENERATION.modelSelectionLabel"),
  });
  ...
  const currentModel = useMemo(() => {
    ...
    if (openRouterImageGenerationSelectedModel) {
      ...
      const modelInfo = IMAGE_GENERATION_MODELS.find(
        (m) =>
          m.value === openRouterImageGenerationSelectedModel &&
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center">
          ...
          >
            <span className="font-medium text-[13px]">
              {t("settings:experimental.IMAGE_GENERATION.name")}
            </span>
          </VSCodeCheckbox>
        </div>
      ...
      {enabled && (
        <div className="flex flex-col gap-5 pl-4 border-l-2 border-vscode-focusBorder/50 ml-1">
          ...
          <div
            style={{
              display: currentProvider === "openrouter" ? "none" : undefined,
            }}
            ...
            <p className="text-vscode-descriptionForeground text-[11px] mt-2 opacity-80 leading-relaxed">
              {currentProfileKilocodeToken ? (
                <a
                  href="#"
                  onClick={() =>
                    handleKiloApiKeyChange(currentProfileKilocodeToken)
                  }
                  className="text-vscode-textLink-foreground hover:text-vscode-textLink-activeForeground"
                >
                  {t(
                    "settings:experimental.IMAGE_GENERATION.kiloCodeApiKeyPaste",
                  )}
                </a>
              ) : (
                <>
          ...
          {currentProvider === "openrouter" && (
            <div data-setting-id="image-gen-key">
              <label className="block text-[13px] font-medium mb-2">
                {t(
                  "settings:experimental.IMAGE_GENERATION.openRouterApiKeyLabel",
                )}
              </label>
              <VSCodeTextField
          ...
          <div data-setting-id="image-gen-model">
            ...
            <p className="text-vscode-descriptionForeground text-[11px] mt-2 opacity-80 leading-relaxed">
              {t(
                "settings:experimental.IMAGE_GENERATION.modelSelectionDescription",
              )}
            </p>
          </div>
