...
export const Tab = ({ className, children, ...props }: TabProps) => (
  // kade_change
  <div className={cn("absolute inset-0 flex flex-col", className)} {...props}>
    {children}
  </div>
);
...
export const TabHeader = ({ className, children, ...props }: TabProps) => (
  <div
    className={cn("px-5 py-2.5 border-b border-vscode-panel-border", className)}
    {...props}
  >
    {children}
  </div>
...
export const TabContent = forwardRef<HTMLDivElement, TabProps>(
  ({ className, children, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn("flex-1 overflow-auto p-5", className)}
        onWheel={onWheel}
        {...props}
      >
        {children}
...
>(
  (
    { children, className, value: _value, isSelected, onSelect, ...props },
    ref,
  ) => {
    return (
      <button
