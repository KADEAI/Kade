...
const getDisplayPath = (text: string) => {
	...
	const pathWithoutLines = lineMatch ? path.slice(0, -lineMatch[1].length) : path
...
const premiumStreamingReveal = keyframes`
	0% {
		opacity: 0.72;
	}
	100% {
...
const StyledMarkdown = styled.div<{ $isStreaming?: boolean }>`
	...
	${({ $isStreaming }) =>
		$isStreaming &&
		css`
			animation: ${premiumStreamingReveal} 0.12s ease-out;
	...
	*/
	...
	li::marker {
		color: color-mix(in srgb, var(--vscode-editor-foreground) 45%, transparent);
		font-weight: 500;
	}
	...
	th, td {
		...
		border-right: 1px solid color-mix(in srgb, var(--vscode-foreground) 8%, transparent);
		white-space: normal;
...
const mentionRegexSource = /(?:^|(?<=[\s\(\)\[\]\{\}'"`]))(?<!\\)@(?:(?:\/|\w+:\/\/)(?:[^\s\\]|\\ )+?|[\w\.-]+\.[a-zA-Z0-9]{2,10}|[a-f0-9]{7,40}\b|problems\b|git-changes\b|terminal\b)(?=[.,;:!?]?(?:[\s\(\)\[\]\{\}'"`]|$))/.source
const absolutePathRegexSource = /(?:(?<=^|[\s\(\)\[\]\{\}'"`])(?:[a-zA-Z]:[\\/]|[\\/]))[^:?*"<>|\s\[\]\(\)\{\}]+(?:\.[a-zA-Z0-9]+)+(?::\d+)?(?=[.,;:!?]?(?:[\s\(\)\[\]\{\}'"`]|$))/.source
...
const genericFileRegexSource = /(?<=^|[\s\(\)\[\]\{\}'"`])[\w\.\-\/\\]*[a-zA-Z][\w\.\-\/\\]*\.[a-zA-Z0-9]{2,10}(?::\d+)?(?=[.,;:!?]?(?:[\s\(\)\[\]\{\}'"`]|$))/.source
...
const MarkdownBlock = memo(({ markdown, className, filePaths, cwd, isStreaming }: MarkdownBlockProps & { isStreaming?: boolean }) => {
		...
		() => ({
			...
			strong: ({ children, ...props }: any) => {
				...
				if (!isInsideLink && typeof children === "string") {
					...
					if (masterRegex.test(text) || text.startsWith("file://") || text.startsWith("/")) {
						let href = text
							...
							<strong {...props}>
								<FileLink href={href} onClick={handleClick} title={fullPath}>
									<span className="file-icon">
										<FileIcon fileName={getCleanPathForIcon(fullPath)} size={16} isDirectory={false} />
									</span>
									{displayName}
								</FileLink>
							</strong>
				...
				if (isLocal) {
					...
					return (
						...
						>
							...
							<LinkContext.Provider value={true}>{displayName}</LinkContext.Provider>
						</FileLink>
					)
				} else {
					return (
	...
	const remarkLinkify = useMemo(() => {
		...
		const delimitedRegex = /((?:\(|\{|<|\[\[|«|\[|'|"))([^()\[\]{}<>«»\s'"]+)(?:\)|\]\}|>|\]\]|»|\]|'|")/g

		// Regex to find naked file paths: must have an extension and be a known file or have path separators
		const nakedFileRegex = /(?<=^|[\s\(\)\[\]\{\}'"`])([\w\.\-\/\\]*[a-zA-Z][\w\.\-\/\\]*\.[a-zA-Z0-9]{2,10}(?::\d+)?)(?=[.,;:!?]?(?:[\s\(\)\[\]\{\}'"`]|$))/g

		return () => {
			return (tree: any) => {
				visit(tree, "text", (node: any, index: any, parent: any) => {
					if (parent && (parent.type === "link" || parent.type === "a" || parent.type === "code")) return
					...
					for (const match of validMatches as any[]) {
						...
						const basename = cleanPathText.split(/[\\/]/).pop()?.split(":")[0]
