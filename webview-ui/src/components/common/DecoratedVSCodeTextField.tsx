...
export interface VSCodeTextFieldWithNodesProps
  extends ComponentProps<typeof VSCodeTextField> {
  leftNodes?: ReactNode[];
...
function VSCodeTextFieldWithNodesInner(
  ...
  const handleVSCodeFieldRef = useCallback(
    (element: ComponentRef<typeof VSCodeTextField>) => {
      ...
      const inputElement =
        webComponent.shadowRoot?.querySelector?.("input") ||
        webComponent.querySelector?.("input");
  ...
  return (
    ...
    >
      {hasLeftNodes && (
        <div className="absolute left-2 z-10 flex items-center gap-1 pointer-events-none">
          {leftNodes}
        </div>
      )}
      ...
      {hasRightNodes && (
        <div className="absolute right-2 z-10 flex items-center gap-1 pointer-events-none">
          {rightNodes}
        </div>
      )}
...
export const DecoratedVSCodeTextField = forwardRef(
  VSCodeTextFieldWithNodesInner,
);
