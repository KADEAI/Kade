...
const DiffView = memo(({ source, filePath }: DiffViewProps) => {
  ...
  const normalizedLang = useMemo(
    () => normalizeLanguage(getLanguageFromPath(filePath || "") || "txt"),
    [filePath],
  );
  ...
  const diffLines = useMemo(
    () => parseUnifiedDiff(source, filePath),
    [source, filePath],
  );
  ...
  const hunks = useMemo(() => {
    ...
    if (
      ...
      currentHunk.some((line) => line.type !== "gap")
    ) {
      const oldLines: string[] = [];
      const newLines: string[] = [];
  ...
  const renderContent = (
    ...
    if (
      !shouldHighlight ||
      !hunk.highlightedOldLines ||
      !hunk.highlightedNewLines
    ) {
      return line.content;
    }

    // Find the line index within the old/new text for this hunk
    const hunkLinesBeforeThis = hunk.lines
      .slice(0, lineIndexInHunk)
      .filter((l) => l.type !== "gap");

    if (line.type === "deletion") {
      // Count deletions and context lines before this line
      const oldLineIndex = hunkLinesBeforeThis.filter(
      ...
      const newLineIndex = hunkLinesBeforeThis.filter(
        (l) => l.type === "addition" || l.type === "context",
      ).length;
      ...
      return (
        ...
        hunk.highlightedOldLines[oldLineIndex] ||
        line.content
      );
    }
