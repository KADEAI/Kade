...
describe("CodeBlock", () => {
  ...
  it("handles theme switching", async () => {
    ...
    await act(async () => {
      const { rerender } = render(
        <CodeBlock source={code} language="typescript" />,
      );
  ...
  it("verifies highlighter utility is used correctly", async () => {
    ...
    expect(highlighterUtil.normalizeLanguage).toHaveBeenCalledWith(
      "typescript",
    );
  });
  ...
  it("trims leading and trailing blank lines", async () => {
    ...
    const renderedCode = screen.getByText(/const x = 1;/);
    expect(renderedCode.textContent).toBe(
      "const x = 1;\nconsole.log(x); [dark-theme]",
    );
  });
});
