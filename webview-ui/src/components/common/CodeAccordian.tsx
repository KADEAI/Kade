...
const CodeAccordian = ({
  ...
  const inferredLanguage = useMemo(
    () => language ?? (path ? getLanguageFromPath(path) : "txt"),
    [path, language],
  );
  const source = useMemo(
    () => String(code).trim() /*kade_change: coerce to string*/,
    [code],
  );
  ...
  const hasValidStats = Boolean(
    derivedStats && (derivedStats.added > 0 || derivedStats.removed > 0),
  );
          ...
          {hasValidStats ? (
            ...
            progressStatus.text && (
              <>
                {progressStatus.icon && (
                  <span
                    className={`codicon codicon-${progressStatus.icon} mr-1`}
                  />
                )}
