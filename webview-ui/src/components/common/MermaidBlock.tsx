...
export default function MermaidBlock({
  ...
  useDebounceEffect(
    () => {
      ...
      getMermaid()
        .then((mermaid) => mermaid.parse(code))
        .then(() => {
          const id = `mermaid-${Math.random().toString(36).substring(2)}`;
          return getMermaid().then((mermaid) => mermaid.render(id, code));
        })
        .then(({ svg }) => {
          //kade_change start
          const sanitizedSvg = svg
            .replace(
              /(<svg\b[^>]*)(>)/i,
              `$1 width="100%" height="100%" preserveAspectRatio="xMidYMid meet"$2`,
            )
  ...
  return (
    <MermaidBlockContainer>
      {isLoading && (
        <LoadingMessage>
          {isFixing /* kade_change */
            ? t("common:mermaid.fixing_syntax")
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center" }}>
              ...
              {!!error && (
                ...
                >
                  <span
                    className={`codicon codicon-${isFixing ? "loading" : "wand"}`}
                  ></span>
                </MermaidFixButton>
              )}
              {/* kade_change end */}
          ...
          {needsExpand && (
            <ExpandRow>
              ...
              >
                ...
                {isExpanded
                  ? t("common:mermaid.collapse_diagram", {
                      defaultValue: "Collapse diagram",
                    })
                  : t("common:mermaid.expand_diagram", {
                      defaultValue: "Expand diagram",
                    })}
              </ExpandButton>
            </ExpandRow>
          )}
...
const ErrorShell = styled.div`
  ...
  border: 1px solid
    color-mix(
      in srgb,
      var(--vscode-editorWarning-foreground) 24%,
  ...
  background: linear-gradient(
    ...
    color-mix(
        in srgb,
        var(--vscode-editorWarning-foreground) 7%,
...
const CopyButton = styled.button`
  ...
  &:hover {
    ...
    background: color-mix(
      in srgb,
      var(--vscode-editor-foreground) 7%,
      transparent
    );
...
const SvgContainer = styled.div<SvgContainerProps>`
  ...
  align-items: center;
  max-height: ${(props) =>
    props.$isExpanded ? "none" : `${MERMAID_MAX_HEIGHT}px`};
  overflow: hidden;
  border-radius: 0;
  border: none;
  background: transparent;
  ...
  & > svg {
    ...
    max-height: ${(props) =>
      props.$isExpanded
        ? "none"
        : `${MERMAID_MAX_HEIGHT - 24}px`}; /* Respect container's max-height */
    margin: 0 auto;
...
const OverflowHint = styled.div`
  ...
  background: linear-gradient(
    ...
    color-mix(in srgb, var(--vscode-editor-background) 75%, rgba(0, 0, 0, 0.7))
      100%
  );
  z-index: 5;
