...
export const EmptyState = ({ onSelectPrompt }: EmptyStateProps) => {
  ...
  const allSuggestions = useMemo(() => {
    const baseSuggestions = !folderName
      ? PROMPT_SUGGESTIONS
      : [
  ...
  const selectedBackground = useMemo(
    () =>
      backgroundOptions.find((option) => option.file === emptyStateBackground),
    [backgroundOptions, emptyStateBackground],
  );
  const selectedBackgroundUri =
    emptyStateBackgroundUri ?? selectedBackground?.uri;
  ...
  const recentSessions = (data?.historyItems ?? []).slice(0, 3).map((item) => ({
    id: item.id,
    title: item.task || (item as any).title,
    timeAgo: formatRelativeShortTime(item.ts),
    diffTotals: getTaskDiffTotals(item),
  }));
  ...
  return (
    <div
      className={`emptyState in-chat ${selectedBackgroundUri ? "has-custom-background" : ""}`}
    >
      {selectedBackgroundUri && (
        <div
          className="empty-state-image-background"
          style={{ backgroundImage: `url("${selectedBackgroundUri}")` }}
          aria-hidden="true"
        />
      )}
              ...
              <div className="sub-agent-ad-content">
                ...
                <div className="sub-agent-ad-text">
                  <div className="flex items-center justify-between mb-1">
                    <span className="sub-agent-ad-title">
                      🚀 Welcome to Kade!
                    </span>
                  </div>
                  <span className="sub-agent-ad-description">
                    To get started, select an AI provider in{" "}
                    <span
                  ...
                  <div className="sub-agent-promo-divider">
                    <span className="sub-agent-ad-description">
                      ...
                      Delegate complex tasks to specialized AI agents. You can
                      disable this banner in Display Settings.
                    </span>
                  </div>
                  <div className="sub-agent-promo-divider">
                    <span className="sub-agent-ad-description">
                      <strong className="text-white/90">Kade 3.0</strong> Kade
                      3.0 is now out! Including hundreds of improvements,
                      redesigns, and fixes!{" "}
                      <a
                        href="https://github.com/KADEAI/Kade/"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        https://github.com/KADEAI/Kade/
                      </a>
                      , if you want to contribute, or star, please do!
                    </span>
                  </div>
                </div>
              </div>
          ...
          {recentSessions.map((s) => (
            ...
            >
              ...
              <div className="recent-item-meta">
                {s.diffTotals && (
                  <div className="recent-item-diffs">
                    {s.diffTotals.additions > 0 && (
                      <span className="recent-item-diff recent-item-diff-add">
                        +{s.diffTotals.additions}
                      </span>
                    )}
                    {s.diffTotals.deletions > 0 && (
                      <span className="recent-item-diff recent-item-diff-remove">
                        -{s.diffTotals.deletions}
                      </span>
                    )}
                  </div>
                )}
