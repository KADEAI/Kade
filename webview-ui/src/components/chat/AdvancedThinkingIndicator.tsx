	...
	return (
		<div className="flex items-center gap-2">
			<div className="relative">
				...
				<Icon className={cn("w-4 h-4 animate-pulse relative z-10", config.color)} />
				{isStreaming && (
...
const DetailedThinkingIndicator: React.FC<AdvancedThinkingIndicatorProps> = ({
	phases = ["initializing", "analyzing", "reasoning", "generating", "verifying"],
	currentPhaseIndex = 0,
	elapsedMs = 0,
	showTiming = true,
	showCost = false,
		...
		<div className="w-full">
			...
			<div className="flex items-center justify-between mb-3">
				<div className="flex items-center gap-2">
					...
					<span className="font-semibold text-vscode-foreground">Thinking</span>
							...
							<React.Fragment key={phase}>
								<button
									...
									<Icon
										...
										style={{ animationDirection: isActive ? "reverse" : "normal" }}
									/>
				...
				<div className="flex items-center gap-2">
					...
					{isStreaming && (
						<span className="flex gap-1">
							<span className="w-1 h-1 bg-vscode-foreground rounded-full animate-bounce" style={{ animationDelay: "0ms" }} />
							<span className="w-1 h-1 bg-vscode-foreground rounded-full animate-bounce" style={{ animationDelay: "150ms" }} />
							<span className="w-1 h-1 bg-vscode-foreground rounded-full animate-bounce" style={{ animationDelay: "300ms" }} />
						</span>
					)}
				</div>
				{showTiming && elapsedMs > 0 && (
...
export const AdvancedThinkingIndicator: React.FC<AdvancedThinkingIndicatorProps> = ({
	style = "detailed",
	compact = false,
	...props
}) => {
		...
		return (
			<div className="flex items-center gap-1.5">
				<div className="flex space-x-1">
					<span className="w-1.5 h-1.5 bg-vscode-foreground rounded-full animate-bounce" style={{ animationDelay: "0ms" }} />
					<span className="w-1.5 h-1.5 bg-vscode-foreground rounded-full animate-bounce" style={{ animationDelay: "150ms" }} />
					<span className="w-1.5 h-1.5 bg-vscode-foreground rounded-full animate-bounce" style={{ animationDelay: "300ms" }} />
				</div>
				<span className="text-xs text-vscode-descriptionForeground">
					{PHASE_CONFIG[props.phase ?? "reasoning"].label}
				</span>
			</div>
		)
	}
	...
	switch (style) {
		...
		case "particles":
			return <ParticlesThinkingIndicator {...props} elapsedMs={displayElapsed} />
		case "detailed":
		default:
			return <DetailedThinkingIndicator {...props} elapsedMs={displayElapsed} />
	}
}

// Compact inline indicator for small spaces
export const ThinkingPulse: React.FC<{ className?: string }> = ({ className }) => (
	<div className={cn("flex items-center gap-1", className)}>
		<span className="w-2 h-2 bg-vscode-foreground rounded-full animate-pulse" />
		<span className="w-2 h-2 bg-vscode-foreground/60 rounded-full animate-pulse" style={{ animationDelay: "150ms" }} />
		<span className="w-2 h-2 bg-vscode-foreground/30 rounded-full animate-pulse" style={{ animationDelay: "300ms" }} />
	</div>
)
...
export const ApiRequestIndicator: React.FC<{
	...
	isRetrying?: boolean
	retryCount?: number
}> = ({ provider, model, elapsedMs = 0, isRetrying = false, retryCount = 0 }) => {
	const { t } = useTranslation()
