...
const TaskHeader = ({
  ...
  const [showLongRunningTaskMessage, setShowLongRunningTaskMessage] =
    useState(false);
  ...
  const showBrowserGlobe =
    browserSessionStartIndex !== -1 || !!isBrowserSessionActive;
  ...
  return (
    <div className={cn("group pb-0 px-3", hasTodos ? "pt-0" : "pt-2")}>
      ...
      <div
        className={cn(
          ...
          hasTodos ? "p-0 border-b-0" : "border-b-0",
        )}
        onClick={(e) => {
          // Don't expand if clicking on todos section
          if (
            e.target instanceof Element &&
            e.target.closest("[data-todo-list]")
          ) {
            return;
          }
                  ...
                  {contextWindow > 0 && (
                    <tr>
                      ...
                      <td className="font-normal align-top">
                        <div
                          className={`max-w-md -mt-1.5 flex flex-nowrap gap-1`}
                        >
