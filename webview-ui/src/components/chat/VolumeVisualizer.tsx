// kade_change - new file: Volume visualizer component for microphone input
...
function normalizeEnergy(rawEnergy: number): number {
  ...
  const normalized = Math.max(
    0,
    (rawEnergy - ENERGY_MIN) / (ENERGY_MAX - ENERGY_MIN),
  );
...
export function VolumeVisualizer({
  ...
  const [barHeights, setBarHeights] = useState<number[]>(
    new Array(BAR_COUNT).fill(MIN_HEIGHT_PERCENT),
  );
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      setBarHeights(
        newHeights.map((h) => Math.max(MIN_HEIGHT_PERCENT, h * 100)),
      );
