...
const StatusIcon: React.FC<{ status: "running" | "done" | "error" }> = ({
  status,
}) => {
  if (status === "running") {
    return (
...
export const FastContextTool: React.FC<FastContextToolProps> = ({
  tool,
  toolResult,
  isLastMessage,
  shouldAnimate,
  ...
  const totalItems =
    operations.length + (hasResults && !isRunning ? results.length : 0);
  ...
  useEffect(() => {
    ...
    if (totalItems > visibleCount) {
      ...
      const interval = setInterval(() => {
        ...
        dispatchChatScrollAnchorAdjust(containerRef.current);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Title */}
        <QueryLabel $isRunning={!!isRunning}>Fast Context</QueryLabel>
            ...
            results.map((result, index) => {
              ...
              return (
                <AnimatedRow key={`r-${index}`} $delay={0}>
                  <ResultItem
                    onClick={() =>
                      handleFileClick(result.file, result.startLine)
                    }
                  >
