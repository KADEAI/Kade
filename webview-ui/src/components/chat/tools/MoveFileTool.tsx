...
const MoveItem = ({ source, destination }: { source: string; destination: string }) => {
    const sourceFilename = source.split(/[\\/]/).filter(Boolean).pop() || source;
    const destFilename = destination.split(/[\\/]/).filter(Boolean).pop() || destination;

    return (
        <div className="flex flex-col gap-0.5 px-2 py-1 border-b border-vscode-editorGroup-border last:border-0 hover:bg-vscode-list-hoverBackground rounded-sm transition-colors group font-mono"
            ...
            >
                <div className="flex items-center gap-1.5 flex-shrink-0 opacity-70 group-hover:opacity-100">
                    ...
                    <span style={{ color: 'var(--vscode-editor-foreground)' }}>{source}</span>
                </div>
            </div>
            ...
            >
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0 font-bold ml-1">
                    ...
                    <span style={{ color: 'var(--vscode-editor-foreground)', lineHeight: '1' }}>{destination}</span>
                </div>
            </div>
        </div>
    );
...
export const MoveFileTool: React.FC<MoveFileToolProps> = ({ tool, toolResult, isLastMessage, shouldAnimate, autoApprovalEnabled }) => {
    ...
    const { moves, hasItems } = useMemo(() => {
        const sources = (tool.source || "").split(",").map((s: string) => s.trim()).filter(Boolean);
        const destinations = (tool.destination || "").split(",").map((d: string) => d.trim()).filter(Boolean);
        ...
        if (destinations.length === 1 && sources.length > 1) {
            ...
            sources.forEach((s: string) => {
                ...
                movePairs.push({
                    source: s,
                    destination: destDir.endsWith('/') || destDir.endsWith('\\') ? `${destDir}${filename}` : `${destDir}/${filename}`
                });
            });
        } else {
            // 1-to-1 mapping
            sources.forEach((s: string, i: number) => {
                movePairs.push({ source: s, destination: destinations[i] || destinations[0] || "" });
            });
        }
    ...
    const actionVerb = useMemo(() => {
        ...
        if (isCopy) {
            if (shouldDeleteSource) return `Moved ${count} ${count === 1 ? 'file' : 'files'}`;
                        ...
                        <MoveList>
                            <div className="flex flex-col max-h-[300px] overflow-y-auto pr-1 scrollbar-thin">
                                {moves.map((m, i: number) => (
                                    <MoveItem key={i} source={m.source} destination={m.destination} />
                                ))}
                            </div>
                        </MoveList>
