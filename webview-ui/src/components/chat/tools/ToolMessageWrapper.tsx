...
export const ToolMessageWrapper: React.FC<ToolMessageWrapperProps> = ({
    ...
    return (
        <Wrapper $isCustomLayout={isCustomLayout} className={shouldAnimate ? "animate-tool-entry" : ""}>
            {isCustomLayout ? (
                children
            ) : (
                <>
            ...
            {permissionState === 'pending' && (
                <PermissionActions>
                    <RejectButton
                        ...
                        style={{ opacity: checkPending ? 0.6 : 1, cursor: checkPending ? 'not-allowed' : 'pointer' }}
                    >
                        <span>Reject</span>
                    </RejectButton>
                    <AcceptButton
