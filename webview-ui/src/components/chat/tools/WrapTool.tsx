...
const effectMap: Record<string, ReturnType<typeof css>> = {
  ...
  `,
  bounce: css`animation: ${bounceFloat} 0.9s infinite ease-in-out;`,
  pulse: css`animation: ${pulseGlow} 1.5s infinite ease-in-out;`,
  wave: css`animation: ${waveRipple} 2s infinite ease-in-out;`,
  rainbow: css`animation: ${rainbowShift} 4s infinite linear;`,
  neon: css`animation: ${flickerNeon} 2s infinite;`,
  typewriter: css`
    border-right: 2px solid currentColor;
    animation: ${typewriterBlink} 0.8s infinite;
    display: inline-block;
  `,
  shake: css`animation: ${shakeRattle} 0.5s infinite;`,
  slide: css`animation: ${slideIn} 0.6s ease-out both;`,
  fade: css`animation: ${fadeInUp} 0.5s ease-out both;`,
  scanline: css`
    &::after {
      content: '';
      position: absolute;
  ...
  `,
  fire: css`animation: ${fireFlicker} 0.8s infinite alternate;`,
}
...
const ConfettiParticle = styled.span<{ $delay: number; $x: number; $char: string }>`
  position: absolute;
  top: -10px;
  left: ${p => p.$x}%;
  font-size: 14px;
...
const SparkleParticle = styled.span<{ $delay: number; $x: number; $y: number }>`
  ...
  font-size: 10px;
  color: #ffd700;
  animation: ${sparkleKeyframes} ${() => 1 + Math.random() * 1.5}s ease-in-out infinite;
  animation-delay: ${p => p.$delay}s;
`
...
function GuiWidget({ type }: { type: string }) {
  ...
  if (type === "sparkle" || type === "sparkles") {
    return (
      <GuiOverlay>
        {particles.map(p => (
          <SparkleParticle key={p.id} $delay={p.delay} $x={p.x} $y={"y" in p ? (p as any).y : 50}>
            {p.char}
          </SparkleParticle>
        ))}
      </GuiOverlay>
  ...
  if (/\p{Emoji}/u.test(type)) {
    return (
      <GuiOverlay>
        <span style={{ position: "absolute", top: 4, right: 8, fontSize: 18, opacity: 0.8 }}>{type}</span>
        <span style={{ position: "absolute", bottom: 4, left: 8, fontSize: 18, opacity: 0.8 }}>{type}</span>
      </GuiOverlay>
    )
  }
...
const WrapContainer = styled.div<WrapStyleProps>`
  ...
  ${({ $style }) => $style && stylePresets[$style] ? css`${stylePresets[$style]}` : ""}

  /* Custom overrides from AI */
  ${({ $color }) => $color ? css`color: ${$color};` : ""}
  ${({ $bg }) => $bg ? css`background: ${$bg};` : ""}
  ${({ $border }) => $border ? css`border: 1px solid ${$border};` : ""}
  ...
  ${({ $intensity }) => {
    ...
    if (isNaN(val)) return ""
    return css`animation-duration: ${Math.max(0.1, 2 / val)}s;`
  }}

  .wrap-content {
    position: relative;
    z-index: 1;
    ${({ $effect }) => $effect && effectMap[$effect] ? effectMap[$effect] : ""}
  }
`
