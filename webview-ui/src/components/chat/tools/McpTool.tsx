...
export const McpTool: React.FC<McpToolProps> = ({
    ...
    const isPermissionRequest = !hasResponse && !hasError && !isComplete && isLastMessage;
        ...
        <div className={shouldAnimate ? 'animate-tool-entry' : ''}>
            <div style={{ marginTop: '-10px', marginBottom: '-3px' }}>
                <ToolHeader
                    ...
                    details={
                        <span className="text-vscode-descriptionForeground opacity-75 truncate antialiased" title={`${serverName}/${toolName}`}>
                            {serverName}
                        </span>
                    }
                    extra={
            ...
            <AnimatedAccordion isExpanded={isExpanded}>
                <div className="mt-1 ml-2 border-l-2 border-vscode-editorGroup-border pl-4 text-[length:var(--codex-chat-font-size,14px)] leading-[1.5]" style={{ fontFamily: 'var(--font-sans, var(--vscode-font-family))' }}>
                    <ScrollableContent>
                        ...
                        {formattedArgs && (
                            <div className="mb-2">
                                <div className="text-[10px] font-bold uppercase tracking-wider opacity-40 mb-1">Arguments</div>
                        ...
                        {hasResponse && (
                            <div>
                                <div className="text-[10px] font-bold uppercase tracking-wider opacity-40 mb-1">Response</div>
                        ...
                        {hasError && 'error' in status && status.error && (
                            <ToolError toolResult={{ is_error: true, content: status.error }} />
                        )}
                    </ScrollableContent>
                </div>
            </AnimatedAccordion>
