		...
		<div className="group/todo p-0 m-0 w-full">
			...
			{!isCollapsed && (
				<div className="bg-transparent overflow-hidden p-0 m-0">
					<ul ref={ulRef} className="list-none max-h-[300px] overflow-y-auto flex flex-col p-0 m-0">
						{todos.map((todo: any, idx: number) => {
							...
							return (
								<li
									...
									className={cn(
										"flex items-start gap-1.5 px-2 py-0.5 transition-all duration-300 border-l-2 border-transparent",
										isInProgress ? "border-vscode-charts-yellow/60 bg-vscode-charts-yellow/5 shadow-[inset_0_0_12px_rgba(234,179,8,0.05)]" : "hover:bg-vscode-list-hoverBackground/25",
									)}>
									<div className="pt-0.5 shrink-0">{icon}</div>
									<span
										className={cn(
											"leading-snug break-words text-sm transition-all duration-300",
											isCompleted && "text-vscode-descriptionForeground/60 line-through opacity-50",
											isInProgress && "text-vscode-charts-yellow font-semibold opacity-100 drop-shadow-[0_0_8px_rgba(234,179,8,0.2)]"
										)}>
										{todo.content}
									</span>
								</li>
