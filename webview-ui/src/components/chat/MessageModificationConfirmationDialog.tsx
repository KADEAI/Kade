...
import { AlertTriangle, Trash2, Undo2, Edit3, X, History as HistoryIcon, FilePlus } from "lucide-react"
...
export const MessageModificationConfirmationDialog: React.FC<MessageModificationConfirmationDialogProps> = ({
	...
	const title = isEdit ? t("common:confirmation.editMessage") : "Unsend Message"
	const description = isEdit ? t("common:confirmation.editWarning") : "Unsending this message will remove it and all subsequent messages in the conversation. Do you want to proceed?"
				...
				<div className="flex flex-col max-h-[85vh]">
					...
					<div className="p-4 pb-2 text-center space-y-2.5">
						<div className={cn(
							...
							isEdit ? "bg-blue-500/10 border-blue-500/20" : "bg-red-500/10 border-red-500/20"
						)}>
							{isEdit ? <Edit3 className="text-blue-400" size={18} /> : <Undo2 className="text-red-400" size={18} />}
						</div>
						...
						<div className="px-4 pb-4">
							<div className="bg-black/20 rounded-xl border border-white/5 overflow-hidden">
								<div className="px-2.5 py-1.5 border-b border-white/5 bg-white/5 flex items-center gap-2">
									<HistoryIcon size={10} className="text-vscode-descriptionForeground" />
									<span className="text-[9px] font-bold uppercase tracking-wider text-vscode-descriptionForeground opacity-80">
										{t("common:confirmation.revertFileChanges") || "Changes to Revert"}
									</span>
								...
								<div className="max-h-[120px] overflow-y-auto p-1 space-y-0.5 custom-scrollbar">
									{fileChanges.map((file, idx) => (
										<div key={idx} className="flex items-center gap-2 px-2 py-1 rounded-lg hover:bg-white/5 transition-colors group">
											...
											<div className="flex items-center gap-1.5 shrink-0 ml-1">
												{file.additions > 0 && (
													<span className="text-[10px] font-bold text-green-400/90">+{file.additions}</span>
												)}
												{file.deletions > 0 && (
													<span className="text-[10px] font-bold text-red-400/90">-{file.deletions}</span>
												)}
											</div>
										</div>
									))}
					...
					<AlertDialogFooter className="p-5 pt-2 flex flex-row gap-2">
						...
						<AlertDialogAction
							...
							className={cn(
								...
								isEdit ? "bg-blue-600 hover:bg-blue-500 shadow-blue-600/20" : "bg-red-600 hover:bg-red-500 shadow-red-600/20",
							)}>
