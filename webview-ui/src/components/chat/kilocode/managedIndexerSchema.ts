...
export type ManagedIndexerStateMessage = z.infer<
  typeof managedIndexerStateMessageSchema
>;

/**
 * Parse and validate managed indexer state message
 */
export function parseManagedIndexerStateMessage(
  data: unknown,
): ManagedIndexerState | null {
  try {
    ...
    console.error(
      "[parseManagedIndexerStateMessage] Failed to parse message:",
      error,
    );
    return null;
  }
