...
const CodebaseSearchResultsDisplay: React.FC<CodebaseSearchResultsDisplayProps> = ({
	...
	<div className="-ml-0.5">
		<ToolHeader
			toolName="ask"
			actionVerb={isComplete ? t("chat:codebaseSearch.asked", "Asked") : t("chat:codebaseSearch.asking", "Asking")}
			...
			details={
				<div className="flex items-center min-w-0 flex-1 pr-4">
					<span className="truncate antialiased flex-1">
						{(activeQuery || t("chat:codebaseSearch.semanticSearch", "semantic search")).replace(/\?+$/, "")}
					</span>
					<span
						...
						<div className="flex flex-wrap gap-2 mb-3 mt-1">
							{queries.map((q) => (
								<button
									...
									className={`px-2 py-0.5 rounded text-[10px] font-medium transition-colors border ${activeQuery === q
						...
						{groupedResults[activeQuery || queries[0]]?.map((result, idx) => {
							...
							return (
								<div
									...
									onClick={() => handleFileClick(result.filePath, result.startLine)}>
									<div className="flex items-center gap-2 text-[12px] font-mono">
										...
										<span className="text-[10px] opacity-40 truncate">{result.filePath}</span>
