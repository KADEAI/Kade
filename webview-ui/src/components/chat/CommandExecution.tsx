...
export const CommandExecution = ({
  ...
  const onMessage = useCallback(
    (event: MessageEvent) => {
      ...
      if (message.type === "commandExecutionStatus") {
        const result = commandExecutionStatusSchema.safeParse(
  ...
  const [storedCommand, setStoredCommand] = useState<string | undefined>(
    undefined,
  );
  ...
  const commandFromStatus =
    status?.status === "started" ? status.command : undefined;
  const finalCommand =
    storedCommand ||
    commandFromStatus ||
    assistantCommand ||
  ...
  return (
    <div
      className={cn(
        "mb-1",
        shouldAnimate && "animate-tool-entry",
        isLast && "will-change-transform",
      )}
    >
      <Bash
        ...
        isKey={
          ...
          (terminalShellIntegrationDisabled && !status && isLast)
        }
        isRunning={
          status?.status === "started" ||
          (terminalShellIntegrationDisabled && !status && isLast)
        }
        startCollapsedOnMount={startCollapsed}
