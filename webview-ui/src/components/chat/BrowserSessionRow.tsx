...
const BrowserSessionRow = memo((props: BrowserSessionRowProps) => {
	const { messages, isLast, onHeightChange, lastModifiedMessage, onExpandChange, fullScreen, liveScreenshot } = props
		...
		return (
			<div
				...
				<div
					...
					{(liveScreenshot && currentPageIndex === pages.length - 1) || displayState.screenshot ? (
						<img
							key={liveScreenshot && currentPageIndex === pages.length - 1 ? "live" : displayState.screenshot}
							src={(liveScreenshot && currentPageIndex === pages.length - 1) ? liveScreenshot : displayState.screenshot}
							alt={t("chat:browser.screenshot")}
							className={(liveScreenshot && currentPageIndex === pages.length - 1) ? "" : "animate-in fade-in zoom-in-95 duration-500 ease-out"}
							style={{
								position: "absolute",
								top: 0,
								left: 0,
							...
							onClick={() =>
								vscode.postMessage({
									type: "openImage",
									text: (liveScreenshot && currentPageIndex === pages.length - 1) ? liveScreenshot : displayState.screenshot,
								})
							}
						/>
					) : (
