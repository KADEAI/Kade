  ...
  return (
    <div
      ...
      onWheel={(e) => {
        ...
        (ref as React.MutableRefObject<HTMLDivElement>).current!.scrollLeft +=
          e.deltaY;
      }}
    >
      {children}
...
export const TaskTimeline = memo<TaskTimelineProps>(
  ({ groupedMessages, onMessageClick, isTaskActive = false }) => {
    ...
    const timelineMessagesData = useMemo(() => {
      const { processedMessages, messageToOriginalIndex } =
        consolidateMessagesForTimeline(groupedMessages);
    ...
    useEffect(() => {
      ...
      if (
        timelineMessagesData.length > 0 &&
        (hasNewMessages || isInitialMount)
      ) {
      ...
      <TooltipProvider>
        ...
        >
          <Virtuoso
            ...
            className="w-full"
            style={{
              height: `${TASK_TIMELINE_MAX_HEIGHT_PX + SCROLLBAR_WIDTH_PX}px`,
            }}
          />
        </div>
      </TooltipProvider>
