...
export const ChatScrollDebugger: React.FC<ChatScrollDebuggerProps> = ({
    ...
    const [scrollInfo, setScrollInfo] = useState({ top: 0, height: 0, client: 0 })
    const [showLayers, setShowLayers] = useState(false)
    const [heightLogs, setHeightLogs] = useState<{ ts: number; delta: number; index: number }[]>([])
    ...
    useEffect(() => {
        const handleHeightLog = (e: any) => {
            setHeightLogs((prev: any[]) => [{ ts: Date.now(), ...e.detail }, ...prev].slice(0, 10))
        }
        window.addEventListener("chat-height-change-log", handleHeightLog)
        return () => window.removeEventListener("chat-height-change-log", handleHeightLog)
    }, [])
    ...
    return (
        <div
            ...
            <div style={{ marginTop: "5px", borderTop: "1px dotted #00ff00", paddingTop: "5px" }}>
                ST: {scrollInfo.top.toFixed(1)}px / SH: {scrollInfo.height}px
            </div>
            <div>Diff: {(scrollInfo.height - scrollInfo.top - scrollInfo.client).toFixed(1)}px</div>

            <div style={{ marginTop: "5px", borderTop: "1px dotted #00ff00", paddingTop: "5px" }}>
                <b>RECENT HEIGHT CHANGES:</b>
                {heightLogs.map((log: { ts: number; delta: number; index: number }, i: number) => (
                    <div key={log.ts + (i as number)}>
                        #{log.index}: {log.delta > 0 ? "+" : ""}{log.delta}px
                    </div>
                ))}
            </div>
        </div>
