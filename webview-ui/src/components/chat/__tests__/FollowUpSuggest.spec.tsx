...
vi.mock("@src/i18n/TranslationContext", () => ({
  ...
  useAppTranslation: () => ({
    t: (key: string, options?: any) => {
      if (
        key === "chat:followUpSuggest.countdownDisplay" &&
      ...
      if (
        key === "chat:followUpSuggest.timerPrefix" &&
...
const TestExtensionStateContext = createContext<TestExtensionState | undefined>(
  undefined,
);
...
vi.mock("@src/context/ExtensionStateContext", () => ({
  useExtensionState: () => {
    ...
    if (!context) {
      throw new Error(
        "useExtensionState must be used within TestExtensionStateProvider",
      );
    }
    return context;
  },
...
const TestExtensionStateProvider: React.FC<{
  ...
  return (
    <TestExtensionStateContext.Provider value={value}>
      {children}
    </TestExtensionStateContext.Provider>
  );
};
...
describe("FollowUpSuggest", () => {
  const mockSuggestions = [
    { answer: "First suggestion" },
    { answer: "Second suggestion" },
  ];
