...
describe("CommandPatternSelector", () => {
  ...
  it("should render with command permissions header", () => {
    ...
    expect(
      screen.getByText("chat:commandExecution.manageCommands"),
    ).toBeInTheDocument();
  });
  ...
  it("should show patterns when expanded", () => {
    ...
    const manageCommandsButton = screen
      .getByText("chat:commandExecution.manageCommands")
      .closest("button");
    ...
    const patternsContainer =
      manageCommandsButton?.nextElementSibling as HTMLElement;
  ...
  it("should show allowed status for patterns in allowed list", () => {
    ...
    const allowButton = npmInstallPattern?.querySelector(
      'button[aria-label*="removeFromAllowed"]',
    );
  ...
  it("should show denied status for patterns in denied list", () => {
    ...
    const denyButton = gitPushPattern?.querySelector(
      'button[aria-label*="removeFromDenied"]',
    );
  ...
  it("should call onAllowPatternChange when allow button is clicked", () => {
    ...
    const allowButton = patternRow?.querySelector(
      'button[aria-label*="addToAllowed"]',
    );
    fireEvent.click(allowButton!);

    // Check that the callback was called with the pattern
    expect(mockOnAllowPatternChange).toHaveBeenCalledWith(
      "npm install express",
    );
  });
  ...
  it("should call onDenyPatternChange when deny button is clicked", () => {
    ...
    const patternText = getByText("npm install express");
    const patternRow = patternText.closest(".flex")?.parentElement;
    const denyButton = patternRow?.querySelector(
      'button[aria-label*="addToDenied"]',
    );
