...
vi.mock("../ChatRow", () => ({
  default: () => null,
  clearToolResultCache: vi.fn(),
  setMessageStore: vi.fn(),
  setExtensionStateStore: vi.fn(),
}));
...
describe("ChatView - Keyboard Shortcut Fix for Dvorak", () => {
  ...
  it("uses event.key instead of event.code for keyboard shortcuts", async () => {
    ...
    const modeSwitchAfterPeriod = callsAfterPeriod.some(
      (call: any[]) => call[0]?.type === "mode",
    );
    ...
    const modeSwitchAfterV = callsAfterV.some(
      (call: any[]) => call[0]?.type === "mode",
    );
