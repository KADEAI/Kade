...
describe("TaskHeader", () => {
  ...
  it("should render the condense context button when expanded", () => {
    ...
    const condenseButton = buttons.find((button) =>
      button.querySelector("svg.lucide-fold-vertical"),
    );
  ...
  describe("DismissibleUpsell behavior", () => {
    ...
    it("should show DismissibleUpsell after 2 minutes when task is not complete", async () => {
      ...
      expect(
        screen.queryByTestId("dismissible-upsell"),
      ...
      expect(
        screen.getByText("cloud:upsell.longRunningTask"),
      ).toBeInTheDocument();
    });
