...
describe("ApiConfigSelector", () => {
  ...
  test("applies custom trigger className", () => {
    ...
    render(
      <ApiConfigSelector {...defaultProps} triggerClassName={customClass} />,
    );
  ...
  test("renders info blurb instead of search when 6 or fewer configs", () => {
    ...
    expect(
      screen.queryByPlaceholderText("common:ui.search_placeholder"),
    ).not.toBeInTheDocument();
    // Should have info blurb
    expect(
      screen.getByText("prompts:apiConfiguration.select"),
    ).toBeInTheDocument();
  });
  ...
  test("shows check mark for selected config", () => {
    ...
    const configInDropdown = config1Elements.find((el) =>
  ...
  test("toggles pin status when pin button is clicked", () => {
    ...
    const pinButton = Array.from(buttons || []).find((btn) =>
  ...
  test("renders bottom bar with title and info icon when more than 6 configs", () => {
    ...
    // Check for the title
    expect(
      screen.getByText("prompts:apiConfiguration.title"),
    ).toBeInTheDocument();

    // Check for the info icon
    const infoIcon = screen
      .getByTestId("popover-content")
      .querySelector(".codicon-info");
  ...
  test("pinned configs remain fixed at top while unpinned configs scroll", () => {
    ...
    const scrollContainer = popoverContent.querySelector(
      ".max-h-\\[300px\\].overflow-y-auto",
    );
    expect(scrollContainer).toBeInTheDocument();

    // Check for pinned configs sticky header
    const pinnedStickyHeader = scrollContainer?.querySelector(
      ".sticky.top-0.z-10.bg-vscode-dropdown-background",
    );
    expect(pinnedStickyHeader).toBeInTheDocument();
    expect(pinnedStickyHeader).toHaveAttribute(
      "aria-label",
