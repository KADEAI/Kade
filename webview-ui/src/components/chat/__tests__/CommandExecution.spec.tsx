...
vi.mock("../../kilocode/common/CodeBlock", () => ({
  default: ({ source }: { source: string }) => (
    <div data-testid="code-block">{source}</div>
  ),
}));
...
vi.mock("../CommandPatternSelector", () => ({
  CommandPatternSelector: ({
    ...
    <div data-testid="command-pattern-selector">
      ...
      <button onClick={() => onAllowPatternChange(patterns[0]?.pattern)}>
        Allow
      </button>
      <button onClick={() => onDenyPatternChange(patterns[0]?.pattern)}>
        Deny
      </button>
    </div>
  ),
...
Suggested patterns: npm, npm install, npm run`;
  ...
  it("should not show pattern selector for empty commands", () => {
    ...
    expect(
      screen.queryByTestId("command-pattern-selector"),
...
Output here`;
  ...
  it("should handle command change when moving from denied to allowed", () => {
    ...
    expect(stateWithRmInDenied.setAllowedCommands).toHaveBeenCalledWith([
      "npm",
      "rm file.txt",
    ]);
