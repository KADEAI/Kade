  ...
  switch (provider) {
    case "openai":
      return baseSchema.extend({
        codeIndexOpenAiKey: z
          .string()
          .min(1, t("settings:codeIndex.validation.openaiApiKeyRequired")),
    ...
    case "ollama":
      return baseSchema.extend({
        ...
        codebaseIndexEmbedderModelId: z
          .string()
          .min(1, t("settings:codeIndex.validation.modelIdRequired")),
    ...
    case "mistral":
      return baseSchema.extend({
        codebaseIndexMistralApiKey: z
          .string()
          .min(1, t("settings:codeIndex.validation.mistralApiKeyRequired")),
        codebaseIndexEmbedderModelId: z
          .string()
          .min(1, t("settings:codeIndex.validation.modelSelectionRequired")),
      });
    ...
    case "vercel-ai-gateway":
      return baseSchema.extend({
        codebaseIndexVercelAiGatewayApiKey: z
          ...
          .min(
            1,
            t("settings:codeIndex.validation.vercelAiGatewayApiKeyRequired"),
          ),
        codebaseIndexEmbedderModelId: z
          .string()
          .min(1, t("settings:codeIndex.validation.modelSelectionRequired")),
    ...
    case "bedrock":
      return baseSchema.extend({
        codebaseIndexBedrockRegion: z
          .string()
          .min(1, t("settings:codeIndex.validation.bedrockRegionRequired")),
...
export const CodeIndexPopover: React.FC<CodeIndexPopoverProps> = ({
  ...
  const { codebaseIndexConfig, codebaseIndexModels, cwd, apiConfiguration } =
    useExtensionState();
  ...
  const [initialSettings, setInitialSettings] =
    useState<LocalCodeIndexSettings>(getDefaultSettings());
    ...
    if (codebaseIndexConfig) {
      const settings = {
        ...
        codebaseIndexScannerMaxBatchRetries:
          ...
          CODEBASE_INDEX_DEFAULTS.DEFAULT_SCANNER_MAX_BATCH_RETRIES,
        codebaseIndexIncludePaths:
          codebaseIndexConfig.codebaseIndexIncludePaths || [],
        codebaseIndexBedrockRegion:
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent<any>) => {
      if (event.data.type === "indexingStatusUpdate") {
        if (
          !event.data.values.workspacePath ||
          event.data.values.workspacePath === cwd
        ) {
          setIndexingStatus({
    ...
    const handleFolderSelection = (event: MessageEvent<any>) => {
      if (
        event.data.type === "vsCodeSetting" &&
        event.data.setting === "selectedFolder"
      ) {
        const folderPath = event.data.value;
        if (folderPath) {
          setCurrentSettings((prev) => {
            const existingPaths = prev.codebaseIndexIncludePaths || [];
            if (existingPaths.includes(folderPath)) return prev;
            return {
              ...prev,
              codebaseIndexIncludePaths: [...existingPaths, folderPath],
            };
          });
        }
      }
    ...
    window.addEventListener("message", handleMessage);
    window.addEventListener("message", handleFolderSelection);
    return () => {
      window.removeEventListener("message", handleMessage);
      window.removeEventListener("message", handleFolderSelection);
    };
  }, [t, cwd]);
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data.type === "codeIndexSecretStatus") {
        ...
        const updateWithSecrets = (
          prev: LocalCodeIndexSettings,
        ): LocalCodeIndexSettings => {
          const updated = { ...prev };
          ...
          if (
            ...
            prev.codeIndexOpenAiKey === SECRET_PLACEHOLDER
          ) {
            updated.codeIndexOpenAiKey = secretStatus.hasOpenAiKey
              ? SECRET_PLACEHOLDER
              : "";
          }
          if (
            !prev.codeIndexQdrantApiKey ||
            prev.codeIndexQdrantApiKey === SECRET_PLACEHOLDER
          ) {
            updated.codeIndexQdrantApiKey = secretStatus.hasQdrantApiKey
              ? SECRET_PLACEHOLDER
              : "";
          }
          if (
            ...
            updated.codebaseIndexOpenAiCompatibleApiKey =
              secretStatus.hasOpenAiCompatibleApiKey ? SECRET_PLACEHOLDER : "";
          }
          if (
            !prev.codebaseIndexGeminiApiKey ||
            prev.codebaseIndexGeminiApiKey === SECRET_PLACEHOLDER
          ) {
            updated.codebaseIndexGeminiApiKey = secretStatus.hasGeminiApiKey
              ? SECRET_PLACEHOLDER
              : "";
          }
          if (
            ...
            updated.codebaseIndexMistralApiKey = secretStatus.hasMistralApiKey
              ? SECRET_PLACEHOLDER
              : "";
          }
          if (
            ...
            updated.codebaseIndexVercelAiGatewayApiKey =
              secretStatus.hasVercelAiGatewayApiKey ? SECRET_PLACEHOLDER : "";
          }
          if (
            ...
            updated.codebaseIndexOpenRouterApiKey =
              secretStatus.hasOpenRouterApiKey ? SECRET_PLACEHOLDER : "";
          }
        ...
        if (saveStatus === "idle" || saveStatus === "saved") {
          setCurrentSettings((prev: any) => updateWithSecrets(prev));
          setInitialSettings((prev: any) => updateWithSecrets(prev));
        }
      }
    };
  ...
  const hasUnsavedChanges = useMemo(() => {
    ...
    ] as Array<keyof LocalCodeIndexSettings>;
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <span className="font-medium">
                    {t("settings:codeIndex.enableLabel")} (Semantic Search)
                  </span>
                </VSCodeCheckbox>
                <StandardTooltip
                  content={t("settings:codeIndex.enableDescription")}
                >
                  <span className="codicon codicon-info text-xs text-vscode-descriptionForeground cursor-help" />
                </StandardTooltip>
              </div>
              <p className="text-xs text-vscode-descriptionForeground mt-1 ml-6">
                Basic code search (Ask) is always available. Enable this for
                ...
                <div className="mt-4 space-y-4">
                  ...
                  {currentSettings.codebaseIndexEmbedderProvider ===
                    ...
                    <>
                      <div className="space-y-2">
                        ...
                        <VSCodeTextField
                          value={
                            currentSettings.codebaseIndexEmbedderBaseUrl || ""
                          }
                          onInput={(e: any) =>
                            updateSetting(
                              "codebaseIndexEmbedderBaseUrl",
                          ...
                          placeholder={t(
                            "settings:codeIndex.ollamaUrlPlaceholder",
                          )}
                          className={cn("w-full", {
                            "border-red-500":
                    ...
                    <>
                      <div className="space-y-2">
                        ...
                        <VSCodeTextField
                          type="password"
                          value={
                            currentSettings.codebaseIndexMistralApiKey || ""
                          }
                          onInput={(e: any) =>
                            updateSetting(
                              "codebaseIndexMistralApiKey",
                    ...
                    <>
                      <div className="space-y-2">
                        ...
                        <VSCodeTextField
                          type="password"
                          value={
                            currentSettings.codebaseIndexOpenRouterApiKey || ""
                          }
                          onInput={(e: any) =>
                            updateSetting(
                              "codebaseIndexOpenRouterApiKey",
                      ...
                      {openRouterEmbeddingProviders &&
                        Object.keys(openRouterEmbeddingProviders).length >
                          0 && (
                          <div className="space-y-2">
                            <label className="text-sm font-medium">
                              <a
                    ...
                    <>
                      <div className="space-y-2">
                        ...
                        <VSCodeTextField
                          ...
                          placeholder={t(
                            "settings:codeIndex.qdrantUrlPlaceholder",
                          )}
                          className={cn("w-full", {
                  ...
                  {currentSettings.codebaseIndexVectorStoreProvider ===
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-medium">
                        {t(
                          "settings:codeIndex.lancedbVectorStoreDirectoryLabel",
                        )}
                      </label>
                      <VSCodeTextField
                        value={
                      ...
                      <p className="text-xs text-vscode-descriptionForeground">
                        {t(
                          "settings:codeIndex.lancedbVectorStoreDirectoryDescription",
                        )}
                      </p>
                    </div>
                  )}
              ...
              {isAdvancedSettingsOpen && (
                <div className="mt-4 space-y-4">
                  ...
                  <EmbeddingBatchSizeSlider
                    ...
                    onChange={(value) =>
                      updateSetting("codebaseIndexEmbeddingBatchSize", value)
                    }
                  />
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <label className="text-sm font-medium">
                        {t("settings:codeIndex.includePathsLabel") ||
                          "Include Only Folders"}
                      </label>
                      <StandardTooltip
                        content={
                          t("settings:codeIndex.includePathsTooltip") ||
                          "Recommended for large codebases. When specified, only these folders will be indexed instead of the entire workspace."
                        }
                      >
                        <span className="codicon codicon-info text-xs text-vscode-descriptionForeground cursor-help" />
                      </StandardTooltip>
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                      {(currentSettings.codebaseIndexIncludePaths || []).map(
                        (path) => (
                          <div
                            key={path}
                            className="flex items-center gap-1 px-2 py-1 bg-secondary text-secondary-foreground rounded-md text-xs group"
                          >
                            <span>{path}</span>
                            <button
                              onClick={() => {
                                const newPaths = (
                                  currentSettings.codebaseIndexIncludePaths ||
                                  []
                                ).filter((p) => p !== path);
                                updateSetting(
                                  "codebaseIndexIncludePaths",
                    ...
                    <p className="text-xs text-vscode-descriptionForeground">
                      {t("settings:codeIndex.excludedPathsHint") ||
                        "Relative to workspace root. Patterns like 'node_modules' are already ignored."}
                    </p>
                  </div>
                </div>
              )}
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            {t(
                              "settings:codeIndex.clearDataDialog.description",
                            )}
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          <AlertDialogCancel>
                            {t(
                              "settings:codeIndex.clearDataDialog.cancelButton",
                            )}
                          </AlertDialogCancel>
                          <AlertDialogAction
                            onClick={() =>
                          ...
                          >
                            {t(
                              "settings:codeIndex.clearDataDialog.confirmButton",
                            )}
                          </AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
