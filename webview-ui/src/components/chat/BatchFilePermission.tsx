...
export const BatchFilePermission = memo(({ files = [], onPermissionResponse, ts, isLastMessage }: BatchFilePermissionProps) => {
				...
				return (
					...
					>
						<div className="flex items-center gap-1.5">
							{showLoading ? (
								<span className="codicon codicon-loading codicon-modifier-spin opacity-50" style={{ fontSize: '12px' }} />
							) : (
								<span className="codicon codicon-check opacity-50" style={{ fontSize: '12px' }} />
							)}
								...
								onClick={() =>
									vscode.postMessage({
										...
										values: file.lineRange
											? { line: file.lineRange.start, endLine: file.lineRange.end }
											: undefined,
									})
								}
							>
