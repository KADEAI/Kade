...
const STATUS_OPTIONS = [
  {
    ...
    label: "Not Started",
    icon: Circle,
    color: "text-vscode-descriptionForeground/60",
  },
  {
    value: "in_progress",
    label: "In Progress",
    icon: Loader2,
    color: "text-vscode-charts-yellow",
  },
  {
    value: "completed",
    label: "Completed",
...
const UpdateTodoListToolBlock: React.FC<UpdateTodoListToolBlockProps> = ({
  ...
  const [editTodos, setEditTodos] = useState<TodoItem[]>(
    todos.length > 0
      ? todos.map((todo) => ({ ...todo, id: todo.id || genId() }))
      : [],
  );
  ...
  const completedCount = editTodos.filter(
  ...
  if (userEdited) {
    return (
      <ToolUseBlock>
        ...
        <div className="py-2 text-vscode-descriptionForeground">
          {content || "User modified the todo list."}
        </div>
      </ToolUseBlock>
    );
  }
  ...
  return (
    <div className="select-none p-0 m-0 w-full">
      ...
      <div className="flex items-center justify-between px-2 py-0.5 mb-0 bg-vscode-input-background/40 border border-vscode-widget-border/60 rounded-t-lg">
        <div className="flex items-center gap-1.5">
          <span className="text-[11px] font-medium text-vscode-foreground opacity-80">
      ...
      <div className="bg-vscode-input-background/40 rounded-b-lg overflow-hidden border-x border-b border-vscode-widget-border/60 p-0 m-0">
        {Array.isArray(editTodos) && editTodos.length > 0 ? (
          <ul className="flex flex-col p-0 m-0">
            {editTodos.map((todo, idx) => {
              const currentStatus =
                STATUS_OPTIONS.find((s) => s.value === (todo.status || "")) ||
                STATUS_OPTIONS[0];
              ...
              return (
                <li
                  key={todo.id || idx}
                  className={cn(
                    "group flex items-start gap-1.5 px-2 py-0 text-sm transition-colors",
                    isEditing
                      ? "bg-vscode-editor-background py-0.5"
                      : "hover:bg-vscode-list-hoverBackground/40",
                  )}
                >
                  {isEditing ? (
                    ...
                    <>
                      ...
                      <span
                        className={cn(
                          "flex-1 leading-snug break-words opacity-90 transition-all duration-300",
                          todo.status === "completed" &&
                            "text-vscode-descriptionForeground/60 line-through opacity-50",
                          todo.status === "in_progress" &&
                            "text-vscode-charts-yellow font-medium opacity-100",
                        )}
                      >
                        {todo.content}
                      </span>
            ...
            {adding ? (
              <li className="flex items-center gap-2 px-2 py-1 bg-vscode-editor-background rounded-sm mt-0.5 animate-in fade-in duration-200">
                ...
                <div className="flex gap-1">
                  <button
                    onClick={handleAdd}
                    className="hover:text-vscode-charts-green p-0.5"
                  >
