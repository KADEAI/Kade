...
import {
  useManagedIndexerState,
  useIsIndexing,
} from "./hooks/useManagedIndexerState"; // kade_change
...
export const IndexingStatusBadge: React.FC<IndexingStatusBadgeProps> = ({
  ...
  const [localIndexingStatus, setLocalIndexingStatus] =
    useState<IndexingStatus>({
  ...
  useEffect(() => {
    ...
    const handleMessage = (
      event: MessageEvent<IndexingStatusUpdateMessage>,
    ) => {
  ...
  const indexingStatus = useMemo(() => {
    if (managedIndexerState.isEnabled) {
      ...
      const hasErrors = managedIndexerState.workspaceFolders.some(
        (folder) => folder.error !== undefined,
      );
      const hasManifests = managedIndexerState.workspaceFolders.some(
        (folder) => folder.hasManifest,
      );
  ...
  const tooltipText = useMemo(() => {
    if (managedIndexerState.isEnabled) {
      ...
      const indexingCount = managedIndexerState.workspaceFolders.filter(
        (f) => f.isIndexing,
      ).length;
      const errorCount = managedIndexerState.workspaceFolders.filter(
        (f) => f.error,
      ).length;
  ...
  const statusColorClass = useMemo(() => {
    ...
    return (
      statusColors[indexingStatus.systemStatus as keyof typeof statusColors] ||
  ...
  const PopoverComponent = managedIndexerState.isEnabled
    ? ManagedCodeIndexPopover
    : CodeIndexPopover; // kade_change
        ...
        <PopoverTrigger asChild>
          <Button
            ...
            className={cn(
              ...
              label &&
                "w-full justify-start px-2 py-1.5 h-auto hover:bg-vscode-list-hoverBackground !opacity-100", // Full mode
