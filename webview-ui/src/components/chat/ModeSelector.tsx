...
export const ModeSelector = ({
  ...
  const { hasOpenedModeSelector, setHasOpenedModeSelector } =
    useExtensionState();
  ...
  const selectedMode = React.useMemo(
    () => modes.find((mode) => mode.slug === value),
    [modes, value],
  );
  ...
  const filteredModes = React.useMemo(() => {
    ...
    const nameMatchedModes = new Set(
      nameMatches.map((result) => result.item.original.slug),
    );
  ...
  return (
    ...
    >
      <StandardTooltip content={title}>
        <PopoverTrigger
          ...
          className={cn(
            ...
            "bg-transparent rounded-md text-vscode-foreground",
      ...
      <PopoverContent
        ...
        className="p-0 overflow-hidden min-w-80 max-w-9/10 border border-vscode-dropdown-border"
      >
