...
const AutoApproveMenu = ({ style }: AutoApproveMenuProps) => {
  ...
  const onAutoApproveToggle = useCallback(
    (key: AutoApproveSetting, value: boolean) => {
      ...
      const willHaveEnabledOptions = Object.values(updatedToggles).some(
        (v) => !!v,
      );
      ...
      // If disabling the last option, disable master auto-approval
        ...
        vscode.postMessage({
          type: "updateSettings",
          updatedSettings: { autoApprovalEnabled: false },
        });
      }
    },
    [
  ...
  const enabledActionsList = Object.entries(toggles)
    ...
    .map(([key]) =>
      t(autoApproveSettingsConfig[key as AutoApproveSetting].labelKey),
    )
    .join(", ");
