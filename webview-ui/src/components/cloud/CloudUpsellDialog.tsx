...
import { ArrowRight, Brain, Cable, CircleDollarSign, FileStack, Router, Users2, Sparkles, CheckCircle2 } from "lucide-react"
...
export const renderCloudBenefitsContent = (t: any) => {
	const benefits = [
		{ icon: Brain, text: t("cloud:cloudBenefitProvider"), color: "text-blue-400" },
		{ icon: Users2, text: t("cloud:cloudBenefitCloudAgents"), color: "text-purple-400" },
		{ icon: Cable, text: t("cloud:cloudBenefitTriggers"), color: "text-emerald-400" },
		{ icon: Router, text: t("cloud:cloudBenefitWalkaway"), color: "text-orange-400" },
		{ icon: CircleDollarSign, text: t("cloud:cloudBenefitMetrics"), color: "text-amber-400" },
		{ icon: FileStack, text: t("cloud:cloudBenefitHistory"), color: "text-indigo-400" },
	]
		...
		<div className="text-left cursor-default space-y-6">
			...
			<div className="grid gap-3 pt-2">
				{benefits.map((benefit, index) => (
					...
					>
						<div className={cn("flex items-center justify-center w-8 h-8 rounded-xl bg-opacity-10", benefit.color.replace("text", "bg"))}>
							<benefit.icon className={cn("size-4 group-hover:scale-110 transition-transform", benefit.color)} />
						</div>
