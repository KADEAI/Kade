  ...
  useAppTranslation: () => ({
    t: (key: string) => {
      const translations: Record<string, string> = {
        ...
        "cloud:cloudBenefitProvider":
          "Access free and paid models that work great with Roo",
        "cloud:cloudBenefitCloudAgents":
          "Give tasks to autonomous Cloud agents",
        "cloud:cloudBenefitTriggers":
          "Get code reviews on Github, start tasks from Slack and more",
        "cloud:cloudBenefitWalkaway":
          "Follow and control tasks from anywhere (including your phone)",
        "cloud:cloudBenefitHistory":
          "Access your task history from anywhere and share them with others",
        "cloud:cloudBenefitMetrics":
          "Get a holistic view of your token consumption",
        "cloud:logOut": "Log out",
        "cloud:connect": "Get started",
        "cloud:visitCloudWebsite": "Visit Roo Code Cloud",
        "cloud:taskSync": "Task sync",
        "cloud:taskSyncDescription":
          "Sync your tasks for viewing and sharing on Roo Code Cloud",
        "cloud:taskSyncManagedByOrganization":
          "Task sync is managed by your organization",
        ...
        "cloud:remoteControlRequiresTaskSync":
          "Task sync must be enabled to use Roomote Control",
        "cloud:usageMetricsAlwaysReported":
          "Model usage info is always reported when logged in",
...
describe("CloudView", () => {
  it("should display benefits when user is not authenticated", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Try Roo Code Cloud" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Access free and paid models that work great with Roo"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Give tasks to autonomous Cloud agents"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        "Get code reviews on Github, start tasks from Slack and more",
    ...
    expect(
      screen.getByText("Get a holistic view of your token consumption"),
    ).toBeInTheDocument();
  ...
  it("should not display benefits when user is authenticated", () => {
    ...
    expect(
      screen.queryByText("Give tasks to autonomous Cloud agents"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(
        "Get code reviews on Github, start tasks from Slack and more",
    ...
    expect(
      screen.queryByText("Get a holistic view of your token consumption"),
    ).not.toBeInTheDocument();
  ...
  it("should not display remote control toggle when user does not have extension bridge enabled", () => {
    ...
    expect(
      screen.queryByTestId("remote-control-toggle"),
    ).not.toBeInTheDocument();
  ...
  it("should not display remote control toggle when roomote control is disabled", () => {
    ...
    const originalFeatureRoomoteControlEnabled =
      mockExtensionState.featureRoomoteControlEnabled;
    ...
    mockExtensionState.featureRoomoteControlEnabled =
      originalFeatureRoomoteControlEnabled;
  });
  ...
  it("should disable task sync toggle for organization users", () => {
    ...
    const lockIcon = screen
      ...
      .parentElement?.querySelector(".lucide-lock");
  ...
  it("should enable task sync toggle for non-organization users", () => {
    ...
    expect(
      screen.queryByText("Task sync is managed by your organization"),
    ).not.toBeInTheDocument();
  });
