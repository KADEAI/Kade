...
export const BatchDeleteTaskDialog = ({
  ...
  const favoritedTasks =
    tasks?.filter((task) => taskIds.includes(task.id) && task.isFavorited) ??
    [];
  ...
  return (
    <AlertDialog {...props}>
      <AlertDialogContent
        ...
        className="bg-[#1e1e1e] border-white/5 p-6 max-w-[320px] rounded-[1.5rem] shadow-none ring-1 ring-white/10"
      >
            ...
            <AlertDialogDescription className="text-vscode-descriptionForeground text-xs leading-relaxed space-y-3 px-2 text-center">
              ...
              {hasFavoritedTasks && (
                <div className="flex items-center justify-center gap-2 p-2 rounded-xl bg-yellow-500/5 text-yellow-500/80 text-[10px] italic">
                  ...
                  {t("history:deleteTasksFavoritedWarning", {
