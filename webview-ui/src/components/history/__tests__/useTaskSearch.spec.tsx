...
function kiloCodeSetUpUseTaskHistoryMock(
  taskHistory: Partial<HistoryItem>[],
  cwd: string,
) {
...
describe("useTaskSearch", () => {
  ...
  it("filters tasks by current workspace by default", () => {
    ...
    expect(
      result.current.tasks.every(
        (task) => task.workspace === "/workspace/project1",
      ),
    ).toBe(true);
  });
  ...
  it("filters tasks by search query", () => {
    ...
    expect((result.current.tasks[0] as any).highlight).toBe(
      "<mark>Create a React component</mark>",
    );
  });
  ...
  it("filters out tasks without timestamp or task content", () => {
    ...
    expect(result.current.tasks).toHaveLength(3);
    expect(result.current.tasks.every((task) => task.ts && task.task)).toBe(
      true,
    );
  });
