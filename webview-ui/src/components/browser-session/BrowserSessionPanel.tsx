...
} from "./BrowserPanelStateProvider";
...
const BrowserSessionPanelContent: React.FC = () => {
  const { browserViewportSize, isBrowserSessionActive } =
    useBrowserPanelState();
  const [state, setState] = useState<BrowserSessionPanelState>({
    messages: [],
  });
  // Target page index to navigate BrowserSessionRow to
  const [navigateToStepIndex, setNavigateToStepIndex] = useState<
    number | undefined
  >(undefined);
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      switch (message.type) {
        case "browserSessionUpdate":
          ...
          break;
        case "browserLiveScreenshot":
          if (message.screenshot) {
            setLiveScreenshot(message.screenshot);
          }
          break;
