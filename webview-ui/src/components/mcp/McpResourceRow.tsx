		...
		<div
			...
			{item.description || item.name ? (
				<div className="text-[11px] text-vscode-descriptionForeground leading-relaxed px-1">
					{item.name && (
						<span className="font-bold text-vscode-foreground mr-1.5 opacity-90">{item.name}</span>
					)}
					<span className="opacity-80">{item.description || "No description"}</span>
				</div>
			) : (
				<div className="text-[11px] text-vscode-descriptionForeground/40 italic px-1">No description</div>
			)}
			<div className="flex items-center gap-2 px-1">
				<span className="text-[10px] font-black uppercase tracking-widest opacity-30">Returns</span>
