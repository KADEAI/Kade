...
const McpView = ({ onDone, hideHeader = false }: McpViewProps) => {
	...
	const content = (
		<div className="flex flex-col h-full overflow-hidden bg-vscode-sideBar-background">
			{!hideHeader && (
				<TabHeader className="flex justify-between items-center px-4 py-3 border-b border-vscode-panel-border bg-vscode-sideBar-background shrink-0">
					<h3 className="text-vscode-foreground font-bold m-0 text-base">{t("mcp:title")}</h3>
								...
								<Trans i18nKey="mcp:description">
									<a
										...
										onClick={() => vscode.postMessage({
											type: "openExternal",
											url: buildDocLink("features/mcp/using-mcp-in-kilo-code", "mcp_settings")
										})}
									>
								...
								<div className="flex flex-col items-center justify-center py-12 px-6 border border-dashed border-vscode-panel-border rounded-xl opacity-60 text-center space-y-3 bg-vscode-editor-background/30">
									...
									<div className="space-y-1">
										...
										<p className="text-xs text-vscode-descriptionForeground max-w-[200px]">
											Install servers from the marketplace or add your own local servers.
										</p>
									</div>
								</div>
							)}
							...
							<div className="space-y-4 pt-4 border-t border-vscode-panel-border/50">
								...
								<div className="flex flex-wrap gap-2">
									...
									<Button
										...
										size="sm"
										className="flex items-center gap-2 h-9 px-4 shrink-0 transition-all hover:bg-vscode-button-secondaryHover"
										onClick={() => vscode.postMessage({ type: "refreshAllMcpServers" })}>
	...
	return hideHeader ? content : <Tab className="relative overflow-hidden">{content}</Tab>
}
...
const ServerRow = ({ server, alwaysAllowMcp }: { server: McpServer; alwaysAllowMcp?: boolean }) => {
	...
	const isExpandable = (server.status === "connected" || server.status === "connecting") && !server.disabled
		...
		<div
			...
			<div
				...
				<div className="flex items-center gap-3 min-w-0 flex-1 h-full">
					<div className={cn("shrink-0 relative flex items-center justify-center")}>
						<div className={cn("w-2.5 h-2.5 rounded-full z-10 relative", getStatusColor())} />
				...
				<div className="flex items-center gap-1.5 shrink-0 ml-auto h-full">
					<div className="hidden group-hover:flex items-center gap-1 h-full" onClick={handleToggle}>
								...
								<div
									...
									onClick={(e) => {
										...
										vscode.postMessage({
											...
											serverName: server.name,
											source: server.source || "global",
											alwaysAllowAll: !(server.tools?.every((t) => t.alwaysAllow) ?? false),
										})
									}}>
									...
									<div
										role="switch"
										aria-checked={server.tools?.every((t) => t.alwaysAllow) ?? false}
										className="relative transition-all duration-200"
										style={{
											width: "24px",
											height: "12px",
											backgroundColor:
												server.tools?.every((t) => t.alwaysAllow)
													? "#f59e0b"
													: "rgba(245, 158, 11, 0.3)",
											borderRadius: "6px",
											cursor: "pointer",
										...
										<div
											style={{
												...
												position: "absolute",
												top: "2px",
												left: server.tools?.every((t) => t.alwaysAllow) ? "14px" : "2px",
				...
				<div className="animate-in slide-in-from-top-2 duration-200 bg-vscode-sideBar-background">
					<div className="px-3 pb-4">
						<VSCodePanels className="mcp-panels">
							<VSCodePanelTab id="tools" className="text-[10px] uppercase tracking-wider font-bold h-8">
								Tools ({server.tools?.length || 0})
							</VSCodePanelTab>
							<VSCodePanelTab id="resources" className="text-[10px] uppercase tracking-wider font-bold h-8">
								Resources (
								{[...(server.resourceTemplates || []), ...(server.resources || [])].length ||
									0}
								)
							</VSCodePanelTab>
							{server.instructions && (
							...
							<VSCodePanelTab id="logs" className="text-[10px] uppercase tracking-wider font-bold h-8">
