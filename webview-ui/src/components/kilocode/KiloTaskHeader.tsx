...
const KiloTaskHeader = ({
  ...
  const { apiConfiguration, currentTaskItem, customModes } =
    useExtensionState();
  const { id: modelId, info: model } = useSelectedModel(apiConfiguration);
  const [isTaskExpanded, setIsTaskExpanded] = useState(defaultExpanded);

  useEffect(() => {
    setIsTaskExpanded(defaultExpanded);
  }, [defaultExpanded]);
  ...
  const condenseButton = (
    <StandardTooltip content={t("chat:task.condenseContext")}>
      <button
        ...
        onClick={() =>
          currentTaskItem && handleCondenseContext(currentTaskItem.id)
        }
  ...
  return (
    <div className="w-full">
      <div
        className={cn(
          "p-2.5 flex flex-col relative z-1",
          hasTodos ? "rounded-t-xs" : "rounded-xs",
          isTaskExpanded
            ? "text-vscode-foreground"
            : "text-vscode-foreground/80",
        )}
      >
...
export const highlightText = (
  ...
  const resultWithSlashHighlighting = highlightSlashCommands(
    text,
    withShadow,
    customModes,
  );
  ...
  if (
    Array.isArray(resultWithSlashHighlighting) &&
    resultWithSlashHighlighting.length === 3
  ) {
    const [beforeCommand, commandElement, afterCommand] =
      resultWithSlashHighlighting as [string, JSX.Element, string];
