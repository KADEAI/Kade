...
export const KilocodeNotifications: React.FC = () => {
  const { dismissedNotificationIds, currentApiConfigName, apiConfiguration } =
    useExtensionState();
  const { provider, providerModels, isLoading, isError } =
  ...
  useEffect(() => {
    ...
    setCurrentIndex(
      Math.max(0, Math.min(currentIndex, filteredNotifications.length - 1)),
    );
  ...
  const goToPrevious = () => {
    setCurrentIndex(
      (prev) =>
        (prev - 1 + filteredNotifications.length) %
  ...
  return (
    <div className="kilocode-notifications flex flex-col mb-4 w-full">
      <div className="bg-vscode-editor-background/50 backdrop-blur-sm border border-vscode-panel-border rounded-xl p-4 gap-3 shadow-sm">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold text-vscode-foreground m-0 tracking-tight">
            {currentNotification.title}
          </h3>
          <button
            onClick={() => dismissNotificationId(currentNotification.id)}
            className="text-vscode-descriptionForeground hover:text-vscode-foreground p-1 transition-colors"
