...
export const KiloCode = ({
  ...
  const handleToggleTesterWarnings = useCallback(() => {
    const newTimestamp =
      Date.now() + (areKilocodeWarningsDisabled ? 0 : 24 * 60 * 60 * 1000);
    setApiConfigurationField(
    ...
    <>
      <div>
        <label className="block font-medium -mb-2">
          {t("kilocode:settings.provider.account")}
        </label>
      </div>
      {!hideKiloCodeButton &&
        (apiConfiguration.kilocodeToken ? (
      ...
      >
        <div className="flex justify-between items-center mb-1">
          <label className="block font-medium">
            {t("kilocode:settings.provider.apiKey")}
          </label>
        </div>
      </VSCodeTextField>
      ...
      {isKiloCodeAiUser && (
        <div className="mb-4">
          <label className="block font-medium mb-2">
            Disable KILOCODE-TESTER warnings
          </label>
          ...
          <Button
            variant="secondary"
            onClick={handleToggleTesterWarnings}
            className="text-sm"
          >
            {areKilocodeWarningsDisabled
              ? "Enable warnings now"
              : "Disable warnings for 1 day"}
          </Button>
        </div>
      )}
    </>
