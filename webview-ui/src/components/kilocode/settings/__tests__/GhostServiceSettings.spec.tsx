...
describe("GhostServiceSettingsView", () => {
  ...
  it("renders basic trigger settings", () => {
    ...
    expect(
      screen.getByText(/kilocode:ghost.settings.triggers/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/kilocode:ghost.settings.enableAutoTrigger.label/),
    ).toBeInTheDocument();
  });
  ...
  it("toggles auto trigger checkbox correctly", () => {
    ...
    expect(onGhostServiceSettingsChange).toHaveBeenCalledWith(
      "enableAutoTrigger",
      true,
    );
  });
  ...
  it("toggles quick inline task keybinding checkbox correctly", () => {
    ...
    const checkboxLabel = screen
      .getByText(
        /kilocode:ghost.settings.enableQuickInlineTaskKeybinding.label/,
      )
      .closest("label");
    const checkbox = checkboxLabel?.querySelector(
      'input[type="checkbox"]',
  ...
  it("toggles smart inline task keybinding checkbox correctly", () => {
    ...
    const checkboxLabel = screen
      .getByText(
        /kilocode:ghost.settings.enableSmartInlineTaskKeybinding.label/,
      )
      .closest("label");
    const checkbox = checkboxLabel?.querySelector(
      'input[type="checkbox"]',
  ...
  it("toggles chat autocomplete checkbox correctly", () => {
    ...
    const checkboxLabel = screen
      .getByText(/kilocode:ghost.settings.enableChatAutocomplete.label/)
      .closest("label");
    const checkbox = checkboxLabel?.querySelector(
      'input[type="checkbox"]',
    ) as HTMLInputElement;
  ...
  it("displays provider and model information when available", () => {
    ...
    expect(
      screen.getByText(/kilocode:ghost.settings.provider/),
    ).toBeInTheDocument();
    expect(screen.getByText(/openrouter/)).toBeInTheDocument();
    expect(
      screen.getAllByText(/kilocode:ghost.settings.model/).length,
    ).toBeGreaterThan(0);
  ...
  it("displays error message when provider and model are not configured", () => {
    ...
    expect(
      screen.getByText(/kilocode:ghost.settings.noModelConfigured/),
    ).toBeInTheDocument();
  });
  ...
  describe("snooze status refresh", () => {
    it("updates snooze status when timer fires and snooze expires", () => {
      ...
      expect(
        screen.getByText(/kilocode:ghost.settings.snooze.currentlySnoozed/),
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByText(/kilocode:ghost.settings.snooze.button/),
      ).toBeInTheDocument();
      expect(
        screen.queryByText(/kilocode:ghost.settings.snooze.currentlySnoozed/),
