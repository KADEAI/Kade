...
} from "../../../../src/shared/kilocode/wrapper";
import { getAppUrl } from "@roo-code/types";

const getJetbrainsUrlScheme = (code: string) => {
  return (
    JETBRAIN_PRODUCTS[code as keyof typeof JETBRAIN_PRODUCTS]?.urlScheme ||
    "jetbrains"
  );
};

const getKiloCodeSource = (
...
export function getKiloCodeBackendSignInUrl(
  ...
  const source =
    uiKind === "Web"
      ? "web"
      : getKiloCodeSource(uriScheme, kiloCodeWrapperProperties);
