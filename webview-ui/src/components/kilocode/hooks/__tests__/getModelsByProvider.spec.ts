...
import { opencodeDefaultModelId } from "@roo-code/types";
...
describe("getModelsByProvider", () => {
  ...
  const routerModels: RouterModels = {
    ...
    opencode: { "test-model": testModel },
    "cli-proxy": { "test-model": testModel },
    roo: { "test-model": testModel },
    antigravity: { "test-model": testModel },
    kiro: {},
    corethink: {},
    apertis: {},
    poe: {},
  ...
  it("returns models for all providers", () => {
    ...
    const providersWithoutModels = providerNames
      ...
      .filter(
        (provider) =>
          exceptions.indexOf(provider[0]) < 0 &&
          Object.values(provider[1].models).length === 0,
      )
  ...
  it("returns Bluesminds router models", () => {
    const result = getModelsByProvider({
      provider: "bluesminds",
      routerModels: {
        ...routerModels,
        bluesminds: { "deepseek-chat": testModel },
      },
      kilocodeDefaultModel: "test-default-model",
      options: {},
    });

    expect(result.models).toEqual({ "deepseek-chat": testModel });
    expect(result.defaultModel).toEqual("deepseek-chat");
  });

  it("returns OpenCode router models", () => {
    const result = getModelsByProvider({
      provider: "opencode",
      routerModels: {
        ...routerModels,
        opencode: { "gpt-5.4": testModel },
      },
      kilocodeDefaultModel: "test-default-model",
      options: {},
    });

    expect(result.models).toEqual({ "gpt-5.4": testModel });
    expect(result.defaultModel).toEqual(opencodeDefaultModelId);
  });
});
...
describe("getOptionsForProvider", () => {
  ...
  it("returns isChina: false for zai provider with default apiConfiguration", () => {
    const result = getOptionsForProvider("zai", {
