...
function warningText(innerMessage?: InnerMessage) {
  ...
  if (isAlphaPeriodEndedError(innerMessage?.error)) {
    return i18next.t("kilocode:invalidModel.alphaPeriodEnded", {
      model: unavailableModel,
    });
  }
  if (isModelNotAllowedForTeamError(innerMessage?.error)) {
    return i18next.t("kilocode:invalidModel.notAllowedForTeam", {
      model: unavailableModel,
    });
  }
  return i18next.t("kilocode:invalidModel.modelUnavailable", {
    model: unavailableModel,
  });
}

export const InvalidModelWarning = ({
  message,
  isLast,
}: {
  ...
  const canChangeToDefaultModel =
    ...
    defaultModelId in providerModels;
          ...
          >
            {canChangeToDefaultModel
              ? t("kilocode:invalidModel.continueWith", {
                  model:
                    providerModels[defaultModelId]?.displayName ??
                    defaultModelId,
                })
              : t("kilocode:invalidModel.continue")}
          </VSCodeButton>
          {didAlphaPeriodEnd && (
            <FreeModelsLink className="w-full" origin="invalid_model" />
          )}
        </>
      )}
    </div>
