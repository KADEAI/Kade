...
export function useStateManager(existingManager?: MarketplaceViewStateManager) {
  const [manager] = useState(
    () => existingManager || new MarketplaceViewStateManager(),
  );
    ...
    const handleStateChange = (newState: ViewState) => {
      setState((prevState) => {
        ...
        const hasChanged =
          ...
          JSON.stringify(prevState.allItems) !==
            JSON.stringify(newState.allItems) ||
          JSON.stringify(prevState.organizationMcps) !==
            JSON.stringify(newState.organizationMcps) ||
          JSON.stringify(prevState.displayItems) !==
            JSON.stringify(newState.displayItems) ||
