...
export function MarketplaceListView({
  ...
  const items = filterByType
    ? allItems.filter((item) => item.type === filterByType)
    : allItems;
  ...
  return (
    <>
      ...
      <div className="mb-6 space-y-3">
        <div className="relative flex flex-col gap-2 min-[720px]:flex-row">
          ...
          <Button
            size="sm"
            variant="primary"
            className="h-10 px-4 min-[720px]:px-5 rounded-xl font-bold shadow-lg shadow-primary/10 hover:shadow-primary/20 transition-all active:scale-95 min-[720px]:self-auto self-stretch"
          ...
          >
            ...
            <SelectContent className="rounded-xl border-vscode-panel-border shadow-xl">
              <SelectItem value="all" className="rounded-lg">
                {t("marketplace:filters.installed.all")}
              </SelectItem>
              <SelectItem value="installed" className="rounded-lg">
                {t("marketplace:filters.installed.installed")}
              </SelectItem>
              ...
              {(orgMcps.length > 0 || !isSearchActive) && (
                <div className="flex items-center gap-2 mb-3 px-1">
                  <span
                    className={cn(
                      "codicon text-lg",
                      isSearchActive ? "codicon-globe" : "codicon-star-full",
                    )}
                  ></span>
                  <h3 className="text-sm font-semibold text-vscode-foreground">
                    {isSearchActive
                      ? t("marketplace:sections.marketplace")
                      : t("marketplace:sections.featured")}
                  </h3>
                  <div className="flex-1 h-px bg-vscode-input-border"></div>
                </div>
              )}
