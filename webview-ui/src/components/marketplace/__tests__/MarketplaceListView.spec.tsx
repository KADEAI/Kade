...
describe("MarketplaceListView", () => {
  ...
  it("renders search input", () => {
    ...
    const searchInput = screen.getByPlaceholderText(
      "marketplace:filters.search.placeholder",
    );
  ...
  it("does not render type filter (removed in simplified interface)", () => {
    ...
    expect(
      screen.queryByText("marketplace:filters.type.label"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("marketplace:filters.type.all"),
    ).not.toBeInTheDocument();
  });
  ...
  it("does not render sort options (removed in simplified interface)", () => {
    ...
    expect(
      screen.queryByText("marketplace:filters.sort.label"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("marketplace:filters.sort.name"),
    ).not.toBeInTheDocument();
  });
  ...
  it("renders tags section when tags are available", () => {
    ...
    expect(
      screen.getByText("marketplace:filters.tags.label"),
    ).toBeInTheDocument();
  });
  ...
  it("shows loading state when fetching", () => {
    ...
    expect(
      screen.getByText("marketplace:items.refresh.refreshing"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("marketplace:items.refresh.mayTakeMoment"),
    ).toBeInTheDocument();
  });
  ...
  it("shows empty state when no items and not fetching", () => {
    ...
    expect(
      screen.getByText("marketplace:items.empty.noItems"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("marketplace:items.empty.adjustFilters"),
    ).toBeInTheDocument();
  });
