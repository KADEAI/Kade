...
export interface ViewState {
  ...
  isFetching: boolean;
  activeTab: "mcp" | "mode" | "installed" | "skills";
  filters: {
    type: string;
    search: string;
    tags: string[];
...
export class MarketplaceViewStateManager {
  ...
  public getState(): ViewState {
    ...
    const organizationMcps = this.state.organizationMcps.length
    ...
    const displayOrganizationMcps = this.state.displayOrganizationMcps
      ...
      : [...organizationMcps];
    const tags = this.state.filters.tags.length
  ...
  public async transition(transition: ViewStateTransition): Promise<void> {
    switch (transition.type) {
      ...
      case "FETCH_COMPLETE": {
        const { items } =
          transition.payload as TransitionPayloads["FETCH_COMPLETE"];
        ...
        if (this.isFilterActive()) {
          newDisplayItems = this.filterItems(
            [...items],
      ...
      case "SET_ACTIVE_TAB": {
        const { tab } =
          transition.payload as TransitionPayloads["SET_ACTIVE_TAB"];
      ...
      case "UPDATE_FILTERS": {
        const { filters = {} } =
          (transition.payload as TransitionPayloads["UPDATE_FILTERS"]) || {};

        // Create new filters object preserving existing values for undefined fields
        const updatedFilters = {
          type:
            filters.type !== undefined ? filters.type : this.state.filters.type,
          search:
            filters.search !== undefined
              ? filters.search
              : this.state.filters.search,
          tags:
            filters.tags !== undefined ? filters.tags : this.state.filters.tags,
          installed:
        ...
        this.state = {
          ...
          isFetching:
            filters.search !== undefined ? true : this.state.isFetching,
        };

        // Apply filters to displayItems and displayOrganizationMcps with the updated filters
        const newDisplayItems = this.filterItems(
          this.state.allItems,
          this.state.installedMetadata,
        );
  ...
  public async handleMessage(message: any): Promise<void> {
    ...
    if (message.type === "state") {
      ...
      const marketplaceInstalledMetadata =
        message.state.marketplaceInstalledMetadata;
      ...
      if (marketplaceItems !== undefined) {
        ...
        this.state = {
          ...
          displayOrganizationMcps: newDisplayOrganizationMcps,
          installedMetadata:
            marketplaceInstalledMetadata || this.state.installedMetadata,
        };
      ...
      if (message.text) {
        ...
        if (
          ...
          (message.values.marketplaceTab === "mcp" ||
            ...
            message.values.marketplaceTab === "installed")
        ) {
          // Set the active tab
