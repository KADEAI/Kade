...
export function MarketplaceView({ stateManager, onDone, targetTab, hideHeader = false }: MarketplaceViewProps) {
	...
	useEffect(() => {
		if (targetTab && (targetTab === "mcp" || targetTab === "mode" || targetTab === "installed" || targetTab === "skills")) {
	...
	const modesHeaderMessage = useMemo(
		() => ({
			...
			onLinkClick: () => {
				vscode.postMessage({ type: "switchTab", tab: "settings", values: { section: "modes" } })
			},
		}),
		[],
	)
	...
	<div className="flex relative py-1">
		...
		<button
			className="flex items-center justify-center gap-2 flex-1 text-sm font-medium rounded-sm transition-all duration-200 relative z-10 text-vscode-foreground hover:bg-vscode-toolbar-hoverBackground active:scale-95 py-1"
			onClick={() => manager.transition({ type: "SET_ACTIVE_TAB", payload: { tab: "mode" } })}>
			Modes
		</button>
		<button
			className="flex items-center justify-center gap-2 flex-1 text-sm font-medium rounded-sm transition-all duration-200 relative z-10 text-vscode-foreground hover:bg-vscode-toolbar-hoverBackground active:scale-95 py-1"
		...
		<button
			...
			Installed
				...
				<TabContent className="p-3 pt-2">
					...
					{state.activeTab === "skills" && <SkillsView onDone={onDone} hideHeader={true} />}
				</TabContent>
			</Tab>
		</TooltipProvider>
	)
