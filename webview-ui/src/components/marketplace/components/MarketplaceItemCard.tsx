...
export const MarketplaceItemCard: React.FC<MarketplaceItemCardProps> = ({
  ...
  const [installModalVersion, setInstallModalVersion] = useState(0); // kade_change
  const [showRemoveConfirm, setShowRemoveConfirm] = useState(false);
  const [removeTarget, setRemoveTarget] = useState<"project" | "global">(
    "project",
  );
  ...
  const handleInstallClick = () => {
    ...
    setInstallModalVersion((prev) => prev + 1); // kade_change
  ...
  return (
    <>
        ...
        <div className="flex gap-3 items-start justify-between relative z-10">
          <div className="flex gap-3 items-start flex-1 min-w-0">
            ...
            <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-vscode-sideBar-background flex items-center justify-center border border-vscode-panel-border group-hover:border-vscode-focusBorder/30 transition-colors">
              <span
                className={cn(
                  "codicon text-2xl opacity-80 group-hover:opacity-100 transition-opacity",
                  displayIconClass,
                )}
              />
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-2 items-center">
                ...
                {(item as any).installCount !== undefined && (
                  <>
                    <span className="text-vscode-descriptionForeground/40 text-[10px]">
                      •
                    </span>
              ...
              item.tags.map((tag) => (
                ...
                >
                  <Button
                    ...
                    variant="secondary"
                    className={cn(
                      "rounded-full capitalize text-[10px] px-2.5 h-5 bg-vscode-badge-background/30 hover:bg-vscode-badge-background/50 border-none",
                      {
      ...
      <MarketplaceInstallModal
        key={
          `install-modal-${item.id}-${installModalVersion}` /* kade_change */
        }
        item={item}
