...
export const MarketplaceInstallModal: React.FC<
  MarketplaceInstallModalProps
> = ({ item, isOpen, onClose, hasWorkspace }) => {
  const { t } = useAppTranslation();
  const [scope, setScope] = useState<"project" | "global">(
    hasWorkspace ? "project" : "global",
  );
  ...
  const methodNames = useMemo(() => {
    ...
    return (item.content as Array<{ name: string; content: string }>).map(
      (method) => method.name,
    );
  }, [item]);
  ...
  const effectiveParameters = useMemo(() => {
    ...
    if (Array.isArray(item.content)) {
      const selectedMethod = item.content[
        selectedMethodIndex
      ] as McpInstallationMethod;
            ...
            {hasMultipleMethods && (
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setSelectedMethodIndex(Number(value))
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
        ...
        <DialogFooter>
          {installationComplete ? (
            <>
              ...
              <Button
                // kade_change: only display when item type is modes
                style={item.type === "mcp" ? { display: "none" } : undefined}
                onClick={() =>
                  handlePostInstallAction(item.type === "mcp" ? "mcp" : "modes")
                }
              >
                {item.type === "mcp"
