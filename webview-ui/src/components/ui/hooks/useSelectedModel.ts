...
function getValidatedModelId(
  ...
  return configuredId && availableModels?.[configuredId]
    ? configuredId
    : defaultModelId;
}
...
export const useSelectedModel = (apiConfiguration?: ProviderSettings) => {
  ...
  // kade_change start
  const { kilocodeDefaultModel, virtualQuotaActiveModel, zedAuthenticated } =
    useExtensionState();
  const lmStudioModelId =
    provider === "lmstudio" ? apiConfiguration?.lmStudioModelId : undefined;
  const ollamaModelId =
    provider === "ollama" ? apiConfiguration?.ollamaModelId : undefined;
  // kade_change end

  // Only fetch router models for dynamic providers
  const shouldFetchRouterModels =
    isDynamicProvider(provider) && provider !== "antigravity";
  ...
  const openRouterModelProviders = useModelProviders(
  ...
  const { id, info } =
    apiConfiguration && isReady
      ? getSelectedModel({
          ...
          lmStudioModels: (lmStudioModels.data || undefined) as
            | ModelRecord
            | undefined,
          kilocodeDefaultModel,
          ollamaModels: (ollamaModels.data || undefined) as
            | ModelRecord
            | undefined,
...
function getSelectedModel({
  ...
  switch (provider) {
    ...
    case "bluesminds": {
      const id = getValidatedModelId(
        apiConfiguration.apiModelId,
        routerModels.bluesminds,
        bluesmindsDefaultModelId,
      );
      const info = routerModels.bluesminds?.[id];
      return { id, info };
    }
    // kade_change end
    case "unbound": {
    ...
    case "huggingface": {
      const id =
        apiConfiguration.huggingFaceModelId ??
    ...
    case "zed": {
      ...
      const info = routerModels.zed?.[id];
      return { id, info };
    }
    case "kiro": {
      const id = apiConfiguration.apiModelId ?? defaultModelId;
      const info = routerModels.kiro?.[id];
      return { id, info };
    }
    case "mistral": {
      const id = apiConfiguration.apiModelId ?? defaultModelId;
    ...
    case "ollama": {
      ...
      const info =
        ollamaModels && ollamaModels[apiConfiguration.ollamaModelId!];
      ...
      if (info) {
        adjustedInfo = userContextWindow
          ? { ...info, contextWindow: userContextWindow }
          : info;
    ...
    case "lmstudio": {
      ...
      const info =
        lmStudioModels && lmStudioModels[apiConfiguration.lmStudioModelId!];
      return {
        id,
        info: info || undefined,
      };
    ...
    case "deepinfra": {
      const id = getValidatedModelId(
        apiConfiguration.deepInfraModelId,
        routerModels.deepinfra,
        defaultModelId,
      );
    ...
    case "kilocode": {
      ...
      if (routerModels["kilocode"] && apiConfiguration.kilocodeModel) {
        ...
        const selectedModel =
          modelEntries.find(
            (model) => model[0].toLowerCase() === selectedModelId,
          ) ??
          modelEntries.find((model) =>
            model[0].toLowerCase().includes(selectedModelId),
          );
    ...
    case "io-intelligence": {
      ...
      const info =
        routerModels["io-intelligence"]?.[id] ??
        ioIntelligenceModels[id as keyof typeof ioIntelligenceModels];
      return { id, info };
    }
    case "roo": {
      const id = getValidatedModelId(
    ...
    case "ovhcloud": {
      const id =
        apiConfiguration.ovhCloudAiEndpointsModelId ??
        ovhCloudAiEndpointsDefaultModelId;
    ...
    case "inception": {
      const id =
        apiConfiguration.inceptionLabsModelId ?? inceptionDefaultModelId;
    ...
    case "opencode": {
      const id = getValidatedModelId(
        apiConfiguration.opencodeModelId,
        routerModels.opencode,
        defaultModelId,
      );
      ...
      if (
        ...
        if (tier) {
          ...
          const info: ModelInfo = {
            ...
            cacheWritesPrice:
              tier.cacheWritesPrice ?? baseInfo.cacheWritesPrice,
