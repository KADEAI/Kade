...
const mockTranslate = (key: string, options?: Record<string, any>): string => {
  ...
  if (key.startsWith("settings.") || key.startsWith("settings:")) {
    ...
    if (key === "settings.notifications.sound.label")
      return "Enable sound effects";
    if (key === "settings.autoApprove.execute.label")
      return "Always approve allowed execute operations";
    if (key === "settings.autoApprove.execute.allowedCommands")
      return "Allowed Auto-Execute Commands";
    if (key === "settings.autoApprove.execute.commandPlaceholder")
      return "Enter command prefix";
    if (key === "settings.autoApprove.execute.addButton") return "Add";
    if (key === "settings.common.save") return "Save";
    if (key === "settings.contextManagement.terminal.label")
      return "Terminal output limit";
    if (key === "settings.header.title") return "Settings";
    if (key === "settings:providers.zed.signInButton")
      return "Sign in with Zed";
    if (key === "settings:providers.zed.signOutButton") return "Sign Out";
