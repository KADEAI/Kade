...
export class MermaidSyntaxFixer {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : i18next.t("common:mermaid.errors.unknown_syntax"),
      };
    }
  }
  ...
  private static requestLLMFix(
    ...
    return new Promise((resolve, _reject) => {
      ...
      const timeout = setTimeout(() => {
        cleanup();
        resolve({
          requestError: i18next.t("common:mermaid.errors.fix_timeout"),
        });
      }, this.FIX_TIMEOUT);
      ...
      const messageListener = (event: MessageEvent) => {
        ...
        if (
          message.type === "mermaidFixResponse" &&
          ...
          if (message.success) {
            ...
            resolve({
              requestError:
                message.error ||
                i18next.t("common:mermaid.errors.fix_request_failed"),
            });
          }
        }
      };
  ...
  static async autoFixSyntax(code: string): Promise<MermaidFixResult> {
    ...
    while (true) {
      ...
      const lastError =
        validation.error || i18next.t("common:mermaid.errors.unknown_syntax");
