...
vi.mock("i18next", () => ({
  default: {
    t: (key: string, options?: any) => {
      ...
      if (key === "common:mermaid.errors.unknown_syntax")
        return "Unknown syntax error";
      if (key === "common:mermaid.errors.fix_timeout") return "Fix timeout";
      if (key === "common:mermaid.errors.fix_request_failed")
        return "Fix request failed";
      if (key === "common:mermaid.errors.fix_attempts")
        return `Failed to fix syntax after ${options?.attempts} attempts: ${options?.error}`;
      if (key === "common:mermaid.errors.no_fix_provided")
        return "LLM failed to provide a fix";
      return key;
    },
  },
}));
  ...
  describe("applyDeterministicFixes", () => {
    ...
    describe("autoFixSyntax", () => {
      ...
      it("should return success and fixed code when LLM validation succeeds", async () => {
        const applyDeterministicFixesSpy = vi.spyOn(
          MermaidSyntaxFixer,
          "applyDeterministicFixes",
        );
        applyDeterministicFixesSpy.mockReturnValueOnce("original code"); // First call
        applyDeterministicFixesSpy.mockReturnValueOnce(
          "deterministically fixed code",
        ); // Second call after LLM fix

        validateSyntaxSpy.mockResolvedValueOnce({
          isValid: false,
          error: "error",
      ...
      it("should return the best attempt even when fix is not successful", async () => {
        ...
        applyDeterministicFixesSpy.mockReturnValueOnce(
          "deterministically fixed first attempt",
        );
        applyDeterministicFixesSpy.mockReturnValueOnce(
          "deterministically fixed second attempt",
        );
        ...
        expect(applyDeterministicFixesSpy).toHaveBeenNthCalledWith(
          2,
          "first attempt",
        );
        expect(applyDeterministicFixesSpy).toHaveBeenNthCalledWith(
      ...
      it("should return the best attempt when LLM request fails", async () => {
        ...
        requestLLMFixSpy.mockResolvedValueOnce({ fixedCode: "first attempt" });
        requestLLMFixSpy.mockResolvedValueOnce({
          requestError: "LLM request failed",
        });
