...
export class LocalWhisperClient {
    ...
    try {
      ...
      this.worker.onmessage = (e) => {
        ...
        if (event.status === "ready") {
          ...
          console.log(
            "[LocalWhisperClient] Model ready, processing",
  ...
  public async startRecording(deviceId?: string): Promise<void> {
    ...
    try {
      ...
      const constraints = deviceId
        ...
        : { audio: true };
      console.log(
        "[LocalWhisperClient] Requesting microphone access with constraints:",
        constraints,
      );
      ...
      this.mediaRecorder.onstop = async () => {
        try {
          ...
          if (!f32 || f32.length === 0) {
            this.onStatusChange({
              status: "error",
              data: "Audio processing failed (empty result)",
            });
            return;
          }
  ...
  public transcribeAudio(audio: Float32Array): void {
    ...
    if (!this.modelReady) {
      ...
      console.log(
        `[LocalWhisperClient] Model not ready, queuing ${audio.length} samples (${this.pendingAudio.length} chunks queued)`,
      );
  ...
  private flushPendingAudio(): void {
    ...
    const totalLength = this.pendingAudio.reduce(
    ...
    console.log(
      `[LocalWhisperClient] Flushing ${combined.length} queued samples for transcription`,
    );
    this.worker.postMessage({
      type: "generate",
