  ...
  return {
    ...
    closeBundle: {
      ...
      handler: async () => {
        ...
        console.log(
          `Source map processing for ${mode} build in ${resolvedOutDir}`,
        );

        // Check if build directory exists
        if (!fs.existsSync(resolvedOutDir)) {
          console.warn("Build directory not found:", resolvedOutDir);
          return;
        }
          ...
          if (fs.existsSync(mapPath)) {
            ...
            try {
              ...
              if (mapContent.sources) {
                mapContent.sources = mapContent.sources.map(
                  (source: string) => {
              ...
              console.error(
                `Error processing source map for ${jsFile}:`,
                error,
              );
            }
          } else {
        ...
        fs.writeFileSync(
          path.join(resolvedOutDir, "sourcemap-manifest.json"),
          JSON.stringify({
            enabled: true,
