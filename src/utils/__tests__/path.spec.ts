...
import { arePathsEqual, getReadablePath, getWorkspacePath, resolveRecursivePath } from "../path"
...
describe("Path Utilities", () => {
	...
	describe("resolveRecursivePath", () => {
		...
		it("should return validated path if file exists at location", async () => {
			; (fs.existsSync as any).mockReturnValue(true)
			const result = await resolveRecursivePath(cwd, "existing.ts")
			expect(result).toEqual({ resolvedPath: "existing.ts" })
		})

		it("should not search recursively if path contains slashes", async () => {
			const result = await resolveRecursivePath(cwd, "subdir/file.ts")
			expect(result).toEqual({ resolvedPath: "subdir/file.ts" })
			// Should not call findFiles
			const vscode = await import("vscode")
			expect(vscode.workspace.findFiles).not.toHaveBeenCalled()
		})

		it("should call findFiles for simple filename", async () => {
			const result = await resolveRecursivePath(cwd, "missing.ts")
			const vscode = await import("vscode")
			expect(vscode.workspace.findFiles).toHaveBeenCalledWith("**/missing.ts", "**/node_modules/**")
		})

		it("should return original path if no recursive match found", async () => {
			const vscode = await import("vscode")
				; (vscode.workspace.findFiles as any).mockResolvedValue([])
			const result = await resolveRecursivePath(cwd, "missing.ts")
			expect(result).toEqual({ resolvedPath: "missing.ts" })
		})

		it("should return resolved path and notice if single key match found", async () => {
			const vscode = await import("vscode")
				; (vscode.workspace.findFiles as any).mockResolvedValue([
					{ fsPath: "/test/cwd/src/missing.ts" }
				])

			const result = await resolveRecursivePath(cwd, "missing.ts")
			expect(result.resolvedPath).toBe("src/missing.ts")
			expect(result.notice).toBeDefined()
			expect(result.notice).toContain("File was found recursively")
		})

		it("should return duplicate warning if multiple matches found", async () => {
			const vscode = await import("vscode")
				; (vscode.workspace.findFiles as any).mockResolvedValue([
					{ fsPath: "/test/cwd/src/missing.ts" },
					{ fsPath: "/test/cwd/lib/missing.ts" }
				])

			const result = await resolveRecursivePath(cwd, "missing.ts")
			expect(result.resolvedPath).toBe("missing.ts")
			expect(result.notice).toContain("Duplicate file detected")
		})
	})
})
