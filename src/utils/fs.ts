 ...
 * @returns A promise that resolves to an array of newly created directories (empty for modern recursive calls).
 */
export async function createDirectoriesForFile(filePath: string): Promise<string[]> {
	const normalizedFilePath = path.normalize(filePath)
	const directoryPath = path.dirname(normalizedFilePath)

	// Using { recursive: true } is native, atomic, and handles existing directories gracefully.
