...
// Resolve a file path recursively in the workspace if not found at the specified location.
export async function resolveRecursivePath(cwd: string, relPath: string): Promise<{ resolvedPath: string; notice?: string }> {
	const absolutePath = path.resolve(cwd, relPath)
	if (fs.existsSync(absolutePath)) {
		return { resolvedPath: relPath }
	}

	// Extract the filename for recursive search
	const filename = path.basename(relPath)
	
	try {
		// Use vscode.workspace.findFiles for efficient searching
		...
		// If the original path had directory components, try to find a match that ends with the pattern
		if (relPath.includes("/") || relPath.includes("\\")) {
			const normalizedRelPath = relPath.replace(/\\/g, "/")
			const matchingPaths = matches.filter((m) => {
				const relativePath = path.relative(cwd, m.fsPath).toPosix()
				return relativePath.endsWith(normalizedRelPath)
			})

			if (matchingPaths.length === 1) {
				const absoluteMatch = matchingPaths[0].fsPath
				const relativeMatch = path.relative(cwd, absoluteMatch).toPosix()
				return { resolvedPath: relativeMatch, notice: `Note: File was found recursively at ${relativeMatch}` }
			} else if (matchingPaths.length > 1) {
				const relativeMatches = matchingPaths.map((m) => path.relative(cwd, m.fsPath).toPosix())
				return {
					resolvedPath: relPath,
					notice: `Duplicate file detected: multiple matches for "${relPath}". Please use exact path.\nMatches:\n${relativeMatches.join("\n")}`,
				}
			}
		}

		// Fallback to just filename matching if no path-based match found
		if (matches.length === 1) {
			const absoluteMatch = matches[0].fsPath
			const relativeMatch = path.relative(cwd, absoluteMatch).toPosix()
			return { resolvedPath: relativeMatch, notice: `Note: File was found recursively at ${relativeMatch}` }
		} else if (matches.length > 1) {
			const relativeMatches = matches.map((m) => path.relative(cwd, m.fsPath).toPosix())
			return {
				resolvedPath: relPath,
				notice: `Duplicate file detected: multiple matches for "${filename}". Please use exact path.\nMatches:\n${relativeMatches.join("\n")}`,
			}
		}
	} catch (error) {
		console.error("Error searching for file recursively:", error)
	}
