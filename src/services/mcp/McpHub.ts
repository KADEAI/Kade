...
export class McpHub {
  ...
  "mcpServers": {
	...
	async toggleAlwaysAllowAllTools(
		serverName: string,
		source: "global" | "project",
		...
		try {
			// Get current tools for the server
			...
			if (!config.mcpServers[serverName]) {
				config.mcpServers[serverName] = { type: "stdio", command: "", args: [] }
			}

			if (shouldAllowAll) {
				// Add all tool names to alwaysAllow, avoiding duplicates
				const currentAlwaysAllow = config.mcpServers[serverName].alwaysAllow || []
				const newAlwaysAllow = Array.from(new Set([...currentAlwaysAllow, ...toolNames]))
				config.mcpServers[serverName].alwaysAllow = newAlwaysAllow
			} else {
				// Clear all items from alwaysAllow
			...
			// Update tools in-memory directly - no server refetch needed
			if (connection.server.tools) {
				connection.server.tools = connection.server.tools.map((tool) => ({
					...tool,
					alwaysAllow: shouldAllowAll,
				}))
			}
			await this.notifyWebviewOfServerChanges()
		} catch (error) {
			this.showErrorMessage(`Failed to toggle always allow all tools for server ${serverName}`, error)
			throw error
		}
	}
