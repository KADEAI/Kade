...
export class FileWatcher implements IFileWatcher {
	...
	private scheduleBatchProcessing(): void {
		...
		// Increase debounce for large batches to prevent UI lockup
		const delay = this.accumulatedEvents.size > 100 ? 2000 : this.BATCH_DEBOUNCE_DELAY_MS
		this.batchProcessDebounceTimer = setTimeout(() => this.triggerBatchProcessing(), delay)
	}
		...
		if (pointsForBatchUpsert.length > 0 && this.vectorStore && !overallBatchError) {
			try {
				for (let i = 0; i < pointsForBatchUpsert.length; i += this.batchSegmentThreshold) {
					...
					while (retryCount < this.maxBatchRetries /* kade_change */) {
						try {
							...
							retryCount++
							if (retryCount === this.maxBatchRetries /* kade_change */) {
								...
								TelemetryService.instance.captureEvent(TelemetryEventName.CODE_INDEX_ERROR, {
									...
									retryCount: this.maxBatchRetries, // kade_change
								})
								throw new Error(
									`Failed to upsert batch after ${this.maxBatchRetries} retries: ${upsertError.message}`, // kade_change
								)
							}
