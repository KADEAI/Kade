...
if (this.includePaths.length > 0) {
	for (const relativePath of this.includePaths) {
		const targetDir = path.join(directoryPath, relativePath)
		if (fs.existsSync(targetDir)) {
			const [paths, reached] = await listFiles(targetDir, true, MAX_LIST_FILES_LIMIT_CODE_INDEX)
			allPaths.push(...paths)
			if (reached) limitReached = true
		}
	}
} else {
	const [paths, reached] = await listFiles(directoryPath, true, MAX_LIST_FILES_LIMIT_CODE_INDEX)
	allPaths = paths
	...
	// Filter by supported extensions, ignore patterns, and excluded directories
		const supportedPaths = allowedPaths.filter((filePath) => {
			...
			// KILOCODE FIX: Strict noise filtering for "toxic" assets
			const toxicDirectories = ["dist", "build", "out", "assets", "node_modules", "vendor", "target"]
			const pathParts = relativeFilePath.split(path.sep)

			// 1. Skip known build/artifact directories
			if (pathParts.some(part => toxicDirectories.includes(part.toLowerCase()))) {
				return false
			}

			// 2. Skip minified or hashed production assets (e.g., app-A1B2C3D4.js)
			const isMinified = fileName.includes(".min.")
			const isHashedAsset = /-[a-z0-9]{8,}\.(js|css)$/.test(fileName)
			if (isMinified || isHashedAsset) {
				return false
			}

			// 3. Standard ignored directory check
		...
		const parsePromises = supportedPaths.map((filePath) =>
			parseLimiter(async () => {
				...
				try {
					...
					// PERFORMANCE OPTIMIZATION: Disable code index telemetry to reduce overhead
					// Code indexing is performance-critical and telemetry adds unnecessary CPU/network overhead
					// Safe to disable as this is only used for analytics, not core functionality
					/*
