	...
	private async loadLanceDBModule(): Promise<any> {
		...
		while (retryCount < MAX_RETRIES) {
			try {
				...
				const isBinaryError =
					errorMessage.includes("not a valid Win32 application") ||
					errorMessage.includes("%1 is not a valid Win32 application") ||
					errorMessage.includes("The specified module could not be found") ||
					errorMessage.includes("Module did not self-register")

				if (isBinaryError && retryCount < MAX_RETRIES - 1) {
					console.warn(
						`[LanceDBVectorStore] Failed to load LanceDB module (attempt ${retryCount + 1}/${MAX_RETRIES}). Detecting possible corruption. Cleaning up and retrying...`,
						error,
					)
					await this.lancedbManager.cleanupDependencies()

					// Clear require cache for the bad module if it partially loaded
					try {
						const resolvedPath = require.resolve("@lancedb/lancedb", { paths: [this.lancedbManager.getNodeModulesPath()] })
				...
				throw new Error(t("embeddings:vectorStore.lancedbLoadFailed", { errorMessage }))
			}
		}
	}
