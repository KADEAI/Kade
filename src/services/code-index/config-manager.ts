...
export class CodeIndexConfigManager {
	...
	// kade_change - start
	private vectorStoreProvider: "lancedb" | "qdrant" | "local-qdrant" = "qdrant"
	...
	// kade_change start: Kilo org indexing props
	...
	// kade_change start: Kilo org indexing methods
	/**
	...
	private _loadAndSetConfiguration(): void {
		...
		const codebaseIndexConfig: any = this.contextProxy?.getGlobalState("codebaseIndexConfig") ?? {
		...
		const {
			...
			codebaseIndexEmbedderBaseUrl,
			codebaseIndexEmbedderModelId,
			codebaseIndexLancedbVectorStoreDirectory, // kade_change
		...
		const openAiKey =
			...
			this.contextProxy?.getSecret("openAiNativeApiKey") ||
			this.contextProxy?.getSecret("openAiApiKey") ||
			""
		const qdrantApiKey = this.contextProxy?.getSecret("codeIndexQdrantApiKey") ?? ""
		// Fix: Read OpenAI Compatible settings from the correct location within codebaseIndexConfig
		const openAiCompatibleBaseUrl = codebaseIndexConfig.codebaseIndexOpenAiCompatibleBaseUrl ?? ""
		const openAiCompatibleApiKey =
			this.contextProxy?.getSecret("codebaseIndexOpenAiCompatibleApiKey") ||
			this.contextProxy?.getSecret("openAiApiKey") ||
			""
		const geminiApiKey =
			this.contextProxy?.getSecret("codebaseIndexGeminiApiKey") ||
			this.contextProxy?.getSecret("geminiApiKey") ||
			""
		const mistralApiKey =
			this.contextProxy?.getSecret("codebaseIndexMistralApiKey") ||
			this.contextProxy?.getSecret("mistralApiKey") ||
			""
		const vercelAiGatewayApiKey =
			this.contextProxy?.getSecret("codebaseIndexVercelAiGatewayApiKey") ||
			this.contextProxy?.getSecret("vercelAiGatewayApiKey") ||
			""
		...
		const openRouterApiKey =
			this.contextProxy?.getSecret("codebaseIndexOpenRouterApiKey") ||
			this.contextProxy?.getSecret("openRouterApiKey") ||
			""
		...
		// kade_change end
		this.includePaths = codebaseIndexIncludePaths ?? []
	 ...
	 * kade_change: Also returns true if Kilo org mode is available
	 */
	public isConfigured(): boolean {
		// kade_change start: Allow Kilo org mode as configured
	...
	doesConfigChangeRequireRestart(prev: PreviousConfigSnapshot): boolean {
		...
		// Include paths change - scanner needs to be recreated with new paths
		const prevIncludePaths = prev?.includePaths ?? []
		const currentIncludePaths = this.includePaths ?? []
		if (
			prevIncludePaths.length !== currentIncludePaths.length ||
			!prevIncludePaths.every((p, i) => currentIncludePaths[i] === p)
		) {
			return true
		}
