...
import { LobeHubLoader } from "./LobeHubLoader"
...
export class MarketplaceManager {
	...
	private lobeHubLoader: LobeHubLoader
	...
	constructor(
		...
		this.lobeHubLoader = new LobeHubLoader()
		this.installer = new SimpleInstaller(context, customModesManager)
	}

	async getMarketplaceItems(search?: string): Promise<MarketplaceItemsResponse> {
		try {
			const errors: string[] = []
			...
			// Fetch LobeHub MCPs if not hidden
			let lobeHubMcps: MarketplaceItem[] = []
			if (!orgSettings?.hideMarketplaceMcps) {
				try {
					lobeHubMcps = await this.lobeHubLoader.loadMcps(search)
				} catch (error) {
					// Silently handle LobeHub failures - they return empty array on auth issues
					// Don't fail the whole request if LobeHub fails
				}
			}

			// Merge LobeHub items
			if (lobeHubMcps.length > 0) {
				console.log(`Loaded ${lobeHubMcps.length} LobeHub MCPs`)
			}
			allMarketplaceItems.push(...lobeHubMcps)
	...
	async getCurrentItems(search?: string): Promise<MarketplaceItem[]> {
		const result = await this.getMarketplaceItems(search)
	...
	async updateWithFilteredItems(filters: {
		...
		// If searching, we fetch again to get results from the full 24k LobeHub database
		const allItems = await this.getCurrentItems(filters.search)
