...
export class SimpleInstaller {
	...
	private async installMcp(
		...
		} else if ((item as any).downloadUrl) {
			// Fetch content from downloadUrl if content is missing
			try {
				const response = await fetch((item as any).downloadUrl, { method: "GET" })
				if (!response.ok) {
					throw new Error(`Status ${response.status} ${response.statusText}`)
				}
				const text = await response.text()
				contentToUse = text // JSON.parse will be handled later
			} catch (error) {
				throw new Error(`Failed to download MCP content from ${(item as any).downloadUrl}: ${error}`)
			}
		} else {
			throw new Error("MCP item missing content and downloadUrl")
		}
		...
		let mcpData = JSON.parse(contentToUse)

		// Check if this is a LobeHub manifest and transform it
		if (mcpData.deploymentOptions && Array.isArray(mcpData.deploymentOptions)) {
			// Find the recommended deployment option or use the first one
			const deployment = mcpData.deploymentOptions.find((d: any) => d.isRecommended) || mcpData.deploymentOptions[0]

			if (deployment && deployment.connection) {
				const connection = deployment.connection

				// Transform to standard MCP config
				// Only 'stdio' is fully supported for now via SimpleInstaller without extra checks
				// But we map whatever is provided and let McpHub validate it
				mcpData = {
