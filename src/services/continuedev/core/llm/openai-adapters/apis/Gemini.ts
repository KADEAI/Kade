...
import { streamResponse, streamSse } from "../../../fetch/stream.js"
...
export class GeminiApi implements BaseLlmApi {
	...
	async *handleStreamResponse(response: Response, model: string) {
		...
		for await (const data of streamSse(response)) {
			...
			if (contentParts) {
				...
				// Only warn if it's not a usage-only chunk and not an empty candidate list at the end
				if (!data.usageMetadata && data.candidates?.length > 0) {
	...
	async *chatCompletionStream(
		...
		const apiURL = new URL(
			`models/${body.model}:streamGenerateContent?key=${this.config.apiKey}&alt=sse`,
			this.apiBase,
		).toString()
