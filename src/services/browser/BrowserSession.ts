...
export class BrowserSession {
	...
	private async launchLocalBrowser(): Promise<void> {
		...
		const disableHeadless = this.context.globalState.get("disableBrowserHeadless") as boolean | undefined
	...
	private async emitLiveScreenshot() {
		if (this.page && this.onScreenshot) {
			try {
				const screenshotBase64 = await this.page.screenshot({
					type: "webp",
					quality: 40, // lower quality for live stream
					encoding: "base64",
				})
				if (screenshotBase64) {
					this.onScreenshot(`data:image/webp;base64,${screenshotBase64}`)
				}
			} catch (e) {
	...
	private async waitTillHTMLStable(page: Page, timeout = 5_000) {
		...
		while (checkCounts++ <= maxChecks) {
			// Emit live screenshot during wait
			await this.emitLiveScreenshot()
