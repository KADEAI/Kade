...
const METHOD_CAPTURE = ["definition.method", "definition.method.start"] // kade_change
...
function processCaptures(captures: QueryCapture[], lines: string[], language: string): string | null {
	...
	captures.forEach((capture) => {
		...
		if (shouldSkipMinLines(lineCount, capture, language) /*kade_change: orginal logic moved into function*/) {
			return
		}
...
export interface SymbolItem {
	name: string
	type: "Class" | "Method" | "Function" | "Struct" | "Enum" | "Trait" | "Impl" | "Module" | "Macro" | "Const" | "TypeAlias" | "Interface" | "Match" | "Unsafe"
	line: number // 1-indexed
	endLine: number // 1-indexed
	signature?: string
}

export async function getSymbolMapForFile(
	filePath: string,
	rooIgnoreController?: RooIgnoreController,
): Promise<SymbolItem[]> {
	const ext = path.extname(filePath).toLowerCase()
	...
	try {
		...
		for (const capture of captures) {
			...
			let type: SymbolItem["type"] | undefined
			if (name.includes("class")) type = "Class"
			else if (name.includes("method")) type = "Method"
			else if (name.includes("function")) type = "Function"
			else if (name.includes("struct")) type = "Struct"
			else if (name.includes("enum")) type = "Enum"
			else if (name.includes("trait")) type = "Trait"
			else if (name.includes("impl")) type = "Impl"
			else if (name.includes("module")) type = "Module"
			else if (name.includes("macro")) type = "Macro"
			else if (name.includes("const")) type = "Const"
			else if (name.includes("type_alias")) type = "TypeAlias"
			else if (name.includes("interface")) type = "Interface"
			else if (name.includes("match")) type = "Match"
			else if (name.includes("unsafe")) type = "Unsafe"

			if (!type) continue

			// Only process "name" captures to get the identifier, OR specific definition start nodes
			if (!name.includes("name") && !name.includes("definition")) continue
			...
			if (name.includes("name")) {
				...
				// For blocks like unsafe/match that don't have a specific name capture
				if (type === "Unsafe") symbolName = "unsafe"
				else if (type === "Match") symbolName = "match"
				else continue
			}

			// Extract signature: first line of the definition
			const definitionNode = node.parent || node
			const signatureLine = lines[definitionNode.startPosition.row].trim()
		...
		console.error("Error generating symbol map:", error)
		return []
	}
}
