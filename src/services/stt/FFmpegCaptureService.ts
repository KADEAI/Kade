// kade_change - new file: FFmpeg-based PCM16 audio capture for OpenAI Realtime API
...
export class FFmpegCaptureService extends EventEmitter {
	 ...
	 */
	async start(sampleRate: number = 24000): Promise<void> {
		if (this.isCapturing) {
		...
		try {
			const args = this.buildFFmpegArgs(sampleRate)
	...
	private buildFFmpegArgs(sampleRate: number): string[] {
		...
		const outputArgs = [
			...
			"-ar",
			sampleRate.toString(), // Customizable sample rate
			"-ac",
			"1", // Mono
			"-f",
