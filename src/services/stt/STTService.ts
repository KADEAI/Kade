...
import { GeminiSTTClient } from "./GeminiSTTClient"
...
export class STTService {
	...
	private transcriptionClient: OpenAIWhisperClient | GeminiSTTClient | null = null
	private localWhisperService: LocalWhisperSTTService | null = null
	...
	private currentProvider: "openai" | "gemini" | "local" | null = null
	...
	constructor(
		...
		this.postMessageToWebview = postMessageToWebview || null
	}
	...
	async start(config: STTProviderConfig, language?: string): Promise<void> {
		...
		try {
			if (config.provider === "local") {
				// Local Whisper - uses FFmpeg for audio capture, sends to webview for transcription
				if (!this.postMessageToWebview) {
					throw new Error("Local Whisper requires postMessageToWebview callback")
				}
				this.localWhisperService = new LocalWhisperSTTService(this.emitter, this.postMessageToWebview)
				await this.localWhisperService.start(config.modelId, this.sessionId!)
			} else if (config.provider === "gemini") {
				this.transcriptionClient = new GeminiSTTClient(this.providerSettingsManager, {
					apiKey: config.apiKey,
				})
				await this.audioCapture.start(16000) // Gemini requires 16kHz
				...
				await this.audioCapture.start(24000) // OpenAI requires 24kHz
				this.setupEventHandlers()
				await this.transcriptionClient?.connect()
			}
	...
	async stop(): Promise<string> {
		...
		try {
			...
			if (this.currentProvider === "local" && this.localWhisperService) {
				await this.localWhisperService.stop()
				// Local whisper handles its own completion via handleWebviewMessage
				return ""
			}
	...
	cancel(): void {
		...
		if (this.currentProvider === "local" && this.localWhisperService) {
			this.localWhisperService.cancel()
		...
		try {
			...
			// Only disconnect if actually connected to avoid WebSocket errors
	...
	/**
	 * Handle messages from webview for local whisper transcription
	 */
	handleLocalWhisperMessage(message: any): void {
		if (this.localWhisperService) {
			this.localWhisperService.handleWebviewMessage(message)
		}
	}
