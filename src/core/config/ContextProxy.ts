...
export class ContextProxy {
	...
	private getAllGlobalState(): GlobalState {
		const keys = [...new Set([...GLOBAL_STATE_KEYS, ...PASS_THROUGH_STATE_KEYS])] as GlobalStateKey[]
		return Object.fromEntries(keys.map((key) => [key, this.getGlobalState(key)]))
	}
	...
	public getValues(): RooCodeSettings {
		...
		const merged = { ...globalState, ...secretState }
