...
const PASS_THROUGH_STATE_KEYS = [
  ...
  "dismissedUpsells",
  // Read formatter settings directly from VS Code storage so they survive
  // reloads even if the generated global state key list is stale.
  "formatterSettings",
];
...
export class ContextProxy {
  ...
  public async initialize() {
    ...
    try {
      this.stateCache.formatterSettings =
        this.originalContext.globalState.get("formatterSettings");
    } catch (error) {
      logger.error(
        `Error loading global formatterSettings: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  ...
  private getAllGlobalState(): GlobalState {
    const keys = [
      ...new Set([...GLOBAL_STATE_KEYS, ...PASS_THROUGH_STATE_KEYS]),
    ] as GlobalStateKey[];
    return Object.fromEntries(
      keys.map((key) => [key, this.getGlobalState(key)]),
    );
  }
  ...
  public async setProviderSettings(values: ProviderSettings) {
    ...
    const oldTesterWarnings =
      this.stateCache.kilocodeTesterWarningsDisabledUntil;
    // kade_change end
    ...
    const newTesterWarnings =
      this.stateCache.kilocodeTesterWarningsDisabledUntil;

    if (
      oldToken !== newToken ||
      oldOrgId !== newOrgId ||
      oldTesterWarnings !== newTesterWarnings
    ) {
  ...
  public getValues(): RooCodeSettings {
    ...
    const merged = {
      ...
      formatterSettings:
        this.originalContext.globalState.get("formatterSettings") ??
        globalState.formatterSettings,
    };
    this._mergedValuesCache = merged;
    return merged;
  }
   ...
   */
  public onManagedIndexerConfigChange(
    listener: (config: ManagedIndexerConfig) => void,
  ): vscode.Disposable {
    this.configEmitter.on("managed-indexer-config-changed", listener);
    return {
      dispose: () =>
        this.configEmitter.off("managed-indexer-config-changed", listener),
    };
  }
  // kade_change end
