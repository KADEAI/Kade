...
export class ProviderSettingsManager {
	...
	async init_runMigrations() {
		try {
			return await this.lock(async () => {
				...
				// Ensure all configs have IDs, and set default values for todoListEnabled and diffEnabled.
				for (const [_name, apiConfig] of Object.entries(providerProfiles.apiConfigs)) {
					if (!apiConfig.id) {
						apiConfig.id = this.generateId()
						isDirty = true
					}
					// Always disable todo list tool and diff editing by default
					if (apiConfig.todoListEnabled !== false) {
						apiConfig.todoListEnabled = false
						isDirty = true
					}
					if (apiConfig.diffEnabled !== false) {
						apiConfig.diffEnabled = false
						isDirty = true
					}
				}
	...
	private async load(): Promise<ProviderProfiles> {
		try {
			...
			const apiConfigs = Object.entries(providerProfiles.apiConfigs).reduce<Record<string, any>>(
				(acc, [key, apiConfig]) => {
					...
					return { ...acc, [key]: result.success ? result.data : (sanitizedConfig as any) }
				},
				{},
			)
