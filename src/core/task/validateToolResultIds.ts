...
export function validateAndFixToolResultIds(
	...
	// Build a set of existing tool_result IDs (includes text blocks with _toolUseIds for unified/markdown protocols)
	const existingToolResultIds = new Set(toolResults.map((r) => r.tool_use_id))
	for (const block of userMessage.content as any[]) {
		if (block.type === "text") {
			if (block._toolUseId) existingToolResultIds.add(block._toolUseId)
			if (Array.isArray(block._toolUseIds)) {
				block._toolUseIds.forEach((id: string) => existingToolResultIds.add(id))
			}
		}
	}
	...
	correctedContent.forEach((b) => {
		if (b.type === "tool_result") {
			coveredToolUseIds.add((b as Anthropic.ToolResultBlockParam).tool_use_id)
		} else if (b.type === "text") {
			// For Unified/Markdown protocols, tool results are embedded in text blocks
			const tb = b as any
			if (tb._toolUseId) coveredToolUseIds.add(tb._toolUseId)
			if (Array.isArray(tb._toolUseIds)) {
				tb._toolUseIds.forEach((id: string) => coveredToolUseIds.add(id))
			}
		}
	})
