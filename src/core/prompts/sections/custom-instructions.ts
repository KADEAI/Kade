...
async function getRooDirectoriesForCwd(cwd: string): Promise<string[]> {
	try {
		const { getRooDirectoriesForCwd } = await import("../../../services/roo-config")
		return getRooDirectoriesForCwd(cwd)
	} catch {
		return [path.join(cwd, ".kilocode"), path.join(cwd, ".roo")]
	}
}
...
export async function loadRuleFiles(cwd: string): Promise<string> {
	const rules: string[] = []
	const rooDirectories = await getRooDirectoriesForCwd(cwd)
