...
export function getReadFileDescription(args: ToolArgs): string {
  ...
  const supportsImages = args.supportsBrowserUse ?? args.supportsComputerUse

  if (args.compact) {
    return `## read
Read contents of ${isMultipleReadsEnabled ? "up to 5 files" : "a file"}. The tool outputs line-numbered content.
<read><args><file><path>...</path>${args.partialReadsEnabled ? "<line_range>...</line_range>" : ""}</file></args></read>`
  }
...
${isMultipleReadsEnabled ? `**IMPORTANT: You can read a maximum of ${maxConcurrentReads} files in a single request.** If you need to read more files, use multiple sequential read requests.` : "**IMPORTANT: Multiple file reads are currently disabled. You can only read one file at a time.**"}
