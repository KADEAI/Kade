...
export function getEditFileDescription(args?: ToolArgs): string {
  if (args?.compact) {
    return `## edit_file
Edit a file using specialized fast-apply model. All 3 params are mandatory. Use \`// ... existing code ...\` for unchanged parts.
<edit_file><target_file>...</target_file><instructions>...</instructions><code_edit>...</code_edit></edit_file>`
  }
  return `## edit_file
...
<instructions>I will update the calculate function to handle null values.</instructions>
<code_edit>
// ... existing code ...
