...
import { ANTIGRAVITY_TEMPLATE } from "./sections/antigravity"
import { ANTIGRAVITY_MINIMAL_TEMPLATE } from "./sections/antigravityminimal"
...
async function generatePrompt(
	...
	const [modesSection, mcpServersSection] = await Promise.all([
		getModesSection(context, mode, roleDefinition, modelId, effectiveProtocol),
		shouldIncludeMcp
			? getMcpServersSection(
				mcpHub,
	...
	// Build tools catalog section
	let toolsCatalog = ""
	const isIndexingEnabled = codeIndexManager?.isFeatureEnabled ?? false
...
${customInstructions}`
	...
	// Read projectinit.md if it exists
	let projectInit = ""
	try {
		const projectInitPath = path.join(cwd, "projectinit.md")
		projectInit = (await fs.readFile(projectInitPath, "utf-8")).trim()
	} catch (error) {
