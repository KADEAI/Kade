...
export async function readApiMessages({
	...
	try {
		...
		if (error.code !== "ENOENT") {
			console.error(
				`[Roo-Debug] readApiMessages: Error reading/parsing API conversation history file. TaskId: ${taskId}, Path: ${filePath}, Error: ${error}`,
			)
			throw error
		}

		// If new file doesn't exist, try the old path
