...
export class KilocodeEventProcessor {
	...
	public handle(sessionId: string, event: KilocodeStreamEvent): void {
		...
		// Skip empty partial messages (but allow tool asks that have metadata)
		const rawContent = payload.content || payload.text
		const hasToolMetadata = payload.ask === "tool" && payload.metadata
		if (payload.partial && !rawContent && !isCommandOutput && !hasToolMetadata) {
			return
		}
		...
		if (!message.text && message.type === "ask" && message.ask) {
			if (message.ask === "tool") {
				message.text = message.metadata ? JSON.stringify(message.metadata) : this.formatToolAskText(message.metadata)
	...
	private deriveMessageText(payload: KilocodePayload, checkpoint?: Record<string, unknown>): string {
		...
		// Tool asks
		if (payload.ask === "tool") {
			return payload.metadata ? JSON.stringify(payload.metadata) : (this.formatToolAskText(payload.metadata) || "")
		}
