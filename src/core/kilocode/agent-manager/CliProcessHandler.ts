...
interface PendingProcessInfo {
	...
	teamId?: string // Group Chat team ID
	teamRole?: "leader" | "worker" | "validator" // Agent's role in the team
	folderId?: string
}
...
export interface CliProcessHandlerCallbacks {
	...
	onSessionCreated: (sawApiReqStarted: boolean, folderId?: string) => void
	onSessionRenamed?: (oldId: string, newId: string, folderId?: string) => void
...
export class CliProcessHandler {
	...
	public spawnProcess(
		...
		options:
			| {
				...
				shellPath?: string
				/** Secrets like credentials for specific providers */
	...
	private createProvisionalSession(proc: ChildProcess): void {
		...
		const { prompt, startTime, parallelMode, desiredLabel, gitUrl, parser, worktreeBranch, worktreePath, teamId, teamRole } =
			this.pendingProcess
		...
		this.registry.clearPendingSession()
		this.callbacks.onPendingSessionChanged(null)
		this.callbacks.onSessionCreated(this.pendingProcess?.sawApiReqStarted ?? false, this.pendingProcess?.folderId)
	...
	private upgradeProvisionalSession(
		...
		this.callbacks.onSessionRenamed?.(provisionalSessionId, realSessionId, this.pendingProcess?.folderId)
	...
	private handleSessionCreated(event: SessionCreatedStreamEvent): void {
		...
		// Notify callbacks
		this.callbacks.onPendingSessionChanged(null)
		this.callbacks.onSessionCreated(sawApiReqStarted ?? false, folderId)
		this.callbacks.onStateChanged()
	}
