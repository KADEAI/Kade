...
import { t } from "../../i18n"
import { isSupportedImageFormat } from "../tools/helpers/imageHelpers" // kade_change
...
export async function openMention(cwd: string, mention?: string): Promise<void> {
	...
	if (mention.startsWith("/")) {
		...
		try {
			...
			// Path doesn't exist at CWD, keep as relative for openFile fallback
		}
	...
	try {
		...
		if (stats.isFile()) {
			if (rooIgnoreController && !rooIgnoreController.validateAccess(absPath)) {
				return `(File ${mentionPath} is ignored by .kadeignore)`
			}
			// kade_change start
