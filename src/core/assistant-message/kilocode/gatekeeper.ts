...
export async function evaluateGatekeeperApproval(
	...
	try {
		...
		// Use a race to prevent the gatekeeper from hanging the entire task
		const timeout = new Promise<never>((_, reject) => setTimeout(() => reject(new Error("Gatekeeper timeout")), 15000))

		const { text, usage } = await Promise.race([
			streamResponseFromHandler(handler, userPrompt, systemPrompt),
			timeout
		])
