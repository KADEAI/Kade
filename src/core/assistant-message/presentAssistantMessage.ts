...
} from "../tools/kilocode/editFileTool";
...
import { appendConsolidatedToolResult } from "../task/toolResultSeparators";
    ...
    while (true) {
      ...
      switch (block.type) {
        case "mcp_tool_use": {
          ...
          let mcpToolProtocol: ToolProtocol = TOOL_PROTOCOL.UNIFIED;
          if (mcpToolCallId) {
            if (mcpToolCallId.startsWith("unified_")) {
              ...
              mcpToolProtocol = TOOL_PROTOCOL.JSON;
            }
          }
          const isUnifiedOrMarkdownMcp =
            (mcpToolProtocol as string) === "unified" ||
            mcpToolProtocol === TOOL_PROTOCOL.MARKDOWN;
          ...
          if (cline.didRejectTool) {
            ...
            if (mcpToolCallId && !isUnifiedOrMarkdownMcp) {
          ...
          const pushToolResult = (content: ToolResponse) => {
            ...
            if (isNativeProtocol(toolProtocol)) {
              ...
              if (
                lastIndex >= 0 &&
                cline.userMessageContent[lastIndex].type === "text"
              ) {
                const lastBlock = cline.userMessageContent[
                  lastIndex
                ] as Anthropic.TextBlockParam;
                lastBlock.text = appendConsolidatedToolResult(
                  lastBlock.text,
                  fullResult,
                );
                if (!(lastBlock as any)._toolUseIds) {
                  (lastBlock as any)._toolUseIds = (lastBlock as any)._toolUseId
                    ? [(lastBlock as any)._toolUseId]
                    : [];
                }
                if (toolCallId) {
          ...
          if (mcpHub) {
            const originalName = mcpHub.findServerNameBySanitizedName(
              mcpBlock.serverName,
            );
            if (originalName) {
        ...
        case "text": {
          ...
          if (content) {
            ...
            content = content.replace(
              /^\s*<<<\s*(?:tool:|tool\s+)?[a-zA-Z_][\w-]*(?:\([^)]*)?\s*$/gm,
              "",
            );
        ...
        case "tool_use": {
          ...
          if (block.name === "attempt_completion" && block.partial) {
            console.log(
              "[presentAssistantMessage] Skipping attempt_completion during streaming (handled by AgentLoop)",
            );
            break;
          }
          ...
          const toolDescription = (): string => {
            const rawToolName = block.originalName || block.name;
            const toolName = rawToolName.toUpperCase();
            const getPath = () =>
              block.params.path ||
            ...
            switch (block.name) {
              case "bash":
                return `[${toolName} for '${block.params.command}'${
                  block.params.cwd ? ` in '${block.params.cwd}'` : ""
                }]`;
              case "batch": {
                const nativeCalls = (
                  block.nativeArgs as { calls?: unknown[] } | undefined
                )?.calls;
                const count = Array.isArray(nativeCalls)
                  ? nativeCalls.length
                  : (() => {
                      try {
                        const parsed = block.params.calls
                          ? JSON.parse(block.params.calls)
                          : [];
                ...
                return `[${toolName} for ${count} batched call${count === 1 ? "" : "s"}]`;
              }
              case "read":
              ...
              case "grep": {
                const grepPattern =
                  block.params.file_pattern || block.params.include;
                return `[${toolName} for '${block.params.query || block.params.regex}'${
                  getPath() ? ` in '${getPath()}'` : ""
              ...
              case "attempt_completion":
                return `[${toolName}]`;
              case "switch_mode":
                return `[${toolName} to '${block.params.mode_slug}'${
                  block.params.reason ? ` because: ${block.params.reason}` : ""
                }]`;
              case "ask": // Add case for the new tool
              ...
              case "run_slash_command":
                return `[${toolName} for '${block.params.command}'${
                  block.params.args ? ` with args: ${block.params.args}` : ""
                }]`;
              case "generate_image":
                return `[${toolName} for '${getPath()}']`;
              default:
          ...
          let toolProtocol: ToolProtocol = TOOL_PROTOCOL.UNIFIED;
          if (toolCallId) {
          ...
          const pushToolResult = (content: ToolResponse) => {
            ...
            // For native protocol, tool_result blocks are tracked per tool_use_id,
            // so multiple sequential tool calls in the same assistant turn are safe.
          };
          ...
          const askApproval = async (
            ...
            if (cline.yoloMode || state?.yoloMode) {
              ...
              const approved = await evaluateGatekeeperApproval(
                cline,
                block.name,
                block.params,
              );
              ...
              console.warn(
                `[presentAssistantMessage] Auto-approved ${block.name} in YOLO mode; finalized partial ${type} shell.`,
              );
          ...
          const handleError = async (action: string, error: Error) => {
            ...
            const errorString = `Error ${action}: ${JSON.stringify(serializeError(error))}`;
            await surfaceToolFailureMessage(cline, errorString);
            pushToolResult(formatResponse.toolError(errorString, toolProtocol));
          };
          ...
          if (!block.partial) {
            ...
            const { resolveToolAlias } = await import(
              "../prompts/tools/filter-tools-for-mode"
            );
            const includedTools = rawIncludedTools?.map((tool) =>
            ...
            try {
              ...
              await surfaceToolFailureMessage(cline, validationError);
              const errorContent = formatResponse.toolError(
                validationError,
                toolProtocol,
              );
              if (isNativeProtocol(toolProtocol) && toolCallId) {
          ...
          switch (block.name) {
            case "write":
              ...
              await writeToFileTool.handle(cline, block as ToolUse<"write">, {
                askApproval,
                handleError,
                pushToolResult,
                removeClosingTag,
              ...
              break;
            case "todo":
              await updateTodoListTool.handle(cline, block as ToolUse<"todo">, {
                askApproval,
                handleError,
                pushToolResult,
                removeClosingTag,
            ...
            case "condense":
              ...
              if (block.name === "edit_file") {
                await editTool.handle(
                  cline,
                  block as unknown as ToolUse<"edit">,
                  {
                    askApproval,
                    handleError,
                    pushToolResult,
            ...
            case "read":
              ...
              // Type assertion is safe here because we're in the "read" case
              await readFileTool.handle(cline, block as ToolUse<"read">, {
                askApproval,
                handleError,
                pushToolResult,
                removeClosingTag,
            ...
            case "fetch_instructions":
              ...
              break;
            case "list":
              await listDirTool.handle(cline, block as ToolUse<"list">, {
                askApproval,
                handleError,
                pushToolResult,
                removeClosingTag,
            ...
            case "move_file":
              ...
              break;
            case "ask":
              await codebaseSearchTool.handle(cline, block as ToolUse<"ask">, {
                askApproval,
                handleError,
                pushToolResult,
                removeClosingTag,
              ...
              break;
            case "fast_context":
              await fastContextTool.handle(
                cline,
                block as ToolUse<"fast_context">,
                {
                  askApproval,
                  handleError,
                  pushToolResult,
            ...
            case "computer_action": {
              ...
              await computerActionTool.handle(cline, block as ToolUse<"computer_action">, {
                askApproval,
                handleError,
                pushToolResult,
                removeClosingTag,
            ...
            case "bash":
              console.info(
                `[presentAssistantMessage] bash case reached, partial=${block.partial}, wasPartialAtStart=${wasPartialAtStart}`,
              );
              await executeCommandTool.handle(cline, block as ToolUse<"bash">, {
                askApproval,
                handleError,
                pushToolResult,
                removeClosingTag,
              ...
              break;
            case "batch":
              await batchTool.handle(cline, block as ToolUse<"batch">, {
                askApproval,
                handleError,
                pushToolResult,
                removeClosingTag,
            ...
            case "switch_mode":
              await switchModeTool.handle(
                cline,
                block as ToolUse<"switch_mode">,
                {
                  askApproval,
                  handleError,
                  pushToolResult,
            ...
            // kade_change end
            case "web":
              await webSearchTool.handle(cline, block as ToolUse<"web">, {
                askApproval,
                handleError,
                pushToolResult,
                removeClosingTag,
              ...
              break;
            case "fetch":
              await webFetchTool.handle(cline, block as ToolUse<"fetch">, {
                askApproval,
                handleError,
                pushToolResult,
                removeClosingTag,
              ...
              break;
            case "research_web":
              await researchWebTool.handle(
                cline,
                block as ToolUse<"research_web">,
                {
                  askApproval,
                  handleError,
                  pushToolResult,
            ...
            case "agent":
              await runSubAgentTool.handle(cline, block as ToolUse<"agent">, {
                askApproval,
                handleError,
                pushToolResult,
                removeClosingTag,
      ...
      const shouldFinalize =
        !wasPartialAtStart || cline.didRejectTool || cline.didAlreadyUseTool;
