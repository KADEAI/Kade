...
export const PRIMARY_PARAMS: Record<string, string[]> = {
  read: ["path"],
  bash: ["command", "stdin", "execution_id", "cwd"],
  write: ["path", "content"],
  edit: ["path", "edit"],
  list: ["path", "recursive"],
  ...
  computer_action: ["action", "coordinate", "text"],
  ...
  return files.map((file: any) => {
    const parseInlineTarget = (target: string): FileEntry | null => {
      ...
      if (colonIndex !== -1) {
        ...
        const modifierText = trimmed.slice(colonIndex + 1).trim();
        ...
        if (path && modifiers.length > 0) {
          ...
          for (const modifier of modifiers) {
            ...
            if (headMatch) {
              entry.head = parseInt(headMatch[1], 10);
              continue;
            }

            const tailMatch = modifier.match(/^T(\d+)$/i);
            if (tailMatch) {
              entry.tail = parseInt(tailMatch[1], 10);
              continue;
            }

            const rangeMatch = modifier.match(/^(?:L)?(\d+)-(\d+)$/i);
            if (rangeMatch) {
              entry.lineRanges ??= [];
    ...
    if (typeof file === "string") {
      return parseInlineTarget(file) ?? { path: file.trim() };
    }

    const inlineEntry =
      typeof file.path === "string" ? parseInlineTarget(file.path) : null;
    const entry: FileEntry = inlineEntry ?? { path: file.path };
    ...
    if (file.head !== undefined) {
      entry.head = Number(file.head);
    }
    if (file.tail !== undefined) {
      entry.tail = Number(file.tail);
    }
    return entry;
  });
}
 ...
 */
export function reparseVulnerableParams(
  ...
  for (const paramName of Object.keys(params) as ToolParamName[]) {
    if (VULNERABLE_PARAMS.includes(paramName)) {
      ...
      if (startIndex !== -1 && endIndex > startIndex) {
        const correctedValue = toolContent.slice(
...
export class KiloXmlHandler {
  ...
  public handleReadVariadic(tagContent: string, currentToolUse: ToolUse): void {
    ...
    currentToolUse.params.path = currentPath
  ...
  public finalizeToolUse(
    ...
    if (this.openedWithTag && Object.keys(currentToolUse.params).length === 0) {
      ...
      if (primaryParams && primaryParams.length > 0) {
        ...
        if (bareContent) {
          if (currentToolUse.name === "read") {
            this.handleReadVariadic(bareContent, currentToolUse);
          } else {
            currentToolUse.params[primaryParams[0] as ToolParamName] =
              bareContent;
          }
        }
      }
    ...
    if (
      this.positionalContentParamName &&
      currentToolUse.params[this.positionalContentParamName]
    ) {
      let content = currentToolUse.params[
        this.positionalContentParamName
      ] as string;
 ...
 */
export function populateToolParamsFromXmlArgs(
  toolName: string,
  args: string[],
  toolUse: ToolUse,
): void {
  ...
  switch (toolName) {
    case "read": {
      ...
      if (native.files.length === 1 && native.files[0].lineRanges.length > 0) {
        params.lineRange = native.files[0].lineRanges
