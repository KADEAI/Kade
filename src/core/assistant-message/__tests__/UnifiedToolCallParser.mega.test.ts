...
describe("UnifiedToolCallParser — THE MEGA GAUNTLET", () => {
  ...
  describe("R (read)", () => {
    ...
    it("parses R with a line range inline", () => {
      ...
      const t = tool(blocks);
      expect(t.nativeArgs.files[0].lineRanges[0]).toEqual({
        start: 10,
        end: 50,
      });
    });
    ...
    it("parses multi-file R block where all files are in the body", () => {
      ...
      // Multi-file R gets split into N individual read tool calls
      const readTools = tools(blocks).filter((t: any) => t.name === "read");
      expect(readTools.length).toBeGreaterThanOrEqual(1);
      const allPaths = readTools.map(
        (t: any) => t.nativeArgs?.files?.[0]?.path ?? t.params?.path,
      );
    ...
    it("parses multi-file R where first file is inline and rest are in body", () => {
      ...
      const blocks = parse(msg);
      // Multi-file R produces individual read calls
      const readTools = tools(blocks).filter((t: any) => t.name === "read");
      expect(readTools.length).toBeGreaterThanOrEqual(1);
      const allPaths = readTools.map(
        (t: any) => t.nativeArgs?.files?.[0]?.path ?? t.params?.path,
    ...
    it("parses multi-file R with mixed ranges inline", () => {
      ...
      const blocks = parse(msg);
      // Multi-file R produces individual read calls split by file
      const readTools = tools(blocks).filter((t: any) => t.name === "read");
      expect(readTools.length).toBeGreaterThanOrEqual(1);
    ...
    it("treats range-only continuation lines as extra ranges for the same file", () => {
      const msg =
        "R webview-ui/src/components/chat/ChatRow.tsx 1750-1780\n1870-1950\n2100-2120\n/R";
      const blocks = parse(msg);
      const t = tool(blocks);
      expect(t.nativeArgs.files).toHaveLength(1);
      expect(t.nativeArgs.files[0].path).toBe(
    ...
    it("treats '/R' with trailing prose as a closer instead of a second read", () => {
      ...
      expect(
        readTools.some((toolUse: any) =>
          String(toolUse.nativeArgs.files[0].path).includes(
            "see this is the exact message",
          ),
        ),
      ).toBe(false);
    });
  ...
  describe("B (bash)", () => {
    ...
    it("parses shell --path as a cwd alias without leaving it in the command", () => {
      const blocks = parse(
        "```tool\nshell npm test --path src/components\n```",
      );
      const t = tool(blocks);
      expect(t.name).toBe("bash");
      expect(t.params.cwd).toBe("src/components");
      expect(t.params.command).toBe("npm test");
    });

    it("parses shell --cwd without leaving it in the command", () => {
      const blocks = parse("```tool\nshell npm test --cwd src/components\n```");
      const t = tool(blocks);
      expect(t.name).toBe("bash");
      expect(t.params.cwd).toBe("src/components");
      expect(t.params.command).toBe("npm test");
    ...
    it("parses shell stdin handoff arguments", () => {
      const blocks = parse(
        '```tool\nshell --stdin "alice" --execution_id exec-123\n```',
      );
      const t = tool(blocks);
      expect(t.name).toBe("bash");
      expect(t.params.stdin).toBe("alice");
      expect(t.params.execution_id).toBe("exec-123");
      expect(t.params.command).toBeUndefined();
    });
  ...
  describe("Edge cases & resilience", () => {
    ...
    it("keeps M reserved for mkdir in unified mode even when MCP tools are registered", () => {
      const p = new UnifiedToolCallParser();
      p.setMcpToolNames([
        {
          compositeName: "my-server_my_tool",
          serverName: "my-server",
          toolName: "my_tool",
        },
      ]);
      const msg = "M tmp/cache /M";
      p.processChunk(msg);
      p.finalizeContentBlocks();
      const blocks = p.getContentBlocks();
      const toolBlock = blocks.find((b) => b.type === "tool_use") as any;
      expect(toolBlock).toBeDefined();
      expect(toolBlock.name).toBe("mkdir");
      expect(toolBlock.params.path).toBe("tmp/cache");
    });
  });
  ...
  describe("Realistic end-to-end scenarios", () => {
    it("discovery workflow: L + F + G", () => {
      ...
      const ts = tools(blocks);
      expect(ts).toHaveLength(3);
      // L = list, F = list_files/search_files, G = grep
      expect(ts[0].name).toBe("list");
    ...
    it("read multi with ranges, then grep", () => {
      ...
      const blocks = parse(msg);
      const ts = tools(blocks);
      // Multi-file R gets split into individual read calls by the parser
      // So 3 files + 1 grep = 4 tools total
      const readTools = ts.filter((t: any) => t.name === "read");
...
/G`;
      ...
      readTools.forEach((t) => expect(t.name).toBe("read"));
