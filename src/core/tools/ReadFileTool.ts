...
import { convertFileEntries } from "../assistant-message/XmlToolParser";
...
export { joinNativeReadResults, joinNativeReadSections, wrapNativeReadResult };
...
export class ReadFileTool extends BaseTool<"read"> {
  ...
  parseLegacy(params: Partial<Record<string, string>>): { files: FileEntry[] } {
    ...
    if (legacyPath) {
      const parts: string[] = [];
      for (const rawPart of legacyPath.split(",")) {
        const trimmed = rawPart.trim();
        if (!trimmed) continue;

        if (
          parts.length > 0 &&
          (/^(?:L)?\d+-\d+$/i.test(trimmed) || /^[HT]\d+$/i.test(trimmed))
        ) {
      ...
      for (const part of parts) {
        ...
        const parsedEntry = convertFileEntries([trimmed])[0];
        let filePath = parsedEntry.path;
        let ranges: LineRange[] = parsedEntry.lineRanges ?? [];
        ...
        if (filePath) {
          ...
          if (parsedEntry.head !== undefined) {
            entry.head = parsedEntry.head;
          } else if (headStr && !isNaN(parseInt(headStr))) {
            entry.head = parseInt(headStr);
          }
          if (parsedEntry.tail !== undefined) {
            entry.tail = parsedEntry.tail;
          } else if (tailStr && !isNaN(parseInt(tailStr))) {
            entry.tail = parseInt(tailStr);
          }
  ...
  async execute(
    ...
    const rawFileEntries = Array.isArray(params?.files) ? params.files : [];
    const fileEntries = rawFileEntries
      .filter((entry): entry is FileEntry & { path: string } => {
        return (
          !!entry &&
          typeof entry === "object" &&
    ...
    const useNative =
      isNativeProtocol(protocol) ||
      protocol === "unified" ||
      protocol === "markdown";

    if (!fileEntries || fileEntries.length === 0) {
      task.consecutiveMistakeCount++;
      task.recordToolError("read");
      const errorMsg = await task.sayAndCreateMissingParamError("read", "path");
    ...
    for (const entry of fileEntries) {
      const { resolvedPath, notice } = await resolveRecursivePath(
        task.cwd,
        entry.path,
      );
    ...
    try {
      ...
      for (const fileResult of fileResults) {
        ...
        if (
          ...
          (fileResult.head || fileResult.tail)
        ) {
          try {
            const totalLines = await countFileLines(fullPath);
            fileResult.lineRanges = [];
            if (fileResult.head) {
              fileResult.lineRanges.push({
                start: 1,
                end: Math.min(fileResult.head, totalLines),
              });
            }
            if (fileResult.tail) {
              const start = Math.max(1, totalLines - fileResult.tail + 1);
              const lastRange =
                fileResult.lineRanges[fileResult.lineRanges.length - 1];
        ...
        if (validationError) {
          Object.assign(fileResult, {
            ...
            nativeContent: wrapNativeReadResult(
              relPath,
              `Error: ${validationError.error}`,
            ),
          });
      ...
      for (const fileResult of fileResults) {
        ...
        try {
          ...
          if (fileResult.lineRanges && fileResult.lineRanges.length > 0) {
            ...
            const fullFileContent = await fs.promises.readFile(
              fullPath,
              "utf8",
            );
            const fullFileLines = fullFileContent.split(/\r?\n/);
            task.luxurySpa.injectFreshContent(relPath, fullFileLines);
            // Track the line ranges that were read
            task.luxurySpa.mergeLineRanges(relPath, fileResult.lineRanges);

            for (const range of fileResult.lineRanges) {
              // Read raw content
              const rawContent = await readLines(
                fullPath,
                range.end - 1,
                range.start - 1,
              );
              ...
              const finalContentLines = rawLines.map(
                (line, idx) => `${range.start + idx}→${line}`,
              );
            ...
            let nativeContent = wrapNativeReadResult(
              relPath,
              joinNativeReadSections(nativeRangeResults),
            );
          ...
          if (maxReadFileLine > 0 && totalLines > maxReadFileLine) {
            ...
            // Read raw content
            const contentRaw = await readLines(
              fullPath,
              maxReadFileLine - 1,
              0,
            );
            ...
            Object.assign(fileResult, {
              xmlContent: `<file><path>${relPath}</path>\n${xmlInfo}</file>`,
              nativeContent: wrapNativeReadResult(relPath, nativeInfo),
            });
          ...
          if (
            ...
            if (content.length > 0) {
              ...
              const truncFinal = truncRawLines.map(
                (line, idx) => `${idx + 1}→${line}`,
              );
            ...
            if (totalLines === 0) {
              ...
              const rawLines =
                stripLineNumbers(contentWithNumbers).split(/\r?\n/);
      ...
      if (useNative) {
        ...
        finalResult = joinNativeReadResults(nativeResults);
      } else {
      ...
      if (useNative) {
        const nativeResults = fileResults
          ...
          .map((result) => result.nativeContent)
          .filter((result): result is string => Boolean(result));
        errorResult = joinNativeReadResults(nativeResults);
      } else {
        const xmlResults = fileResults
          .filter((result) => result.xmlContent)
          .map((result) => result.xmlContent);
  ...
  getReadFileToolDescription(blockName: string, second: any): string {
    ...
    if (blockParams?.args) {
      try {
        ...
        console.error("Failed to parse read args XML for description:", error);
  ...
  override async handlePartial(
    ...
    if (!filePath && legacyPath) {
      const parsedEntry = convertFileEntries([legacyPath])[0];
      if (parsedEntry?.path) {
        filePath = parsedEntry.path;
        if (parsedEntry.lineRanges && parsedEntry.lineRanges.length > 0) {
          parsedLineSnippet = parsedEntry.lineRanges
            .map((range) =>
              t("tools:readFile.linesRange", {
                start: range.start,
                end: range.end,
    ...
    const sharedMessageProps: ClineSayTool = {
      ...
      lineNumber: parsedLineSnippet
        ? parseInt(parsedLineSnippet.match(/\d+/)?.[0] || "")
        : undefined,
      id: block.id,
    };
    ...
    await task
      .say("tool", partialMessage, undefined, block.partial)
      .catch(() => {});
  }
}
