...
export class AttemptCompletionTool extends BaseTool<"attempt_completion"> {
	...
	async execute(params: AttemptCompletionParams, task: Task, callbacks: AttemptCompletionCallbacks): Promise<void> {
		...
		try {
			...
			// Stop the "Thinking" indicator by updating the api_req_started message with cost
			// The cost update in AgentLoop now handles stopping the spinner
			console.log("[AttemptCompletionTool] Showing completion_result ask")
			...
			// 3. Starting an entirely new request
			// By saying "continue the task and attempt completion again", we force the AI
			// to loop back to the original task instead of responding to what the user said.
	...
	override async handlePartial(task: Task, block: ToolUse<"attempt_completion">): Promise<void> {
		...
		if (command) {
			if (lastMessage && lastMessage.ask === "command") {
				await task
					.say("command", this.removeClosingTag("command", command, block.partial), undefined, block.partial)
					.catch(() => { })
			} else {
				await task.say(
					"completion_result",
