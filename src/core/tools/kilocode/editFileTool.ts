...
export async function editFileTool(
	...
	try {
		...
		if (block.partial) {
			...
			await cline.say("tool", JSON.stringify(partialMessageProps), undefined, block.partial).catch(() => {
		...
		const diffStats = unifiedPatch ? (await import("../../diff/stats")).computeDiffStats(unifiedPatch) : undefined

		// Ensure diffStats matches expected type
		const formattedDiffStats = diffStats ? { added: diffStats.added, removed: diffStats.removed } : undefined
		...
		// Invalidate hot cache so next refresh reads fresh content from disk
		cline.luxurySpa.invalidateHotCache(relPath)
