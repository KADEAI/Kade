...
export async function newRuleTool(
	...
	try {
		if (block.partial) {
			...
			await cline.say("tool", partialMessage, undefined, block.partial).catch(() => { })
			...
			if (!cline.diffViewProvider.isEditing) {
				...
				const partialMessage = JSON.stringify(sharedMessageProps)
				await cline.say("tool", partialMessage, undefined, true).catch(() => { }) // sending true for partial even though it's not a partial, cline shows the edit row before the content is streamed into the editor
				await cline.diffViewProvider.open(relPath)
			}
			...
			const unifiedPatchRaw = fileExists
				? formatResponse.createPrettyPatch(relPath, cline.diffViewProvider.originalContent, newContent)
				: (await import("../../diff/stats")).convertNewFileToUnifiedDiff(newContent, relPath)
			const unifiedPatch = (await import("../../diff/stats")).sanitizeUnifiedDiff(unifiedPatchRaw)
			const diffStats = (await import("../../diff/stats")).computeDiffStats(unifiedPatch) || undefined
