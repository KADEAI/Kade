...
export async function browserActionTool(
	...
	try {
		if (block.partial) {
			if (action === "launch") {
				await cline.say("browser_action_launch", removeClosingTag("url", url), undefined, block.partial).catch(() => { })
			} else {
				await cline.say(
					"browser_action",
					JSON.stringify({
			...
			switch (action) {
				...
				case "screenshot": {
					...
					messageText += `\n3. Use format: x,y@WIDTHxHEIGHT (e.g. 500,300@1000x625) where WIDTHxHEIGHT is the EXACT pixel size of the screenshot image`
					...
					messageText += `\nExample: click (500,300) on a 1000x625 screenshot -> 500,300@1000x625`
