// kade_change - new file: Speech-to-text availability check (extracted from ClineProvider)
 ...
 */
let cachedResult: { available: boolean; reason?: "apiKeyMissing" | "ffmpegNotInstalled"; timestamp: number } | null =
	null
 ...
 * 1. OpenAI or Gemini API key is configured
...
export async function checkSpeechToTextAvailable(
	...
	try {
		// Check 1: API key (OpenAI or Gemini)
		const openAiKey = await getOpenAiApiKey(providerSettingsManager)
		const geminiKey = await getGeminiApiKey(providerSettingsManager)

		if (!openAiKey && !geminiKey) {
			cachedResult = { available: false, reason: "apiKeyMissing", timestamp: Date.now() }
			return { available: false, reason: "apiKeyMissing" }
		}
