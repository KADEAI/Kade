...
export const webviewMessageHandler = async (
	...
	if (message.type === "subAgentToolEnabled" && typeof message.bool === "boolean") {
		await updateGlobalState("subAgentToolEnabled", message.bool)
		await provider.postStateToWebview()
	}

	if (message.type === "showSubAgentBanner" && typeof message.bool === "boolean") {
	...
	switch (message.type) {
		...
		case "newTask":
			...
			try {
				...
				void provider.context.workspaceState.update("claudix.acceptedToolIds", []).then(undefined, (error: unknown) => {
					console.warn("Failed to clear accepted tool IDs for new task:", error)
				})
			} catch (error) {
				// Show error to user
				vscode.window.showErrorMessage(
		...
		case "command":
			if (message.command === 'claudix.undoEdits' && message.args) {
				try {
					console.log('[webviewMessageHandler] Undo command received:', message.args);
					const { EditHistoryService } = await import("../../services/edit-history/EditHistoryService");
					const toolUseIds = message.args[0]; // Expecting [[id1, id2...]] or [id]
					if (Array.isArray(toolUseIds)) {
						const revertedFiles = await EditHistoryService.getInstance().undo(toolUseIds);
						console.log('[webviewMessageHandler] Undo successful, reverted:', revertedFiles);
					...
					vscode.window.showErrorMessage(`Undo failed: ${e instanceof Error ? e.message : String(e)}`);
				}
			} else if (message.command === 'claudix.redoEdits' && message.args) {
				try {
					console.log('[webviewMessageHandler] Redo command received:', message.args);
					const { EditHistoryService } = await import("../../services/edit-history/EditHistoryService");
					const toolUseIds = message.args[0]; // Expecting [[id1, id2...]] or [id]
					if (Array.isArray(toolUseIds)) {
						const redoneFiles = await EditHistoryService.getInstance().redo(toolUseIds);
						console.log('[webviewMessageHandler] Redo successful, redone:', redoneFiles);
						// Kilo Code Change: Notification is now handled client-side in the webview for a better UX.
					}
				} catch (e) {
					console.error('[webviewMessageHandler] Redo failed:', e);
					vscode.window.showErrorMessage(`Redo failed: ${e instanceof Error ? e.message : String(e)}`);
				}
				...
				const loginProvider = (message as any).args?.[0] || (message as any).provider;
				console.log(`[webviewMessageHandler] Provider: ${loginProvider}`);

				const args: string[] = []
				if (loginProvider === "google") args.push("-login")
				else if (loginProvider === "antigravity") args.push("-antigravity-login")
					...
					vscode.window.showErrorMessage(`Unknown login provider category: ${loginProvider}`)
					console.error(`[webviewMessageHandler] Unknown provider: ${loginProvider}`);
					return
				}

				let executablePath = ""
				const cwd = (await provider.getState()).cwd || vscode.workspace.workspaceFolders?.[0]?.uri.fsPath || "";
				...
				for (const candidate of candidates) {
					...
					console.log(`[webviewMessageHandler] Checking candidate: ${candidate} -> Exists: ${exists}`);
					if (exists) {
				...
				try {
					...
					if (!configExists && exampleExists) {
						console.log(`[webviewMessageHandler] Copying config.example.yaml to config.yaml`);
						await fs.copyFile(configExamplePath, configPath);
						vscode.window.showInformationMessage("Created default config.yaml for CLI Proxy.");
					}
				} catch (e) {
					console.error('[webviewMessageHandler] Failed to setup config.yaml:', e);
				}

				try {
					console.log(`[webviewMessageHandler] Spawning: ${executablePath} with args: ${args.join(" ")}`);
					const { spawn } = await import("child_process")
					vscode.window.showInformationMessage(`Launching CLI: ${executablePath} ${args.join(" ")}`)
					...
					child.on("error", (err: any) => {
						...
						vscode.window.showErrorMessage(`Failed to start CLI Proxy process: ${err.message}`)
					})

					child.on("close", (code: any) => {
						console.log(`[CLI Proxy] Closed with code: ${code}`);
						if (code === 0) vscode.window.showInformationMessage(`Login flow for ${loginProvider} finished.`)
						else vscode.window.showWarningMessage(`Login flow exited with code ${code}. Check output.`)
					})
				} catch (err: any) {
					console.error(`[webviewMessageHandler] Catch error:`, err);
					vscode.window.showErrorMessage(`Exception spawning CLI Proxy: ${err.message}`)
				}
			}
			break;
		...
		case "cliProxyLogin": {
			...
			console.log(`[webviewMessageHandler] Opening 9Router Dashboard: ${dashboardUrl}`);
		...
		case "geminiCliSignIn": {
			try {
				...
				const { apiConfiguration: geminiConfig, currentApiConfigName: geminiConfigName = "default" } = await provider.getState()
				await provider.upsertProviderProfile(geminiConfigName, { ...geminiConfig, apiProvider: "gemini-cli" })
				vscode.window.showInformationMessage("Gemini CLI authentication successful")
			} catch (error) {
				vscode.window.showErrorMessage(
		...
		case "claudeCodeSignIn": {
			try {
				...
				// Persist provider selection to active profile so settings don't glitch on fresh install
				const { apiConfiguration: claudeConfig, currentApiConfigName: claudeConfigName = "default" } = await provider.getState()
				await provider.upsertProviderProfile(claudeConfigName, { ...claudeConfig, apiProvider: "claude-code" })
				vscode.window.showInformationMessage("Claude Code authentication successful")
			} catch (error) {
				vscode.window.showErrorMessage(
		...
		case "requestCliProxyModels": {
			...
			try {
				...
				const response = await axios.get(`${baseUrl}/v1/models`, {
					headers: (apiConfiguration as any).cliProxyApiKey ? { 'Authorization': `Bearer ${(apiConfiguration as any).cliProxyApiKey}` } : {}
				})
				const models = response.data?.data?.reduce((acc: any, model: any) => {
					acc[model.id] = { id: model.id, info: model }
					return acc
				}, {})
				await provider.postMessageToWebview({ type: "cliProxyModels", cliProxyModels: models })
			} catch (error) {
				console.log("[webviewMessageHandler] 9Router not running or unauthorized, attempting to resolve...");

				// If 401/403, we need an API Key
				if (axios.isAxiosError(error) && (error.response?.status === 401 || error.response?.status === 403)) {
					vscode.window.showErrorMessage("9Router Error: Unauthorized. Please check your API Key in Advanced settings.");
					return;
				}
				...
				if (await fileExistsAtPath(routerPath)) {
					try {
						...
						console.log(`[webviewMessageHandler] Starting 9Router in ${routerPath}`);
						vscode.window.showInformationMessage("Starting 9Router background server...");
						...
						try {
							...
							console.error("Failed to fetch models even after starting 9Router:", retryError)
						}
					} catch (spawnError) {
				...
				await provider.postMessageToWebview({ type: "cliProxyModels", cliProxyModels: {} })
			}
			break
		}
		...
		case "updateSettings":
			if (message.updatedSettings) {
				...
				// provider.log(`[PERSISTENCE_DEBUG][updateSettings] Has currentTask: ${!!currentTask}`)
				...
				for (const [key, value] of Object.entries(message.updatedSettings)) {
					...
					if (currentTask && key in currentTask.apiConfiguration) {
						...
						if (key === "apiProvider" || key === "apiModelId" || key === "openRouterModelId" || key === "kilocodeModel") {
							needsRebuild = true
					...
					const isApiField = key === "apiProvider" || key === "apiModelId" || key === "openRouterModelId" || key === "kilocodeModel"
					if (!currentTask || !isApiField) {
						// Handle special cases with side effects
						if (key === "subAgentApiConfiguration") {
							provider.log(`[webviewMessageHandler] Updating subAgentApiConfiguration: ${JSON.stringify(value)}`)
							await provider.contextProxy.setValue("subAgentApiConfiguration" as any, value)
							...
							await vscode.workspace
								...
								.update("allowedCommands", settingValue, vscode.ConfigurationTarget.Global)
							await provider.contextProxy.setValue(key as any, settingValue)
							...
							await vscode.workspace
								.getConfiguration(Package.name)
								.update("deniedCommands", settingValue, vscode.ConfigurationTarget.Global)
							await provider.contextProxy.setValue(key as any, settingValue)
						...
						if (key === "enabledSkills" && currentTask) {
		...
		case "openDiff":
			if (message.text) {
				...
				if (message.toolId) {
					try {
						const { EditHistoryService } = await import("../../services/edit-history/EditHistoryService")
						const snapshots = EditHistoryService.getInstance().getSnapshots(message.toolId)
						const snapshot = snapshots.find((s) => arePathsEqual(s.filePath, filePath))
						...
						if (snapshot) {
							...
							await openDiff(filePath, message.edits || [], snapshot.originalContent)
						} else {
							...
							if (currentTask) {
								...
								if (activeRelPath && arePathsEqual(path.resolve(getCurrentCwd(), activeRelPath), filePath)) {
									// It's the currently active edit! Show as proposed.
									await openDiff(filePath, message.edits || [], activeNewContent, true)
		...
		case "openFile":
			...
			openFile(filePath, message.values as { create?: boolean; content?: string; line?: number; endLine?: number })
			break
		case "openMention":
		...
		case "openAiCodexSignIn": {
			try {
				...
				openAiCodexOAuthManager
					...
					.then(async () => {
						// Persist provider selection to active profile so settings don't glitch on fresh install
						const { apiConfiguration: codexConfig, currentApiConfigName: codexConfigName = "default" } = await provider.getState()
						await provider.upsertProviderProfile(codexConfigName, { ...codexConfig, apiProvider: "openai-codex" })
		...
		case "antigravitySignIn": {
			try {
				...
				// Persist provider selection to active profile so settings don't glitch on fresh install
				const { apiConfiguration: antigravityConfig, currentApiConfigName: antigravityConfigName = "default" } = await provider.getState()
				await provider.upsertProviderProfile(antigravityConfigName, { ...antigravityConfig, apiProvider: "antigravity" })
				vscode.window.showInformationMessage(
		...
		case "collapseCodeToolsByDefault":
			await updateGlobalState("collapseCodeToolsByDefault", message.bool ?? false)
			await provider.debouncedPostStateToWebview()
			break
			...
			if (message.text && message.apiConfiguration) {
				...
				if (message.scope !== "task") {
					...
					if (currentTask) {
						...
						const taskIndex = history.findIndex((item) => item.id === currentTask.taskId)
						if (taskIndex !== -1) {
							const updatedHistory = [...history]
							updatedHistory[taskIndex] = {
								...updatedHistory[taskIndex],
								apiConfiguration: { ...currentTask.apiConfiguration, ...configToSave }
							} as any
		...
		case "updateCustomMode":
			if (message.modeConfig) {
				try {
					...
					const existingModes = provider.customModesManager ? await provider.customModesManager.getCustomModes() : []
					const isNewMode = !existingModes.some((mode) => mode.slug === message.modeConfig?.slug)

					await provider.customModesManager.updateCustomMode(message.modeConfig.slug, message.modeConfig)
					// Update state after saving the mode
					const customModes = provider.customModesManager ? await provider.customModesManager.getCustomModes() : []
					await updateGlobalState("customModes", customModes)
		...
		case "telemetrySetting": {
			...
			if (telemetrySetting !== "unset") {
				...
				const firstInstallUpsells = ["welcome", "provider", "toolProtocol", "telemetry"].map(id => `first_install_${id}`)
				const updatedDismissed = Array.from(new Set([...currentDismissed, ...firstInstallUpsells]))
		...
		case "requestMicrophonePermission":
			...
			await vscode.commands.executeCommand("kilo-code.requestMicrophonePermission")
			break
		default: {
