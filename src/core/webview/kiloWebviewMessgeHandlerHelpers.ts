...
export async function fetchAndRefreshOrganizationModesOnStartup(
	...
	if (startupState.apiConfiguration.kilocodeToken && startupState.apiConfiguration.kilocodeOrganizationId && provider.customModesManager) {
		try {
			const orgModes = await provider.customModesManager.fetchOrganizationModes(
			...
			if (orgModes.length > 0 && provider.customModesManager) {
				await provider.customModesManager.refreshWithOrganizationModes(orgModes)
				const updatedCustomModes = provider.customModesManager ? await provider.customModesManager.getCustomModes() : []
