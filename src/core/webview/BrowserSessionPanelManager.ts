...
export class BrowserSessionPanelManager {
	...
	/**
	 * Send a live screenshot to the panel without updating the full message history.
	 * This is used for "Live Action" feedback during long operations.
	 */
	public async updateLiveScreenshot(screenshot: string): Promise<void> {
		if (!this.panel || !this.isReady) {
			return
		}

		await this.panel.webview.postMessage({
			type: "browserLiveScreenshot",
			screenshot,
		})
	}
