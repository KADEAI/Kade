...
export function getVitePort(): string {
	try {
		// Look for .vite-port in the extension's root directory.
		// In development, __dirname is in src/core/webview.
		// In production, it's in dist/core/webview.
		...
		const portFilePath = path.resolve(currentPath, ".vite-port")
...
export function getDevCsp(webview: Webview, localServerUrl: string, port: string, nonce: string): string[] {
	return [
		...
		`img-src ${webview.cspSource} https://storage.googleapis.com https://img.clerk.com data: https://*.googleusercontent.com https://*.googleapis.com https://*.githubusercontent.com https://*.google.com https://icons.duckduckgo.com https://registry.npmmirror.com https://*.lobehub.com`,
...
export async function getViteDevServerConfig(webview: Webview): Promise<ViteDevServerConfig | null> {
	...
	const localServerUrl = `127.0.0.1:${port}`
