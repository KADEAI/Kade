...
describe("RooIgnoreController", () => {
	...
	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .kadeignore exists
		 */
		it("should load .kadeignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .kadeignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".kadeignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".kadeignore"), "utf8")
		...
		/**
		 * Tests the controller behavior when .kadeignore doesn't exist
		 */
		it("should allow all access when .kadeignore doesn't exist", async () => {
			// Setup mocks to simulate missing .kadeignore file
		 ...
		 */
		it("should set up file watcher for .kadeignore changes", async () => {
		 ...
		 */
		it("should handle errors when loading .kadeignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .kadeignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .kadeignore exists
		 */
		it("should allow all access when no .kadeignore content", async () => {
			// Create a new controller with no .kadeignore
			mockFileExists.mockResolvedValue(false)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .kadeignore exists
		 */
		it("should allow all commands when no .kadeignore exists", async () => {
			// Create a new controller with no .kadeignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new RooIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .kadeignore
		 */
		it("should generate formatted instructions when .kadeignore exists", async () => {
			// Setup .kadeignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
			...
			expect(instructions).toContain("# .kadeignore")
		 ...
		 * Tests behavior when no .kadeignore exists
		 */
		it("should return undefined when no .kadeignore exists", async () => {
			// Setup no .kadeignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .kadeignore is created
		 */
		it("should reload .kadeignore when file is created", async () => {
			// Setup initial state without .kadeignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .kadeignore
			controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .kadeignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .kadeignore is changed
		 */
		it("should reload .kadeignore when file is changed", async () => {
			// Setup initial state with .kadeignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .kadeignore is deleted
		 */
		it("should reset when .kadeignore is deleted", async () => {
			// Setup initial state with .kadeignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
