...
Your task is to create a detailed summary of the conversation so far. This summary serves as the foundation for continuing the session, so it must capture the current focus, technical context, and overarching narrative.

**CRITICAL: FOCUS ON CONVERSATIONAL CONTINUITY.**
Do not just list "pending tasks". Instead, explain what the user is TRYING TO ACHIEVE right now and how the conversation has evolved to this point.

Your summary should be structured as follows:

1. **Current Focus & Context**: What is the user's primary goal at this moment? What specific problem or feature is under active discussion?
2. **Conversation Evolution**: Briefly trace how the conversation led to the current state (e.g., "Started with X, pivoted to Y after discovering Z").
3. **Core Technical State**: Key technologies, code patterns, and architectural decisions that are settled and should be maintained.
4. **Relevant Files**: Mention specifically which files are currently relevant to the active discussion.
5. **Pending Intent**: What did the user last ask for that hasn't been addressed yet? What are the immediate next steps to move the current work forward?

**Note on "Tasks":** Avoid the "moth-to-lamp" trap. If the original task is completed or has been superseded by a new request, DO NOT emphasize it. Focus on what is relevant NOW.

Output only the summary, without any additional commentary or explanation.
`
...
export async function summarizeConversation(
	...
	// Tag middle messages with condenseParent (skip last N messages)
	const newMessages = messages.map((msg, index) => {
		// Messages in the "keep" range stay as-is
		if (index >= keepStartIndex) {
			return msg
		}
		// Messages get tagged with condenseParent (unless they already have one from a previous condense)
...
export function getMessagesSinceLastSummary(
	messages: ApiMessage[],
	customContextAnchor?: ApiMessage,
): ApiMessage[] {
