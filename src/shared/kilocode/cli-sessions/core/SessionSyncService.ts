...
export class SessionSyncService {
	...
	private async uploadBlobs(
		...
		for (const blobName of blobNames) {
			...
			// Trigger title generation for ui_messages to keep it updated throughout the session
if (blobName === "ui_messages") {
	this.logger.debug("Updating session title from sync", LOG_SOURCES.SESSION_SYNC, { sessionId })
