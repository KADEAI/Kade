...
export class SessionPersistenceManager {
	...
	private readWorkspaceState(): WorkspaceSessionState {
		...
		try {
			...
			if (!content.trim()) {
				// File is empty, return default state to avoid JSON.parse error
				return { taskSessionMap: {} }
			}
			...
			// Log the error but don't crash the extension. This can happen if the state file is corrupted.
			console.error(
				`[SessionPersistenceManager] Failed to parse session state from ${statePath}. Returning default state.`,
				error,
			)
			return { taskSessionMap: {} }
		}
