...
export type NativeToolArgs = {
	...
	list_dir: { path: string; recursive?: boolean }
	grep: { path?: string; query?: string | string[]; command?: string; file_pattern?: string | null; context_lines?: number; literal?: boolean; include?: string | null; include_all?: boolean; exclude?: string | null; whole_word?: boolean; case_sensitive?: boolean; mode?: "text" | "refs" | "all_refs" }
	...
	wrap: { effect?: string; emotion?: string; gui?: string; color?: string; bg?: string; border?: string; shadow?: string; style?: string; intensity?: string; content?: string }
	mkdir: { path: string }
...
export interface GrepToolUse extends ToolUse<"grep"> {
	name: "grep"
	params: Partial<Pick<Record<ToolParamName, string>, "path" | "query" | "command" | "file_pattern" | "context_lines" | "literal" | "include" | "exclude">>
}

export interface ListDirToolUse extends ToolUse<"list_dir"> {
	name: "list_dir"
...
export interface WebSearchToolUse extends ToolUse<"web_search"> {
	name: "web_search"
	params: Partial<Pick<Record<ToolParamName, string>, "query" | "max_results" | "allowed_domains" | "blocked_domains">>
}

export interface WebFetchToolUse extends ToolUse<"web_fetch"> {
	name: "web_fetch"
...
export const TOOL_GROUPS: Record<ToolGroup, ToolGroupConfig> = {
	read: {
		tools: ["read_file", "fetch_instructions", "grep", "list_dir", "glob", "codebase_search", "fast_context"],
	},
	edit: {
		tools: [
			"edit",
	...
	external: {
		tools: ["web_search", "web_fetch", "research_web"],
	}
 ...
 *
 * This allows models to use alternative names for tools (e.g., "write_file" instead of "write_to_file").
