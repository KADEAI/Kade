		...
		// Handle command sequences
			...
			while (j < messages.length) {
				...
				if (ask === "command_output" || say === "command_output") {
					...
					const isExactDuplicate = previous && previous.text === text
					const isCumulativeOutput = previous && text.startsWith(previous.text)
					const nextChunk = isCumulativeOutput && previous ? text.slice(previous.text.length) : text

					if (nextChunk.length > 0 && !isExactDuplicate) {
