...
describe("ExecaTerminalProcess", () => {
	...
	describe("getUnretrievedOutput", () => {
		it("returns partial output when no trailing newline is present", () => {
			terminalProcess["fullOutput"] = "Local: http://localhost:3000"
			terminalProcess["lastRetrievedIndex"] = 0

			const unretrieved = terminalProcess.getUnretrievedOutput()

			expect(unretrieved).toBe("Local: http://localhost:3000")
			expect(terminalProcess["lastRetrievedIndex"]).toBe(terminalProcess["fullOutput"].length)
		})

		it("treats carriage returns as flush boundaries", () => {
			terminalProcess["fullOutput"] = "Starting dev server...\rReady"
			terminalProcess["lastRetrievedIndex"] = 0

			const unretrieved = terminalProcess.getUnretrievedOutput()

			expect(unretrieved).toBe("Starting dev server...\r")
			expect(terminalProcess["lastRetrievedIndex"]).toBe("Starting dev server...\r".length)
		})
	})
})
