...
export class TerminalProcess extends BaseTerminalProcess {
  private static readonly abortShellExecutionTimeoutMs = 500;
  private static readonly lineEmitThrottleMs = 100;
  ...
  public override async run(command: string) {
    ...
    const streamAvailable = new Promise<AsyncIterable<string>>(
      (resolve, reject) => {
    ...
    const isPowerShell =
      ...
      (defaultWindowsShellProfile === null ||
        (defaultWindowsShellProfile as string)
          ?.toLowerCase()
          .includes("powershell"));
    ...
    if (isPowerShell) {
      ...
      // AI-habit correction: PowerShell < 7 does not support '&&' or '||' as statement separators.
      // Replaces these with ';' to prevent syntax errors, which is the most common intent.
      // Using a regex to match &&/|| outside of quotes would be ideal, but even a simple
      // replacement for standalone operators covers 99% of AI failures.
      if (commandToExecute.includes(" && ")) {
        commandToExecute = commandToExecute.replace(/ && /g, " ; ");
      }
      if (commandToExecute.includes(" || ")) {
        commandToExecute = commandToExecute.replace(/ \|\| /g, " ; ");
      }
    ...
    for await (let data of stream) {
      ...
      if (
        this.isListening &&
        (now - this.lastEmitTime_ms > TerminalProcess.lineEmitThrottleMs ||
      ...
      // Check for command end markers (OSC 633;D or OSC 133;D) to explicitly detect exit code
      // This is a fallback for when onDidEndTerminalShellExecution event is not fired (e.g. some macOS versions)
      const exitCodeMatch = data.match(/\x1b\](?:633|133);D;?(\d+)?\x07/);
      if (exitCodeMatch) {
        const exitCode = exitCodeMatch[1] ? parseInt(exitCodeMatch[1], 10) : 0;
        this.terminal.shellExecutionComplete({ exitCode });
      }
    }
    ...
    if (this.abortRequested && !shellExecutionCompleted) {
      await Promise.race([
        shellExecutionComplete,
        new Promise<void>((resolve) =>
          setTimeout(resolve, TerminalProcess.abortShellExecutionTimeoutMs),
        ),
      ]);

      if (!shellExecutionCompleted) {
    ...
    if (commandOutputStarted) {
      ...
      console.error(
        `[Terminal Process] ${errorMsg} preOutput: ${inspectPreOutput}`,
      );
  ...
  public override async write(data: string) {
    ...
    this.terminal.terminal.sendText(data, false);
  }

  public override abort() {
    this.abortRequested = true;
    // Allow stop to work even after continue() has moved the process into the
    // background. VS Code still needs the raw Ctrl+C to reach the foreground
    // shell command.
    this.terminal.terminal.sendText("\x03", false);
  }
  ...
  private scheduleBufferedEmit() {
    ...
    const delay = Math.max(
      0,
      TerminalProcess.lineEmitThrottleMs - (Date.now() - this.lastEmitTime_ms),
    );
  ...
  private removeEscapeSequences(str: string): string {
    ...
    return stripAnsi(
      str
        .replace(/\x1b\]633;[^\x07]+\x07/gs, "")
        .replace(/\x1b\]133;[^\x07]+\x07/gs, ""),
    );
  }
