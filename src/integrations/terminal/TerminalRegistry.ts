...
import { RooTerminal, RooTerminalProcess, RooTerminalProvider } from "./types";
...
export class TerminalRegistry {
  ...
  private static executionProcesses = new Map<string, RooTerminalProcess>();
  ...
  public static registerProcessForExecution(
    executionId: string,
    process: RooTerminalProcess,
  ): void {
    this.executionProcesses.set(executionId, process);
  }

  public static getProcessByExecutionId(
    executionId: string,
  ): RooTerminalProcess | undefined {
    return this.executionProcesses.get(executionId);
  }

  public static clearProcessForExecution(
    executionId: string,
    process?: RooTerminalProcess,
  ): void {
    const registeredProcess = this.executionProcesses.get(executionId);

    if (!registeredProcess) {
      return;
    }

    if (process && registeredProcess !== process) {
      return;
    }

    this.executionProcesses.delete(executionId);
  }
    ...
    return this.getAllTerminals().filter((t) => {
      ...
      if (busy === undefined) {
        return (
          ...
          t.process?.hasUnretrievedOutput()
        );
      }
  ...
  private static getTerminalByVSCETerminal(
    vsceTerminal: vscode.Terminal,
  ): RooTerminal | undefined {
    const found = this.terminals.find(
      (t) => t instanceof Terminal && t.terminal === vsceTerminal,
    );
