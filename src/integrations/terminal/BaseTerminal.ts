import { truncateOutput, applyRunLengthEncoding, processBackspaces, processCarriageReturns, compressWhitespace, cleanLsOutput, removeCommandBoilerplate } from "../misc/extract-text"
...
export abstract class BaseTerminal implements RooTerminal {
	...
	public static compressTerminalOutput(input: string, lineLimit: number, characterLimit?: number): string {
		...
		const compressed = applyRunLengthEncoding(processedInput)
		
		// Clean up ls -l output
		const lsCleaned = cleanLsOutput(compressed)
		
		// Remove git/npm boilerplate and hints
		const boilerplateRemoved = removeCommandBoilerplate(lsCleaned)
		
		// Compress whitespace to save tokens while preserving readability
		const whitespaceCompressed = compressWhitespace(boilerplateRemoved)

		return truncateOutput(whitespaceCompressed, lineLimit, effectiveCharLimit)
	}
