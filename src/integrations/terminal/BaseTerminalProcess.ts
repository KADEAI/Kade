...
export abstract class BaseTerminalProcess
  extends EventEmitter<RooTerminalProcessEvents>
  implements RooTerminalProcess
{
  ...
  /**
   * Writes raw stdin data to the running process.
  ...
  protected startHotTimer(data: string) {
    ...
    this.hotTimer = setTimeout(
      () => (this.isHot = false),
      BaseTerminalProcess.isCompiling(data) ? 15_000 : 2_000,
    );
  }
  ...
  private static isCompiling(data: string): boolean {
    return (
      BaseTerminalProcess.compilingMarkers.some((marker) =>
