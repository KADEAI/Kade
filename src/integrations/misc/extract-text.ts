...
export function addLineNumbers(content: string, startLine: number = 1): string {
	...
	// Returns the numbered content. The XML preserve tag was removed per user request.
...
export function everyLineHasLineNumbers(content: string): boolean {
	...
	return lines.length > 0 && lines.every((line) => line.trim() === "" || /^\d+→/.test(line))
}
 ...
 *                   When true: Uses a more lenient pattern that also matches lines with just an arrow,
...
export function stripLineNumbers(content: string, aggressive: boolean = false): string {
	...
	const processedLines = lines.map((line) => {
		// Match line number pattern with arrow separator and capture everything after
...
/**
 * Smartly compresses whitespace in terminal output to save tokens while preserving readability.
 * - Preserves leading whitespace (indentation)
 * - Replaces multiple consecutive spaces/tabs between words with a single space
 * - Removes excessive blank lines
 */
export function compressWhitespace(content: string): string {
	if (!content) return content

	return content
		.split(/\r?\n/)
		.map(line => {
			// Match leading whitespace and the rest of the line
			const match = line.match(/^([ \t]*)(.*)$/)
			if (!match) return line
			
			const [, leading, rest] = match
			// Replace multiple spaces/tabs in the rest of the line with a single space
			const compressedRest = rest.replace(/[ \t]{2,}/g, ' ')
			return leading + compressedRest
		})
		// Remove multiple consecutive empty lines, keeping at most one
		.join('\n')
		.replace(/\n{3,}/g, '\n\n')
}

/**
 * Simplifies ls -l output by removing owner, group, and long permissions columns,
 * which are rarely useful to the AI and consume a lot of tokens.
 * 
 * Example:
 * -rw-r--r--@  1 imacpro  staff   8196 Feb 21 18:58 .DS_Store
 * becomes:
 * [-] 8196 Feb 21 18:58 .DS_Store
 */
export function cleanLsOutput(content: string): string {
	if (!content) return content

	return content.split(/\r?\n/).filter(line => {
		// Remove the useless "total 123" line at the start of ls output
		return !/^total\s+\d+$/.test(line.trim())
	}).map(line => {
		// Matches standard macOS/Linux ls -l output
		// 1: type (d, -, l, etc.)
		// 2: size
		// 3: date (Month Day Time/Year)
		// 4: filename
		const lsRegex = /^([dcbplss-])[rwx-]{9}[@\+\.]?\s+\d+\s+\S+\s+\S+\s+(\d+)\s+([A-Z][a-z]{2}\s+\d+\s+[\d:]+)\s+(.*)$/
		const match = line.match(lsRegex)
		
		if (match) {
			const [, type, size, date, name] = match
			// Compress multiple spaces in the date part (e.g. "Mar  1" -> "Mar 1")
			const cleanDate = date.replace(/\s+/g, ' ')
			return `[${type}] ${size} ${cleanDate} ${name}`
		}
...
/**
 * Removes common boilerplate and noisy hints from command output (e.g., git, npm)
 * that consume tokens without providing useful context to the AI.
 */
export function removeCommandBoilerplate(content: string): string {
	if (!content) return content

	return content.split(/\r?\n/).filter(line => {
		const trimmed = line.trim()
		
		// Git boilerplate hints
		if (trimmed.startsWith('(use "git ') && trimmed.endsWith(')')) return false
		if (trimmed === 'no changes added to commit (use "git add" and/or "git commit -a")') return false
		
		// NPM boilerplate
		if (trimmed.match(/^\d+ packages are looking for funding$/)) return false
		if (trimmed === 'run `npm fund` for details') return false
...
/**
 * Strips markdown code block markers from a string if the entire string is wrapped in them.
 * e.g. "```javascript\ncode\n```" -> "code"
 */
export function stripMarkdown(content: string): string {
	...
	if (trimmed.startsWith("```") && trimmed.endsWith("```")) {
		...
		if (lines.length >= 2) {
			...
			const lastIdx = findLastIndex_local(lines, (l) => l.trim().startsWith("```"))

			if (firstIdx !== -1 && lastIdx !== -1 && firstIdx < lastIdx) {
				const lineEnding = content.includes("\r\n") ? "\r\n" : "\n"
				return lines.slice(firstIdx + 1, lastIdx).join(lineEnding)
			}
		}
	}
	return content
...
function findLastIndex_local<T>(array: T[], predicate: (value: T, index: number, obj: T[]) => boolean): number {
