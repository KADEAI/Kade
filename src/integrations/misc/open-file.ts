...
export async function openFile(filePath: string, options: OpenFileOptions = {}) {
	try {
		...
		if (filePath.startsWith("./")) {
			...
			// Try workspace root first for relative paths
			if (workspaceRoot) {
				attemptPaths.push(path.join(workspaceRoot, filePath))
			}
			// Then home dir
			if (homeDir) {
				const homePath = path.join(homeDir, filePath)
				if (!attemptPaths.includes(homePath)) {
					attemptPaths.push(homePath)
				}
			}
			// Finally original
			attemptPaths.push(filePath)
		} else {
			attemptPaths.push(filePath) // Absolute path
		}

		let fileStat: vscode.FileStat | undefined
		let successfulUri: vscode.Uri | undefined

		// 1. Try standard attempt paths
		for (const p of attemptPaths) {
			try {
				const tempUri = vscode.Uri.file(p)
				fileStat = await vscode.workspace.fs.stat(tempUri)
				successfulUri = tempUri
				break
			} catch (e) { }
		}

		// 2. Fallback: If not found and looks like a relative path, search workspace
		if (!successfulUri && !path.isAbsolute(filePath)) {
			try {
				// Clean path for glob: remove leading ./ and normalize slashes
				const cleanPath = filePath.replace(/^\.\//, "").replace(/\\/g, "/")
				const files = await vscode.workspace.findFiles(`**/${cleanPath}`, null, 1)
				if (files.length > 0) {
					successfulUri = files[0]
					fileStat = await vscode.workspace.fs.stat(successfulUri)
				}
			} catch (e) {
				console.error(`Failed to find file by basename: ${filePath}`, e)
			}
		}
		...
		const startLine = options.line !== undefined ? Math.max(options.line - 1, 0) : undefined
		const endLine = options.endLine !== undefined ? Math.max(options.endLine - 1, 0) : startLine
		const selection =
			startLine !== undefined
				? new vscode.Selection(startLine, 0, endLine!, Number.MAX_SAFE_INTEGER)
				: undefined
