...
export class GeminiHandler extends BaseProvider implements SingleCompletionHandler {
			...
			: this.options.vertexKeyFile
				...
				: isVertex
					...
					: new GoogleGenAI({ 
						...
						httpOptions: {
							headers: {
								"User-Agent": `GeminiCLI/0.30.0/gemini-2.0-flash (darwin; arm64) (Pro)`,
	...
	async *createMessage(
		...
		const config: GenerateContentConfig = {
			...
			httpOptions: {
				baseUrl: this.options.googleGeminiBaseUrl,
				headers: {
					"User-Agent": `GeminiCLI/0.30.0/${model} (darwin; arm64) (Pro)`,
					"x-gemini-api-privileged-user-id": metadata?.taskId ?? "kilocode-extension",
				}
			},
			thinkingConfig,
			maxOutputTokens,
	...
	if (error instanceof Error) {
		// Handle specific Gemini error cases that might be wrapped or surfaced during streaming
		const message = error.message.toLowerCase()
		if (message.includes("safety") || message.includes("blocked")) {
			throw new Error(t("common:errors.gemini.thinking_complete_safety"))
		}
		if (message.includes("recitation")) {
			throw new Error(t("common:errors.gemini.thinking_complete_recitation"))
		}
