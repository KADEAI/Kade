...
export class OpenRouterHandler extends BaseProvider implements SingleCompletionHandler {
	...
	constructor(options: ApiHandlerOptions) {
		...
		// PERF: Start loading models immediately and cache the promise.
		// First createMessage() will await this instead of refetching.
		this.modelFetchPromise = this.loadDynamicModels().catch((error) => {
	 ...
	 * Optimized: Fast-path for common errors to reduce latency
	 */
	private handleStreamingError(error: OpenRouterErrorResponse, modelId: string, operation: string): never {
		// Fast-path: Skip expensive JSON parsing for common errors
		const errorMessage = error?.message
		if (errorMessage?.includes('rate limit') || errorMessage?.includes('too many requests')) {
			throw new Error(`Rate limit exceeded. Please try again in a moment.`)
		}
		if (errorMessage?.includes('timeout') || errorMessage?.includes('connection')) {
			throw new Error(`Connection timeout. Please check your network and retry.`)
		}
		if (errorMessage?.includes('invalid api key') || errorMessage?.includes('unauthorized')) {
			throw new Error(`Invalid API key. Please check your credentials.`)
		}

		// Full error processing for less common errors
		const rawErrorMessage = error?.metadata?.raw || errorMessage
	...
	public async fetchModel() {
		...
		// instead of starting a redundant fetch. This eliminates the first-message
		// blocking network roundtrip.
		if (this.modelFetchPromise) {
			await this.modelFetchPromise
			// Clear after first use so subsequent calls can refresh if needed
			this.modelFetchPromise = undefined
			return this.getModel()
		}

		// Fallback: models not loaded yet and no in-flight promise
		const [models, endpoints] = await Promise.all([
			getModels({ provider: "openrouter" }),
			getModelEndpoints({
				router: "openrouter",
...
function makeOpenRouterErrorReadable(error: any) {
	...
	try {
		const parsedJson = safeJsonParse(error.error.metadata?.raw) as { error?: { details?: Array<{ retryDelay?: any }> } }
