...
> => {
	...
	try {
		...
		const platformStr = process.platform === "darwin" ? "darwin; arm64" : (process.platform === "win32" ? "windows; amd64" : "linux; amd64")
		const userAgent = `GeminiCLI/0.30.0/gemini-2.0-flash (${platformStr}) (Pro)`
