...
const openRouterModelsResponseSchema = z.object({
	data: z.union([
		...
		z.record(z.string(), openRouterModelSchema)
	]),
})
...
export async function getOpenRouterModels(
	...
	try {
		...
		const modelsArray = Array.isArray(data) 
			? data 
			: Object.values(data).filter(model => model && typeof model === "object" && "name" in model && "id" in model);
...
export async function getOpenRouterModelEndpoints(
	...
	try {
		// Extract base model ID by removing any tag suffix like ":thinking"
		const baseModelId = modelId.split(':')[0]
		const response = await axios.get<OpenRouterModelEndpointsResponse>(`${baseURL}/models/${baseModelId}/endpoints`)
		...
		// Return empty object instead of throwing to avoid breaking functionality
	}
