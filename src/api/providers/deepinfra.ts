...
import { deepInfraDefaultModelId, deepInfraDefaultModelInfo, TOOL_PROTOCOL } from "@roo-code/types"
...
export class DeepInfraHandler extends RouterProvider implements SingleCompletionHandler {
	...
	public override async fetchModel() {
		this.models = await getModels({ provider: "deepinfra", apiKey: this.client.apiKey, baseUrl: this.client.baseURL } as const as any)
	...
	override async *createMessage(
		...
		const useNativeTools =
			supportsNativeTools && _metadata?.tools && _metadata.tools.length > 0 && _metadata?.toolProtocol === TOOL_PROTOCOL.MARKDOWN
